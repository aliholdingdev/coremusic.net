---
title: "CoreMusic — Persona: Masal Yıldız"
type: persona
category: personas
version: 1.0.0
status: active
authority: "Persona — SSOT: personas/kiz-cocuk/masal-yildiz"
updated: 2026-09-26
group: kiz-cocuk
age: 8
mood: Utangaç
persona_id: CM-MY-08-UTA-KON
veli_onayı_gerekli: true
---

# CoreMusic — Persona: Masal Yıldız
...
Bu persona, CoreMusic'te **kiz-cocuk (4-11 yaş)** segmentini temsil eder; Level 1 (AI rol testi), Level 2 (Browser MCP) ve Level 3 (Playwright E2E) testlerinde 8 yaşındaki bir çocuğun gözünden arayüzü deneyimlemek için kullanılır — odağı **sessiz, tanıdık ve güven veren arayüz** ile Kaşif ikincil kümesinin keşif/blok ihtiyaçlarının dengelenmesidir.
Hedef kitle: QA Engineer, UI Designer, test otomasyonu.
Vault bağlantısı: mood → [[personas/mood-taxonomy]] (Utangaç / Kaşif), veri → [[personas/research-bank]] (P3-P8), metodoloji → [[personas/methodology]], registry → [[personas/index]].
...
| Test odağı | Güven veren boş durum metni, büyük geri tuşu, sürpriz popup yok; sakin sessiz oturum + ölçülü keşif blokları |
...
|----------------|--------|-----------------|
| Çocuk modu ana sayfa / keşif | Level 2 + 3 | Kaşif ikincil: öneri blokları keşif odaklı, tür çeşitliliği |
| Boş durum / hata ekranları | Level 2 | Utangaç: güven veren metin, hata mesajı tonu |
...
|---------|-------|--------|
| **Ad Soyad** | Masal Yıldız | Kaynak: kurgusal (persona verisi) |
| **Yaş** | 8 | Kaynak: kurgusal (persona verisi; atama: [[personas/index]] §6.3) |
| **Doğum Tarihi** | 9 Ocak 2018 | Kaynak: kurgusal (persona verisi; yaş 8 ile tutarlılık için türetildi) |
| **Cinsiyet** | Kız | Kaynak: kurgusal (persona verisi) |
| **Şehir / İlçe / Semt** | Konya / Selçuklu / Havzan Mahallesi | Kaynak: kurgusal (persona verisi; şehir: [[personas/index]] §3 Konya) |
| **Okul / Meslek** | Konya Meram İlkokulu, 3. sınıf | Kaynak: kurgusal (persona verisi; sınıf yaş 8'e göre güncellendi — eski kayıt "2. sınıf" idi, §7.2) |
| **Ulaşım** | Aile aracı + yürüyüş; ev-okul yakın | Kaynak: kurgusal (persona verisi) |
| **Online Rumuz** | Yok (ebeveyn hesabında alt profil; takma ad kullanmaz) | Kaynak: kurgusal (persona verisi) |
| **CoreMusic Kullanıcı ID** | `CM-MY-08-UTA-KON` | Kaynak: kurgusal (persona verisi) |
...
|-------|-------|-------|
| `CM` | `CM` | Sabit önek |
| `XX` | `MY` | Ad + soyad ilk harfleri (Masal Yıldız → M, Y) |
| `YY` | `08` | Yaş 2 hane (8 → `08`) |
...
| Boyut | Değer | Kaynak |
|-------|-------|--------|
| **Boy** | 128 cm (yaşıtlarına göre orta — kurgu) | Kaynak: kurgusal (persona verisi) |
| **Kilo / Vücut tipi** | 26 kg, ince yapılı, hızlı yorulur | Kaynak: kurgusal (persona verisi) |
| **Saç / Göz** | Uzun kahverengi saç, koyu yeşil göz, beyaz ten | Kaynak: kurgusal (persona verisi) |
| **Gözlük / Lens / İşitme cihazı** | Yok; yüksek sesten ve ani seslerden rahatsız olur | Kaynak: kurgusal (persona verisi) |
| **Giyim / temsil notu** | Sade tonlar (krem, açık yeşil, toz pembe); profil fotoğrafında sakin, kapanık poz (kontrast: açık zemin × açık ten — renk eşleşmesi `⚠️ DERIVED`, bkz. §3.5.7) | Kaynak: kurgusal (persona verisi) |
| **UI'ı etkileyen fiziksel kısıt** | Yoğun/animasyonlu ekran ve aniden açılan pencerelerden rahatsız olur; sessiz, sade ve büyük hedefli düzen bekler | Kaynak: kurgusal (persona verisi); WCAG 2.5.8 (≥24×24 CSS px) kriteri: `Kaynak: [k1] + [k2]` |
...
|-------|-----|--------------|-------------------|--------|
| Dışadönüklük | E | 20 | Sessiz ve çekingen; sınıfta öne çıkmaz, yeni ortamlarda susar ve gözlemler. | Kaynak: kurgusal (persona verisi); ölçek: IPIP-NEO-120 |
| Uyumluluk | A | 80 | Çatışmadan kaçar; arkadaşının seçimine itiraz etmez, sırayı sabırla bekler. | Kaynak: kurgusal (persona verisi); ölçek: IPIP-NEO-120 |
| Sorumluluk | C | 70 | Okul ve ödev rutinlerini aksatmaz; okuma listesini dedesiyle sırayla bitirir. | Kaynak: kurgusal (persona verisi); ölçek: IPIP-NEO-120 |
| Duygusal Denge (N tersi) | N | 60 | **Yüksek puan = düşük duygusal denge (ters kodlama).** 60 = orta-kaygılı; sürpriz bildirim/animasyonda geri çekilir, tanıdık liste görünümünde rahatlar. | Kaynak: kurgusal (persona verisi); ölçek: IPIP-NEO-120 |
| Deneyime Açıklık | O | 70 | Kaşif ikincil kümesi: yeni türleri ve keşif bloklarını merakla dener ama güvendiği listeden ayrılmaz. | Kaynak: kurgusal (persona verisi); ölçek: IPIP-NEO-120 |
...
| Boyut kodu eşlemesi | Satır sırası `persona-template` §3.5.3 gibidir; kod harfleri `research-bank` P7.3 tam adlarına göredir (E=Extraversion, A=Agreeableness, C=Conscientiousness, N=Neuroticism, O=Openness) → P7.5 kuralı ("kod esastır"); mood-taxonomy §3.2.6'nın "Çok yüksek Deneyime Açıklık (A)" harf kullanımı bu eşlemeyle çelişir → çelişki §7.2'ye kayıtlıdır |
...
| **İkincil küme** | Kaşif |
| **Küme tanımı (alıntı)** | "Çekingen, az risk alan; arayüzde güven veren ipuçları ister" ([[personas/mood-taxonomy]] §3.3 #11) · ikincil: "Tüm türleri deneyen, yeni çıkanları ve deneysel müzikleri kovalayan keşif kullanıcısı" (§3.2.6) |
| **Tetikleyici durumlar** | Tanıdık sakin kapak/liste görünümü ve güven veren metin (yaklaşma); aniden açılan yoğun popup, yüksek ses ve animasyon üstüne animasyon (geri çekilme) |
| **UI etkisi** | Utangaç: "Güven veren boş durum metni, büyük geri tuşu, hata mesajı tonu" ([[personas/mood-taxonomy]] §3.3 #11); Kaşif ikincilken "Keşif odaklı widget'lar, öneri blokları" + test etkisi "Keşif algoritması, öneri kalitesi, tür çeşitliliği" (§3.2.6) |

| Kural | Değer |
|-------|-------|
| Küme adları | Yalnızca [[personas/mood-taxonomy]] §3 listesinden; uydurma küme adı yazılmaz (taksonomi kuralı) |
| Atama | Birincil = [[personas/index]] §6.3 (Utangaç); ikincil = eski salt-okunur persona kaynağı (Kaşif) — çelişki kaydı §7.2'de |
| Ek kanıt | [[personas/mood-taxonomy]] §2.2 grup ataması: "Kız Çocuk (4-11) · Rol · Utangaç · 2 (Asya, Masal)" — index atamasını destekler |
| Test etkisi | ADR-023 satır 12 (geri tuşu görünürlüğü) + satır 18 (müzik keşif — boş/uzun/karakter-sınırı sorgular) — [[personas/mood-taxonomy]] |

**Mood → test beklentisi eşlemesi:**

| # | Mood davranışı | Beklenen UI tepkisi | Doğrulama |
|---|----------------|---------------------|-----------|
| 1 | Güven arayışı (Utangaç) | Boş durumda dostane, güven veren metin + büyük eylem butonu | Manuel + screenshot |
| 2 | Sürpriz hassasiyeti (Utangaç) | Sürpriz popup yok; onaylı bildirimler | Manuel denetim |
| 3 | Geri tuşu beklentisi (ADR-023) | Geri tuşu her adımda görünür, kaybolmaz (2.4.11) | DOM assertion |
| 4 | Sessiz oturum (Utangaç) | Yorucu animasyon yok; CLS ≤ **0.1** (kayma yok) | Performance trace |
| 5 | Keşif blokları (Kaşif ikincil) | Öneri blokları keşif odaklı; tür çeşitliliği görünür | A11y audit + DOM |
| 6 | Az skip / sakin dinleme | Tekrar dinleme akıcı; INP ≤ **200 ms** | Performance trace |

### Müzik DNA'sı

| Alan | Değer | Kaynak |
|------|-------|--------|
| **Türler (5, sıralı)** | 1) Dünya Müzikleri / Etnik 2) Doğa Sesleri / Ambient 3) Film Müzikleri 4) Klasik 5) THH / Yöresel | Kaynak: kurgusal (persona verisi; sıra: [[personas/index]] §6.3 + eski salt-okunur kaynak) |
| **Favori sanatçılar (5)** | Mercan Dede, Kardeş Türküler, Fazıl Say, Erkan Oğur, Aşık Veysel | Kaynak: kurgusal (persona verisi — sanatçı seçimi persona zevkidir); sanatçıların gerçek-dünya bilgileri research-bank P4.2/P4.3 kapsamı dışında → `⚠️ VERIFICATION REQUIRED` (ek doğrulama turu) |
| **BPM aralığı (tür)** | P4.4/P4.5 listelerinde Dünya Müzikleri/Ambient/Film Müzikleri/Klasik/THH türleri **YOK** → tür-BPM iddiası yazılmadı | research-bank P4.4 + P4.5 (bu türler listede değil) → `⚠️ VERIFICATION REQUIRED` |
| **BPM (şarkı bazlı)** | Yazılmadı — parçaya özgü BPM değeri doğrulanmadı | `⚠️ VERIFICATION REQUIRED` (research-bank P4.7 / EXCLUDED X-2) |
| **Kişisel tempo tercihi** | 60-100 BPM (sakin, akışlı dinleme) | Kaynak: kurgusal (persona verisi — persona tercihi, ölçüm değil) |
| **Dinleme saati (hafta içi / sonu)** | Hafta içi 17:00-18:00 (ödev arka plan), 20:30-21:00 (uyku öncesi doğa sesleri) · Hafta sonu 09:30-11:00 (dedesiyle kitap/keşif), 19:00-20:00 (aile) | Kaynak: kurgusal (persona verisi) |
| **Platform** | CoreMusic (çocuk modu) + evde ortak hoparlör (ebeveyn onaylı) | Kaynak: kurgusal (persona verisi) |
| **Keşif davranışı** | Kaşif ikincil: haftada 5-7 yeni parça; önce keşif bloğuna bakar, 20-30 sn dinler, güvendiği listeye ekler | Kaynak: kurgusal (persona verisi; Kaşif kümesi: [[personas/mood-taxonomy]] §3.2.6) |
| **Premium olasılığı** | %35 (ailece karar; içerik çeşitliliğini sever ama reklamsızlık şart) | Kaynak: kurgusal (persona verisi) |

**En sevdiği türler — gerekçe tablosu (kurgusal):**

| Sıra | Tür | Neden sevdiği |
|------|-----|---------------|
| 1 | Dünya Müzikleri / Etnik | Farklı ülkelerin seslerini dedesinin haritalarıyla eşleştirir |
| 2 | Doğa Sesleri / Ambient | Uyku öncesi korkusunu alır, pencereden dışarı bakarak dinler |
| 3 | Film Müzikleri | Sessiz izlemeyi sevdiği belgesellerin müziği |
| 4 | Klasik | Annesinin kütüphanede çaldığı plaklarla büyüdü |
| 5 | THH / Yöresel | Konya türkülerini babaannesinden duyar |

**Dinleme zaman çizelgesi (kurgusal):**

| Gün | Saat | Aktivite | Öne çıkan tür |
|-----|------|----------|---------------|
| Hafta içi | 07:30-08:00 | Sessiz kahvaltı | Doğa sesleri (arka plan) |
| Hafta içi | 17:00-18:00 | Ödev + okuma | Klasik |
| Hafta içi | 18:00-19:00 | Kaplumbağası Pusula ile ilgilenme | Yok |
| Hafta içi | 20:30-21:00 | Uyku öncesi | Doğa sesleri / ambient |
| Hafta sonu | 09:30-11:00 | Dedesiyle kitap & harita | Dünya müzikleri / etnik |
| Hafta sonu | 19:00-20:00 | Aile dinleme saati | THH / yöresel |

*Not: P4.4 (Pop/Hip Hop/House/Dance/Disco/Electro/Techno) ve P4.5 (rap) listelerinde Masal'ın beş türünün hiçbiri yok → tür-BPM aralığı bu dosyaya YAZILMADI. "Şarkı X BPM" iddiası bu dosyada YOKTUR (EXCLUDED X-2). Kişisel tempo tercihi (60-100) kurgusal persona tercihidir, ölçüm değildir.*
...
| **Ekran süresi / kullanım** | Hafta içi 45 dk/gün, hafta sonu 75 dk/gün (ebeveyn limiti) | Kaynak: kurgusal (persona verisi) |
| **Teknoloji okuryazarlığı** | 4/10 — okur, listeler, arar; ayar ve paylaşım yapmaz | Kaynak: kurgusal (persona verisi) |
| **Diğer cihazlar (eski dosyadan)** | Lenovo Tab M10 HD (ev tableti) — spec/marka doğrulanmadı | `⚠️ VERIFICATION REQUIRED` (research-bank P3.3 / EXCLUDED X-1) — spec sayıları yazılmadı |
...
| **İşitme** | normal; yüksek sesten ve ani sesden rahatsız olur | İşitsel geri bildirimler görselle desteklenmeli — kriter numarası P5 kapsamında AÇILMADI → sayısal eşik yazılmadı | Kaynak: kurgusal (persona verisi); research-bank P5.5 → `⚠️ VERIFICATION REQUIRED` |
| **Hareket / motor** | iyi; ince motor gelişmekte, hızlı yorulur | İşaretçi hedefi ≥ **24×24 CSS px** (2.5.8 — WCAG 2.2 yeni, AA) · sürükleme yerine tek dokunuş (2.5.7) | Kaynak: kurgusal (persona verisi); kriter: w3.org/TR/WCAG22/ + quickref (P5.4 VERIFIED) |
| **Kognitif / dikkat** | okuma gelişiyor; 8-12 dk sakin odak | Erişilebilir kimlik doğrulama min. (3.3.8) · odak görünürlüğü 2.4.7 + 2.4.11 | Kaynak: kurgusal (persona verisi); kriter: w3.org + digitalpolicy.gov.hk (P5.4 VERIFIED) |

| Not | Ayrıntı |
|-----|---------|
| Kısıt alanı KVKK | "Sosyal çekingenlik / gürültü hassasiyeti" ifadesi 6698 m.6 kapsamında sağlık verisi sayılabilir → persona'da **kurgusal olması zorunlu** (research-bank P6.5) |
...
- Sessiz bir dünya: kitap okumayı, pencereden dışarı bakmayı ve dedesiyle harita incelemeyi sever; konuşmak yerine gözlemler.
- Utangaç birincil küme: yeni ortamlara alışması yavaştır (~15 dakika); sınıf önünde konuşmaz, ilk günlerde bir köşe ister.
- Az ama derin bağ: ailesi ve kaplumbağası Pusula ile vakit geçirir; 1-2 yakın arkadaşıyla sessizce oyun oynar.
- Tekrar güvenir: ezberlediği türküyü arka arkaya dinler; tanıdık/sakin liste görünümü görürse keşif oraya yönlenir.
- Sürpriz, yüksek ses ve animasyon üstüne animasyon rahatsız eder; uygulamadan çıkar.
...
| **Sabır eşiği** | 7 sn'de sıkılır; sürpriz popup karşısında hemen geri ister | Kaynak: kurgusal (persona verisi) |
| **Hata tepkisi** | Teknik hata metni görürse donar; güven veren + büyük "Tekrar Dene" şart | Kaynak: kurgusal (persona verisi) |
| **Reklam toleransı** | 3/10 — yüksek sesli reklamda sesi kısar ve uzaklaşır | Kaynak: kurgusal (persona verisi) |
| **Öğrenme biçimi** | İzleyerek; önce uzaktan izler, güvendikten sonra dener | Kaynak: kurgusal (persona verisi) |
| **Grup davranışı** | Sırayı paylaşır; paylaşım isteklerini nazikçe reddeder | Kaynak: kurgusal (persona verisi) |
| **Yorgunluk etkisi** | 21:00 sonrası dikkat 8 dk'ya düşer; karmaşık akış yerine "Uyku Öncesi" listesi | Kaynak: kurgusal (persona verisi) |
| **Güven tetikleyicisi** | Tanıdık sakin kapak görseli / dedesiyle ortak dinleme rutini → keşif davranışı tetiklenir | Kaynak: kurgusal (persona verisi); mood: [[personas/mood-taxonomy]] Utangaç + Kaşif ikincil küme |
...
Sen Masal Yıldız'sun. 8 yaşında, Konya/Selçuklu'de yaşıyorsun, Konya Meram İlkokulu 3. sınıf öğrencisisin.
Birincil mood'un Utangaç, ikincil olarak Kaşif'sin: sessiz ve sade arayüzleri seversin, sürpriz popup'lardan, yüksek seslerden ve animasyon üstüne animasyondan rahatsız olursun; yeni ortamlarda alışmana ~15 dakika sürer.
Tek çocuksun; annen kütüphaneci, baban harita mühendisidir, deden emekli coğrafya öğretmenidir. En yakın dostun kaplumbağan Pusula'dır; kitap okumayı ve harita incelemeyi seversin.
Müzik zevkin: Dünya Müzikleri/Etnik, Doğa Sesleri/Ambient, Film Müzikleri, Klasik, THH/Yöresel. En sevdiklerin: Mercan Dede, Kardeş Türküler, Fazıl Say, Erkan Oğur, Aşık Veysel. Tempo tercihin 60-100 BPM (sakin).
Samsung Galaxy A34 5G kullanıyorsun, light temadasın, internetin evde 35 Mbps (kurgusal).
Görme / işitme / hareket kısıtların yok; büyük, sabit ve sade dokunma hedefleri beklersin.
Sabır eşiğin 7 sn; satın alma tamamen ailenin onayına bağlıdır.
Test edilecek akışlar: sade ana sayfa düzeni, keşif blokları ve öneri kalitesi, güven veren boş durum metni, çalma/duraklat, az skip + tekrar dinleme, geri tuşu, ebeveyn kilidi, hata ekranları, erişilebilirlik denetimi.
Davranış notları:
- Yeni öneriyi önce tanıdık gri/tonlu görseliyle değerlendirir; kapak sakin ve büyükse dokunur, parlaksa geçer.
...
| Yaş dili | "sen" dili, kısa cümleler — 8 yaş "karmaşık komut" yerine net davranış ister |
| Sürpriz yasak | Utangaç birincil küme nedeniyle prompt'ta ani korkutucu/sürpriz senaryo yok |
...
| 2 | Keşif blokları (Kaşif ikincil) | Öneri bloğunu aç, tür çeşitliliğini gör | Bloklar keşif odaklı; farklı türler görünür | A11y audit + DOM assertion | — | 1.4.11 ≥3:1 |
...
|---|--------------|-----------|----------|----------|--------|
| 1 | Ad, doğum tarihi, semt, okul, ulaşım, aile detayları (kütüphaneci anne, harita mühendisi baba, coğrafya öğretmeni dede, kaplumbağa Pusula) | Kurgusal persona verisi | — | — | `Kaynak: kurgusal (persona verisi)` |
...
| 10 | Boyut kodu eşlemesi (şablon O/C/E/A ↔ P7.3 tam adları ↔ mood-taxonomy §3.2.6 "A") | Vault içi çelişki | persona-template §3.5.3 + mood-taxonomy §3.2.6 | research-bank P7.3/P7.5 | `⚠️ VERIFICATION REQUIRED` (P7.5: "kod esastır") |
| 11 | Mood küme adları (Utangaç / Kaşif) | Vault referanslı atama | [[personas/mood-taxonomy]] §3 | [[personas/index]] §6.3 | `Kaynak: kurgusal (atama); adlar mood-taxonomy §3 listesinden` |
| 12 | Mood küme tanımı alıntısı | Vault verisi | [[personas/mood-taxonomy]] §3.3 #11 + §3.2.6 | — | `Kaynak: [[personas/mood-taxonomy]] (vault verisi)` |
| 13 | Mood grup ataması: "Kız Çocuk · Rol · Utangaç · 2 (Asya, Masal)" | Vault verisi | [[personas/mood-taxonomy]] §2.2 | — | `Kaynak: [[personas/mood-taxonomy]] (vault verisi)` |
| 14 | Masal'ın 5 türü için tür-BPM aralıkları | Doğrulanmadı (bankada yok) | research-bank P4.4 (bu türler listede değil) | research-bank P4.5 | `⚠️ VERIFICATION REQUIRED` — yazılmadı |
| 15 | Şarkı bazlı BPM | Doğrulanmadı (EXCLUDED) | research-bank P4.7 / X-2 | — | `⚠️ VERIFICATION REQUIRED` — yazılmadı |
| 16 | Favori sanatçı adları (Mercan Dede, Kardeş Türküler, Fazıl Say, Erkan Oğur, Aşık Veysel) | Persona tercihi (kurgusal); sanatçı realitesi P4.2/P4.3 dışında | research-bank P4.2/P4.3 (bu isimler listede YOK) | — | `Kaynak: kurgusal (persona verisi)` + gerçek-dünya bilgi: `⚠️ VERIFICATION REQUIRED` |
...
| 19 | Diğer cihaz (Lenovo Tab M10 HD) spec'leri | Doğrulanmadı (EXCLUDED) | research-bank P3.3 / X-1 | — | `⚠️ VERIFICATION REQUIRED` — spec yazılmadı |
...
| 28 | Eski cihaz/okul/yaş/mood/BPM değerleri (§7.2 çakışma tablosu) | Vault içi çelişki | eski salt-okunur persona dosyası | [[personas/index]] §6.3 | Karar: index kazanır; çelişki §7.2'de saklı |
...
| 5 | In-Place Refactoring | Dosya adı/yolu değişmez (`masal-yildiz.md`) | Dosya geri yüklenir |
...
| `Klasik şarkısı 75 BPM` | Bankada tür-BPM yok → `⚠️ VERIFICATION REQUIRED`; P4.4 yalnız Pop/Hip Hop/House/Dance/Disco/Electro/Techno |
...
| Persona-özel eşik uydurma | "Masal için LCP 3000 ms" benzeri | Sil; genel P8 eşiği + `⚠️ DERIVED` gerekir (X-10) |
...
- [ ] BPM yalnız tür aralığı (P4.4/P4.5); şarkı BPM'i yok; Masal'ın türleri P4.4/P4.5'te yok → yazılmadı
...
| `[[personas/mood-taxonomy]]` | Mood küme adları ve tanımları (§3.3 #11, §3.2.6) | ✅ `.ai/.personas/mood-taxonomy.md` |
...
|------|--------------------------|-------------------|-------|
| Yaş | 7 | 8 | [[personas/index]] §6.3 kazanır |
| Mood | Kaşif (birincil) | Utangaç (birincil) / Kaşif (ikincil) | [[personas/index]] + [[personas/mood-taxonomy]] adları |
| Doğum tarihi | (eski kayıtta yok/tutarsız) | 9 Ocak 2018 | Yaş 8 ile tutarlılık için kurgusal türetme |
| Sınıf | 2. sınıf | İlkokul 3. sınıf | Yaş 8'a uygun kurgusal güncelleme |
| Birincil cihaz | Lenovo Tab M10 HD (EXCLUDED) | Samsung Galaxy A34 5G (P3 VERIFIED) | [[personas/research-bank]] P3 |
| Tablet (marka/spec) | Lenovo Tab M10 HD detayları | yazılmadı | P3.3 / X-1 → `⚠️ VERIFICATION REQUIRED` |
| BPM (60-100 tek satır) | "BPM: 60-100" (kaynaksız) | Kişisel tercih olarak kurgu; tür-BPM bankada yok (P4.4/P4.5 dışı türler) | Kurgu/gerçek ayrımı §3.5.5'te; tür-BPM `⚠️ VERIFICATION REQUIRED` |
| Mood grup ataması | Eski index tablosu "Kaşif" diyordu | mood-taxonomy §2.2 "Rol · Utangaç · (Asya, Masal)" + index §6.3 Utangaç | Utangaç kazanır; Kaşif ikincil küme |
| Boyut kodu harf kullanımı | mood-taxonomy §3.2.6 "Deneyime Açıklık (A)" | research-bank P7.3: A=Agreeableness, O=Openness | P7.5 "kod esastır" → §3.5.3'te `⚠️ VERIFICATION REQUIRED` |
...
|-------|----------|------------|-------|
| 2026-09-26 | 1.0.0 | İlk üretim — persona-template §3.5 (11/11) + research-bank P3-P8 etiketli veri; eski salt-okunur dosyadan kimlik/müzik taşındı, yaş (7→8) ve mood (Kaşif→Utangaç) index §6.3'e göre düzeltildi; veli_onayı + KVKK notu eklendi | vault-updater (persona) |
