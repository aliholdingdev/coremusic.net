---
title: "CoreMusic — Persona: Peri Bulut"
type: persona
category: personas
version: 1.0.0
status: active
authority: "Persona — SSOT: personas/kiz-cocuk/peri-bulut"
updated: 2026-09-26
group: kiz-cocuk
age: 11
mood: Enerjik
persona_id: CM-PB-11-ENE-DIY
...
|----------------|--------|-----------------|
| Çocuk modu ana sayfa / keşif | Level 2 + 3 | Enerjik: canlı öneriler, hızlı geçiş; Kaşif ikincil: tür çeşitliliği |
| Hızlı navigasyon & skip | Level 3 | Enerjik: sık skip, anlık geri bildirim, INP ≤ 200 ms |
| Boş durum / hata ekranları | Level 2 | Güven veren metin, hata mesajı tonu |
| Geri navigasyon & odak | Level 2 + 3 | Büyük geri tuşu beklentisi (2.4.11), hızlı ama öngörülebilir |
| Ebeveyn kilidi & veli onayı akışı | Level 2 | 3.3.8 erişilebilir kimlik doğrulama; sürpriz yok |
| Erişilebilirlik denetimi (kontrast / hedef boyu) | Level 2 | 2.5.8 ≥24×24 px, 1.4.3 ≥4.5:1, yüksek kontrast canlı tema |
...
Kapsam dışı istisnalar: eski vault'taki ayrıntılı fiziksel/ekonomik veri bu şablonda yasaktır (§3.5.2 → 5-8 satır; persentil, ayakkabı no, kan grubu, burç, aile geliri YAZILMAZ); bankada olmayan gerçek-dünya iddiası yazılmaz (§4.5); şarkı bazlı BPM yazılmaz (P4.7 / X-2).
...
| **Okul / Meslek** | Diyarbakır TED Koleji, 6. sınıf | Kaynak: kurgusal (persona verisi; sınıf yaş 11'e göre güncellendi — eski kayıt "4. sınıf" idi, §7.2) |
| **Kardeş** | 2 (abi Rojhat, abla Dilan) — en küçük | Kaynak: kurgusal (persona verisi) |
| **Diller** | Türkçe + Kürtçe/Zazaca (evde üç dil) · İngilizce (ikinci dil) | Kaynak: kurgusal (persona verisi) |
| **Ulaşım** | Aile aracı + yürüyüş (dedesiyle) | Kaynak: kurgusal (persona verisi) |
| **Online Rumuz** | Yok (ebeveyn hesabında alt profil) | Kaynak: kurgusal (persona verisi) |
| **CoreMusic Kullanıcı ID** | `CM-PB-11-ENE-DIY` | Kaynak: kurgusal (persona verisi) |
...
|-------|-------|-------|
| `CM` | `CM` | Sabit önek |
| `XX` | `PB` | Ad + soyad ilk harfleri (Peri Bulut → P, B) |
| `YY` | `11` | Yaş 2 hane (11 → `11`) |
| `ZZZ` | `ENE` | Mood tür kodu (Enerjik → ENE) |
| `XXX` | `DIY` | Şehir kodu (Diyarbakır → DIY) |
...
| Boyut | Değer | Kaynak |
|-------|-------|--------|
| **Boy** | 145 cm (yaşıtlarına göre orta — kurgu) | Kaynak: kurgusal (persona verisi) |
| **Kilo / Vücut tipi** | 36 kg, atletik (dedesiyle yürüyüş — kurgu) | Kaynak: kurgusal (persona verisi) |
| **Saç / Göz** | Siyah gür dalgalı saç, kahverengi göz, buğday ten | Kaynak: kurgusal (persona verisi) |
| **Gözlük / Lens / İşitme cihazı** | Yok; meraklı ve hızlı bakışlı | Kaynak: kurgusal (persona verisi) |
| **Giyim / temsil notu** | Rahat, bol cepli (toprak kahvesi, zeytin yeşili, güneş sarısı); profil fotoğrafında açık havada, doğal poz (kontrast: toprak tonları × buğday ten — renk eşleşmesi `⚠️ DERIVED`, bkz. §3.5.7) | Kaynak: kurgusal (persona verisi) |
| **UI'ı etkileyen fiziksel kısıt** | Hareketli ve hızlı; uzun bekleme ekranlarından sıkılır → anlık geri bildirim + yaşa uygun yükleme animasyonu şart | Kaynak: kurgusal (persona verisi); WCAG 2.5.8 (≥24×24 CSS px) kriteri: `Kaynak: [k1] + [k2]` |
...
|-------|-----|--------------|-------------------|--------|
| Dışadönüklük | E | 70 | Müzede yabancılara eser anlatır; üç dilin verdiği özgüvenle rahat konuşur. | Kaynak: kurgusal (persona verisi); ölçek: IPIP-NEO-120 |
| Uyumluluk | A | 70 | En küçük çocuk olmanın getirdiği uyumluluk; abi/ablasıyla iyi geçinir. | Kaynak: kurgusal (persona verisi); ölçek: IPIP-NEO-120 |
| Sorumluluk | C | 60 | Arkeoloji notları düzenlidir; günlük işlerde dalgınlaşabilir. | Kaynak: kurgusal (persona verisi); ölçek: IPIP-NEO-120 |
| Duygusal Denge (N tersi) | N | 30 | **Yüksek puan = düşük duygusal denge (ters kodlama).** 30 = dengeli; toprak, tarih ve doğa onu sakinleştirir. | Kaynak: kurgusal (persona verisi); ölçek: IPIP-NEO-120 |
| Deneyime Açıklık | O | 95 | Üç dil, üç kültür, sonsuz merak — yeni tür ve keşif bloklarını hiç atlamaz. | Kaynak: kurgusal (persona verisi); ölçek: IPIP-NEO-120 |

| Kural | Değer |
|-------|-------|
| Puan aralığı | 0-100; normalize yöntem `⚠️ DERIVED` (research-bank P7.4: "resmi ölçek 0-100 değildir; yöntem türetilmiştir") |
| Puanların kendisi | Kurgusal persona verisi (`Kaynak: kurgusal (persona verisi)`) — IPIP-NEO ile **ölçülmemiştir**; eski dosyadaki 7/10 benzeri skorlar 0-100 ölçeğe kurgusal olarak ölçeklendi (§7.2) |
| Ölçek künyesi | IPIP-NEO-120, Johnson (2014) → `Kaynak: novopsych.com (Technical Paper) + APA PsycNET (PsycTests kaydı)` (research-bank P7.4 VERIFIED) |
| Türkçe geçerlilik | Bulunamadı (P7.5 / X-9) → "IPIP-NEO Türkçe geçerli" iddiası yazılmadı, `⚠️ VERIFICATION REQUIRED` |
| Boyut kodu eşlemesi | Satır sırası `persona-template` §3.5.3 gibidir; kod harfleri `research-bank` P7.3 tam adlarına göredir (E=Extraversion, A=Agreeableness, C=Conscientiousness, N=Neuroticism, O=Openness) → P7.5 kuralı ("kod esastır"); mood-taxonomy §3.2.2'nin "Yüksek Dışadönüklük (O), yüksek Deneyime Açıklık (A)" harf kullanımı bu eşlemeyle çelişir → çelişki §7.2'ye kayıtlıdır |
...
|---|----------------|---------------------|-----------|
| 1 | Hızlı navigasyon (Enerjik) | Geçişler akıcı; anlık geri bildirim | Performance trace |
| 2 | Sık skip (Enerjik) | Skip sonrası INP ≤ **200 ms** | Performance trace |
| 3 | Canlı renk/yüksek kontrast (Enerjik) | Metin ≥ **4.5:1** (1.4.3), UI ≥ **3:1** (1.4.11) | A11y audit |
| 4 | Keşif blokları (Kaşif ikincil) | Öneri blokları keşif odaklı; tür çeşitliliği görünür | A11y audit + DOM |
| 5 | Geri tuşu beklentisi (ADR-023) | Geri tuşu her adımda görünür, kaybolmaz (2.4.11) | DOM assertion |
| 6 | Uzun bekleme hassasiyeti (Enerjik) | Yükleme yaşa uygun animasyonlu; CLS ≤ **0.1** | Performance trace |
...
| **Kişisel tempo tercihi** | 60-100 BPM (sabırlı keşif — eski dosya tercihi, kurgu) | Kaynak: kurgusal (persona verisi — persona tercihi, ölçüm değil) |
| **Dinleme saati (hafta içi / sonu)** | Hafta içi 16:00-17:00 (yürüyüş/kitap), 21:00-21:30 (uyku öncesi) · Hafta sonu 09:00-11:00 (dedesiyle doğa), 20:00+ (aile) | Kaynak: kurgusal (persona verisi) |
| **Platform** | CoreMusic (çocuk modu) + kısa dalga radyo (eski kayıt — `⚠️ VERIFICATION REQUIRED`, §7.2) | Kaynak: kurgusal (persona verisi) |
| **Keşif davranışı** | Enerjik + Kaşif: haftada 8-10 yeni parça; hızlı skip, beğendiği an listesine alır | Kaynak: kurgusal (persona verisi; mood: §3.2.2 + §3.2.6) |
| **Premium olasılığı** | %50 (amaca göre; keşif çeşitliliği artıyorsa ailece karar) | Kaynak: kurgusal (persona verisi) |
| **Playlist düzeni** | 18 playlist (her medeniyet için bir tane), ~210 favori | Kaynak: kurgusal (persona verisi) |

**En sevdiği türler — gerekçe tablosu (kurgusal):**

| Sıra | Tür | Neden sevdiği |
|------|-----|---------------|
| 1 | Etnik / Geleneksel | Mezopotamya, Anadolu, Orta Doğu kadim müzikleri — annesiyle kazı bağı |
| 2 | Dünya Müziği | "Her kültür bir tarih dersi" — üç dilin verdiği merak |
| 3 | Türk Halk Müziği (Yöresel) | "Her türkü bir tarih belgesi" — dedesiyle yürüyüş rutini |
| 4 | Klasik (Antik) | Antik Yunan modları, Rönesans — etnomüzikoloji hayali |
| 5 | Soundtrack | Tarih belgesellerinin müzikleri — aile izleme rutini |

**Dinleme zaman çizelgesi (kurgusal):**

| Gün | Saat | Aktivite | Öne çıkan tür |
|-----|------|----------|---------------|
| Hafta içi | 16:00-17:00 | Dedeyle yürüyüş + not alma | Dünya müziği |
| Hafta içi | 17:00-18:00 | Ödev | Klasik (antik) |
| Hafta içi | 18:00-19:00 | Bahçede "kazı alanı" | Etnik / geleneksel |
| Hafta içi | 21:00-21:30 | Uyku öncesi | Soundtrack (sakin) |
| Hafta sonu | 09:00-11:00 | Doğa yürüyüşü | THH / yöresel |
| Hafta sonu | 20:00-21:00 | Aile dinleme saati | Dünya müziği |

*Not: BPM satırları yalnız kurgusal tercih düzeyindedir; P4.4 (Pop/Hip Hop/House/Dance/Disco/Electro/Techno) ve P4.5 (rap) listelerinde Peri'nin beş türünün hiçbiri yok → tür-BPM aralığı bu dosyaya YAZILMADI. Mood-taxonomy §3.2.2'nin Enerjik küme notu "120-160 BPM ⚠️" de birincil ölçüm değildir; test hedefi olarak VERIFIED tür aralıkları (varsa) kullanılır. "Şarkı X BPM" iddiası bu dosyada YOKTUR (EXCLUDED X-2).*
...
| **Tema** | light (canlı renkli, yüksek kontrast tercih — mood §3.2.2) | Kaynak: kurgusal (persona verisi) |
| **Ekran süresi / kullanım** | Hafta içi 70 dk/gün, hafta sonu 110 dk/gün (ebeveyn limiti) | Kaynak: kurgusal (persona verisi) |
| **Teknoloji okuryazarlığı** | 6/10 — okur, listeler, arar, playlist kurar; paylaşım az | Kaynak: kurgusal (persona verisi) |
| **Diğer cihazlar (eski dosyadan)** | Samsung tablet + kısa dalga radyo (eski kayıt) — spec/marka doğrulanmadı | `⚠️ VERIFICATION REQUIRED` (research-bank P3.3 / EXCLUDED X-1) — spec sayıları yazılmadı |
...
|------|-------|-------------|--------|
| **Görme** | yok (kurgu) | Metin kontrastı ≥ **4.5:1** (1.4.3), UI/grafik ≥ **3:1** (1.4.11) — canlı renkli temada bu eşikler özellikle kritik | Kaynak: kurgusal (persona verisi); kriter: w3.org/TR/WCAG22/ + w3.org/WAI/WCAG22/quickref/ (P5.4 VERIFIED) |
...
| **Hareket / motor** | iyi; hızlı ve atak | İşaretçi hedefi ≥ **24×24 CSS px** (2.5.8 — WCAG 2.2 yeni, AA) · sürükleme yerine tek dokunuş (2.5.7) | Kaynak: kurgusal (persona verisi); kriter: w3.org/TR/WCAG22/ + quickref (P5.4 VERIFIED) |
| **Kognitif / dikkat** | meraklı, 15-20 dk odak | Erişilebilir kimlik doğrulama min. (3.3.8) · odak görünürlüğü 2.4.7 + 2.4.11 | Kaynak: kurgusal (persona verisi); kriter: w3.org + digitalpolicy.gov.hk (P5.4 VERIFIED) |

| Not | Ayrıntı |
|-----|---------|
| Kısıt alanı KVKK | Dil/etnik köken gibi veriler 6698 kapsamında hassas sayılabilir → persona'da **kurgusal olması zorunlu**, bu dosyada gerçek kişi verisi YOK |
...
- Meraklı kaşif: annesi arkeolog, kazı alanlarında büyüdü; müzik onun için "zaman makinesi".
- Üç dilli büyüme → farklı bakış açıları; hikâye anlatıcılığı güçlü.
- Hareketli ve sabırsız: hızlı skip, anlık geri bildirim bekler; uzun beklemeden sıkılır.
- Doğa ve toprak bağı güçlü; dedesiyle yürüyüş ve not tutma rutini.
- Hayali etnomüzikolog ve arkeolog olmak.
...
| **Sabır eşiği** | 4 sn — hızlıdır, atlar | Kaynak: kurgusal (persona verisi) |
| **Hata tepkisi** | Hata görürse nedenini ister; "Tekrar Dene" + kısa açıklama | Kaynak: kurgusal (persona verisi) |
| **Reklam toleransı** | 3/10 — akışı bozduğu için hızla geçer | Kaynak: kurgusal (persona verisi) |
...
Sen Peri Bulut'sun. 11 yaşında, Diyarbakır/Kayapınar'da yaşıyorsun, Diyarbakır TED Koleji 6. sınıf öğrencisisin.
Birincil mood'un Enerjik, ikincil olarak Kaşif'sin: hızlı ilerlersin, sık sık parça değiştirirsin, beğendiğin anda listene eklersin; aynı zamanda yeni türleri ve keşif bloklarını hiç atlamazsın.
İki kardeşlisin (abi Rojhat, abla Dilan) ve en küçüğüsün; evde Türkçe, Kürtçe/Zazaca konuşulur, İngilizce de bilirsin. Annen arkeolog, baban ziraat mühendisidir; dedesiyle yürüyüş yapmayı ve not tutmayı seversin.
Müzik zevkin: Etnik/Geleneksel, Dünya Müziği, THH/Yöresel, Klasik (Antik), Soundtrack. En sevdiklerin: Mercan Dede, Kardeş Türküler, Aynur Doğan, Loreena McKennitt, Dead Can Dance. Tempo tercihin 60-100 BPM (sabırlı keşif, kurgusal).
Samsung Galaxy A34 5G kullanıyorsun, light ve canlı renkli temadasın, internetin evde 75 Mbps (kurgusal).
...
Test edilecek akışlar: hızlı navigasyon + anlık geri bildirim, sık skip ve performansı, keşif blokları ve öneri kalitesi, çalma/duraklat, büyük geri tuşu, ebeveyn kilidi, hata ekranları, erişilebilirlik denetimi (yüksek kontrast).
Davranış notları:
- Yeni öneriyi hızla tarar; kapak dikkatini çekerse dokunur, değilse anında geçersin.
- Uzun yükleme görürsen sıkılırsın; yaşa uygun, canlı yükleme animasyonu ve anlık geri bildirim istersin.
- Boş listede "kaydın yok" gibi güven veren metin ve büyük bir "Keşfet" butonu beklersin.
- Türkçe telaffuzun nettir; arayüzde kısa, "sen" diliyle yazılmış ve hızlı okunan metinler beklersin.
...
| Yaş dili | "sen" dili, kısa cümleler — 11 yaş net ve hızlı davranış ister |
| Hız yasak değil | Enerjik küme nedeniyle prompt'ta hızlı akış varsayılır; sürpriz/korkutucu senaryo yine yasak |
...
|---|------|-------|----------|-----------|--------|------|
| 1 | Ana sayfa yükleme (çocuk modu) | CoreMusic'i aç, canlı düzeni bekle | Öneri kartları üstte, sakin yerleşim, yaşa uygun yükleme animasyonu | Performance trace + screenshot | LCP ≤ **2500 ms** · CLS ≤ **0.1** | 1.4.11 ≥3:1 |
| 2 | Hızlı navigasyon (Enerjik) | 5 sayfayı hızlıca gez | Geçiş akıcı, anlık geri bildirim | Performance trace | INP ≤ **200 ms** | — |
| 3 | Sık skip (Enerjik) | 8 parçayı üst üste skip'le | Her skip anında tepki; takılma yok | Performance trace + console log | INP ≤ **200 ms** | — |
...
| 5 | Keşif blokları (Kaşif ikincil) | Öneri bloğunu aç, tür çeşitliliğini gör | Bloklar keşif odaklı; farklı türler görünür | A11y audit + DOM assertion | — | 1.4.11 ≥3:1 |
...
| 13 | Erişilebilirlik denetimi | Lighthouse + axe koş | 1.4.3 / 1.4.11 / 2.5.7 / 2.5.8 ihlali yok (canlı temada) | Lighthouse / axe | A11y skoru raporlanır | 1.4.3 ≥4.5:1 |
...
| 7 | 0-100 normalize yöntemi + eski 7/10 → 70 ölçeklemesi | Türetilmiş hesaplama | research-bank P7.4 (IPIP-NEO-120 yapısı) | persona-template §3.5.3 | `⚠️ DERIVED` |
...
| 19 | Diğer cihazlar (Samsung tablet, kısa dalga radyo) spec'leri | Doğrulanmadı (EXCLUDED) | research-bank P3.3 / X-1 | — | `⚠️ VERIFICATION REQUIRED` — spec yazılmadı |
...
| `Etnik şarkı 75 BPM` | Bankada tür-BPM yok → `⚠️ VERIFICATION REQUIRED`; P4.4 yalnız Pop/Hip Hop/House/Dance/Disco/Electro/Techno |
...
| `Aile geliri 60.000-75.000 TL/ay` (eski dosya) | Bankada YOK → yazılmadı; yalnız `Kaynak: kurgusal` kimlik satırı |
...
| Persona-özel eşik uydurma | "Peri için LCP 3000 ms" benzeri | Sil; genel P8 eşiği + `⚠️ DERIVED` gerekir (X-10) |
...
- [ ] BPM yalnız tür aralığı (P4.4/P4.5); şarkı BPM'i yok; Mood-taxonomy BPM notları test eşiği yapılmadı
...
| Mood grup ataması | Eski index "Kaşif" | index §6.3 "Enerjik" (kiz-cocuk ×5 en kalabalık küme) | Enerjik kazanır; Kaşif ikincil küme |
| Boyut kodu harf kullanımı | mood-taxonomy §3.2.2 "Dışadönüklük (O), Açıklık (A)" | research-bank P7.3: A=Agreeableness, O=Openness | P7.5 "kod esastır" → §3.5.3'te `⚠️ VERIFICATION REQUIRED` |
| Burç / kan grubu / persentil / ayakkabı no / aile geliri | Mevcut | yazılmadı | Bankada YOK → `⚠️ VERIFICATION REQUIRED` (§6.1) |
| Şarkı listesi (5 parça) | Mevcut | yazılmadı | Şarkı bazlı veri kapsam dışı (X-2 dolaylı); sanatçı listesi taşındı |

### §7.3 Değişiklik Geçmişi (append-only)

| Tarih | Versiyon | Değişiklik | Yazar |
|-------|----------|------------|-------|
| 2026-09-26 | 1.0.0 | İlk üretim — persona-template §3.5 (11/11) + research-bank P3-P8 etiketli veri; eski salt-okunur dosyadan kimlik/müzik taşındı, yaş (9→11) ve mood (Kaşif→Enerjik) index §6.3'e göre düzeltildi; bankada olmayan ölçüm/ekonomik veriler çıkarıldı; veli_onayı + KVKK notu eklendi | vault-updater (persona) |
