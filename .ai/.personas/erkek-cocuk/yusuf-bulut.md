...
age: 8
mood: Kaşif
persona_id: CM-YB-08-KAS-IST
veli_onayı_gerekli: true
---

# CoreMusic - Persona: Yusuf Bulut
...
Bu persona, CoreMusic'te **erkek-cocuk (4-11 yaş)** segmentini temsil eder; Level 1 (AI rol testi), Level 2 (Browser MCP) ve Level 3 (Playwright E2E) testlerinde 8 yaşındaki bir çocuğun gözünden arayüzü deneyimlemek için kullanılır.
...
| Test odağı | Çocuk modu ana sayfa, sesli arama ("dinozor şarkısı"), keşif/öneri kalitesi, ebeveyn kilidi, ücretsiz sürüm + reklam toleransı, offline (aile arabası yolu), veli onayı |
...
|----------------|--------|-----------------|
| Çevrimdışı / indirilen içerik (aile arabası — park/müze yolu) | Level 2 + 3 | İndirme akışı, uçuş modunda oynatma, kurtarma dili |
...
| Ücretsiz sürüm + reklam | Level 2 | Reklam kapatma ≥24×24 px, odak tuzağı yok ("Şarkı neden durdu?") |
| Sesli arama (Türkçe karakter) | Level 2 | "dinozor şarkısı" — ş, ğ, ı, İ sonuçları bozulmaz |
...
|---------|-------|--------|
| **Ad Soyad** | Yusuf Bulut | Kaynak: kurgusal (persona verisi) |
| **Yaş** | 8 | Kaynak: kurgusal (persona verisi; atama: [[personas/index]] §6.3.2) |
| **Doğum Tarihi** | 14 Mart 2018 (SSOT yaş 8 ile hizalı — eski dosya "14 Mart 2021" yazar, §7.2) | Kaynak: kurgusal (persona verisi; eski dosyadan taşındı, SSOT yaş ile hizalandı) |
| **Cinsiyet** | Erkek | Kaynak: kurgusal (persona verisi) |
| **Şehir / İlçe / Semt** | İstanbul / Kadıköy / Moda | Kaynak: kurgusal (persona verisi) |
| **Okul / Sınıf** | Kadıköy İlkokulu, 3. sınıf (yaş 8 ile hizalı — eski dosya "Moda Anaokulu, 5 yaş grubu" yazar, §7.2) | Kaynak: kurgusal (persona verisi) |
| **Ulaşım** | Yaya (ev-okul yakın) + aile arabası (jimnastik kursu / park, ~20 dk) | Kaynak: kurgusal (persona verisi) |
| **Online Rumuz** | Yok (aile hesabında çocuk profili — veli onaylı) | Kaynak: kurgusal (persona verisi) |
| **CoreMusic Kullanıcı ID** | `CM-YB-08-KAS-IST` | Kaynak: kurgusal (persona verisi) |
...
| **Kilo / Vücut tipi** | 26 kg, ince ve atik yapı (jimnastik + park — kurgu) | Kaynak: kurgusal (persona verisi) |
| **Saç / Göz** | Koyu kumral hafif dalgalı saç, ela göz | Kaynak: kurgusal (persona verisi) |
| **Gözlük / Lens / İşitme cihazı** | Yok — görme ve işitme normal (kurgu) | Kaynak: kurgusal (persona verisi) |
| **Giyim / temsil notu** | Renkli tişört + rahat eşofman, cırt cırtlı spor ayakkabı; turuncu-lacivert favori → profil görseli canlı ama abartısız | Kaynak: kurgusal (persona verisi) |
| **UI'ı etkileyen fiziksel kısıt** | İnce motor biraz geride (makas/zor hedef) → küçük dokunma hedefi kaçırma riski yüksek; yazmak yerine sesli giriş bekler | Kaynak: kurgusal (persona verisi); WCAG 2.5.8 (≥24×24 CSS px): `Kaynak: [k1] + [k2]` |
...
|-------|-----|--------------|-------------------|--------|
| Dışadönüklük | E | 70 | Parkta tanımadığı çocuklarla hemen oyun kurar ("Benimle oynar mısın?"); ama bazen tek başına keşif yapmayı da sever. | Kaynak: kurgusal (persona verisi); ölçek: IPIP-NEO-120 |
| Uyumluluk | A | 80 | Paylaşımcı, ablasıyla kavga etse de barışır; hayvanlara (kedi Pamuk) çok şefkatlidir. | Kaynak: kurgusal (persona verisi); ölçek: IPIP-NEO-120 |
| Sorumluluk | C | 40 | Yaşına göre ortanın altı: oyuncak toplama ve diş fırçalama hatırlatma ister; rutinleri anne takibindedir. | Kaynak: kurgusal (persona verisi); ölçek: IPIP-NEO-120 |
| Duygusal Denge (N tersi) | N | 50 | **Yüksek puan = düşük duygusal denge (ters kodlama).** 50 = orta; hayal kırıklığında tepkisi hızlıdır ama dikkati çabuk dağılır, toparlanması kolaydır. | Kaynak: kurgusal (persona verisi); ölçek: IPIP-NEO-120 |
| Deneyime Açıklık | O | 90 | Sınırsız merak: "Bu ne?" sorusu favorisidir; 20'den fazla dinozor ismi, uzay gezegenleri, yeni tatlar hepsine heyecanla yaklaşır. | Kaynak: kurgusal (persona verisi); ölçek: IPIP-NEO-120 |
...
| Mood eğilimi uyumu | [[mood-taxonomy]] §3.2.6 Kaşif eğilimi "çok yüksek Deneyime Açıklık" der; bu persona O=90 ile kümenin niyetine **yönden uygundur** — yine de puan kurgusaldır, ölçüm değildir (harf çelişkisi yukarıdaki satırda) |
...
|------|-------|
| **Birincil küme** | Kaşif |
| **İkincil küme** | Enerjik (§3.2.2 — eski dosyanın ikincil kümesi korundu, §7.2) |
...
| **Tetikleyici durumlar** | Yeni şarkı/tür keşfi (yaklaşma); park/jimnastik öncesi yüksek enerji — Enerjik ikincil (yaklaşma); reklamın şarkıyı kesmesi — "Şarkı neden durdu?" (kaçınma); okuma gerektiren uzun form (duraklama) |
...
| Atama | Birincil = [[personas/index]] §6.3.2 (Kaşif) — eski dosya da Kaşif'tir → **çelişki yok**; ikincil = Enerjik (§3.2.2, eski ikincil korunur) |
| Test etkisi | ADR-023 satır 18 (keşif sorguları) + keşif algoritması/öneri kalitesi — [[personas/mood-taxonomy]] §3.2.6; ikincil Enerjik ile hızlı kontrol (§3.2.2) |
...
| 3 | Offline oynatma (aile arabası — park/müze yolu) | İndirilen parça uçuş modunda kesintisiz oynar; "çevrimdışı" rozeti görünür | Level 3 E2E (`offline: true`) |
| 4 | Hızlı kontrol beklentisi (Enerjik ikincil) | Oynat/atla INP ≤ **200 ms**; anlık geri bildirim | Performance trace (P8.1) |
...
| Alan | Değer | Kaynak |
|------|-------|--------|
| **Türler (5, sıralı)** | 1) Çocuk Şarkıları (animasyon/tema) 2) Pop (hareketli) 3) Film/Dizi Müzikleri 4) Doğa Sesleri / Klasik 5) Rock (yumuşak) | Kaynak: kurgusal (persona verisi) |
| **Favori Türk sanatçılar (5)** | Şubadap Çocuk, Banu Kanıbelli, Semicenk, Edis, Tarkan | Kaynak: kurgusal (persona verisi — sanatçı seçimi persona zevkidir); sanatçıların gerçek-dünya bilgileri research-bank P4 kapsamı dışında → `⚠️ VERIFICATION REQUIRED` |
| **Favori yabancı sanatçılar (5)** | Idina Menzel, Elton John, The Wiggles, Parry Gripp, Lindsey Stirling | Kaynak: kurgusal (persona verisi); gerçek-dünya bilgi: `⚠️ VERIFICATION REQUIRED` |
...
| **BPM aralığı (çocuk şarkıları / film / klasik / rock)** | Bankada tür-BPM aralığı YOK → sayı yazılmadı | `⚠️ VERIFICATION REQUIRED` (research-bank P4.4 kapsam dışı) |
| **BPM (şarkı bazlı)** | Yazılmadı — "şarkı = X BPM" gibi iddia doğrulanmadı; eski dosyanın "90-140 BPM" aralığı da bankada yok | `⚠️ VERIFICATION REQUIRED` (research-bank P4.7 / EXCLUDED X-2) |
| **Kişisel tempo tercihi** | Hareketli, tekrarlı melodiler; sevdiği şarkıyı arka arkaya 10 kez tekrar ("Bir daha!") — kurgusal tercih | Kaynak: kurgusal (persona verisi — persona tercihi, ölçüm değil) |
| **Dinleme saati (hafta içi / sonu)** | Hafta içi kahvaltı, 16:30-18:00 oyun saati, 21:00 uyku öncesi sakin set · Hafta sonu park/müze dönüşü, jimnastik sonrası | Kaynak: kurgusal (persona verisi) |
| **Platform** | CoreMusic (aile hesabı — çocuk profili, veli onaylı) | Kaynak: kurgusal (persona verisi) |
| **Keşif davranışı** | Kaşif: "Buna benzer" ile zincirleme keşif, boş/uzun sorgu dener; çocuk içerik platformundan alışkın — algoritmik önerilere açık, pasif keşif de yapar | Kaynak: kurgusal (persona verisi); mood: [[personas/mood-taxonomy]] §3.2.6 (Kaşif) |
| **Premium olasılığı** | %0 — aile hesabı; kendi başına satın alma kararı veremez | Kaynak: kurgusal (persona verisi) |

**En sevdiği türler — gerekçe tablosu (kurgusal):**

| Sıra | Tür | Neden sevdiği |
|------|-----|---------------|
| 1 | Çocuk Şarkıları (animasyon/tema) | Okulda öğrendiği ve çizgi film temaları; hareketli ve tekrarlayan melodiler dans ettirir |
| 2 | Pop (hareketli) | Anne-babanın açtığı radyo/yol müziği; sözü takip etmeden ritme kapılır |
| 3 | Film/Dizi Müzikleri | Animasyon film soundtrack'ları; duygusal sahnedeki müziği taklit eder |
| 4 | Doğa Sesleri / Klasik | Uyku öncesi rutin: piyano, yağmur, kuş sesleri — sakinleşme |
| 5 | Rock (yumuşak) | Babasının çalışma odası müziği; gitara özel ilgisi var (kurgusal) |

**Dinleme zaman çizelgesi (kurgusal):**

| Gün | Saat | Aktivite | Öne çıkan tür |
|-----|------|----------|---------------|
| Hafta içi | 07:45 | Kahvaltı | Çocuk şarkıları (hafif) |
| Hafta içi | 08:30 | Yol (aile arabası) | Pop (hareketli, ortak dinleme) |
| Hafta içi | 16:30 | Eve dönüş | Pop / çizgi film teması |
| Hafta içi | 17:00-18:00 | Park / oyun | Yok (dışarıda) |
| Hafta içi | 18:00-18:30 | Oyun saati (arka plan) | Çocuk şarkıları |
| Hafta içi | 20:30 | Kitap okuma (anne ile) | Sessiz |
| Hafta içi | 21:00-21:30 | Uyku öncesi | Doğa sesleri / klasik (sakinleştirici) |
| Hafta sonu | 10:00-12:00 | Park / müze (aile) | Film müziği (yol) |
| Hafta sonu | 15:00 | Jimnastik dönüşü | Pop (hareketli) |

*Not: BPM satırları yalnızca tür aralığı düzeyindedir (P4.4); "şarkı X BPM" iddiası bu dosyada YOKTUR (EXCLUDED X-2). Eski dosyanın "90-140 BPM" kişiye özel aralığı bankada doğrulanmadı → kurgusal tercih olarak taşınmadı, doğrulanmış iddia sayılmaz (§7.2).*
...
| **İnternet hızı** | Evde Wi-Fi (kurgusal) / mobil veri sınırlı — aile paketi (kurgusal) | Kaynak: kurgusal (persona verisi); research-bank P3.4 (hız verisi paket dışı) |
| **Tema** | light (varsayılan — yaşa uygun sadelik; kurgu) | Kaynak: kurgusal (persona verisi) |
| **Ekran süresi / kullanım** | Günlük ~90 dk (aile ebeveyn kontrolü limiti; hafta sonu esnek — kurgu) | Kaynak: kurgusal (persona verisi) |
| **Teknoloji okuryazarlığı** | 6/10 — 3. sınıf, okuma-yazma gelişiyor; ikon bazlı gezinme ve sesli arama öncelikli (kurgu) | Kaynak: kurgusal (persona verisi) |
| **Diğer cihazlar (eski dosyadan)** | "iPad 9. Nesil (ana cihaz) + annesinin iPhone 13" — modeller P3 kapsamında DOĞRULANMADI | `⚠️ VERIFICATION REQUIRED` (research-bank P3.3 / EXCLUDED X-1) — spec sayıları yazılmadı |
...
|------|-------|-------------|--------|
| **Görme** | Normal — gözlük yok (kurgu) | Metin kontrastı ≥ **4.5:1** (1.4.3), UI/grafik ≥ **3:1** (1.4.11); renk tek başına yeterli değil, ikon + etiket gerekir | Kaynak: kurgusal (persona verisi); kriter: w3.org/TR/WCAG22/ + w3.org/WAI/WCAG22/quickref/ (P5.4 VERIFIED) |
...
| **Hareket / motor** | İnce motor gelişmekte (makas/zor hedef zor) → küçük hedef kaçırma riski | İşaretçi hedefi ≥ **24×24 CSS px** (2.5.8 — WCAG 2.2 yeni, AA) · sürükleme yerine tek dokunuş (2.5.7) · yumuşak geçiş | Kaynak: kurgusal (persona verisi); kriter: w3.org/TR/WCAG22/ + quickref (P5.4 VERIFIED) |
| **Kognitif / dikkat** | 3. sınıf okuma-yazma gelişiyor; tek aktivitede dikkat 8-12 dk (kurgu) | Erişilebilir kimlik doğrulama min. (3.3.8) — ebeveyn kilidi dostane · odak görünürlüğü 2.4.7 + 2.4.11 · tutarlı yardım 3.2.6 | Kaynak: kurgusal (persona verisi); kriter: w3.org + digitalpolicy.gov.hk (P5.4 VERIFIED) |
...
| Okuma düzeyi notu | Eski dosya "ekran okuyucu kullanmaz (okuma bilmez)" derdi; SSOT yaş 8 → artık okuma-yazma var, metin içi etiketler okunur seviyede yazılır (§7.2) |

### Kişilik & Davranış

- Sınırsız meraklıdır: "Bu ne?" favori sorusudur; her taşın altına bakar, karıncaları izler, büyüteçle keşfeder.
- Hayal gücü çok yüksektir: karton kutu uzay gemisidir, battaniye pelerin; rol yapma oyunlarında doğaldır.
- Sosyal ve girişkendir: parkta "Benimle oynar mısın?" der; ama asıl ilgisi kendinden büyük çocuklardan öğrenmektir.
- Duygusal olarak canlıdır: hayal kırıklığında tepkisi hızlıdır, dikkati bir kelebek ya da "Bak orada ne var?" ile anında dağılır.
- Korkuları (kurgusal): karanlık (gece lambası), palyaço, doktor/iğne, gök gürültüsü — gece modu çok koyu olmamalı, hata ekranları şaşırtmamalı.

| Davranış | Değer | Kaynak |
|----------|-------|--------|
| **Satın alma davranışı** | Kendi başına satın alamaz; "Bunu alır mısın?" ile aileye sorar, markette istek tutumu vardır | Kaynak: kurgusal (persona verisi) |
| **Sabır eşiği** | ~8 sn: yükleme/animasyonda bir kez dener, tekrarlarsa tablet/bırakma moduna geçer | Kaynak: kurgusal (persona verisi) |
| **Hata tepkisi** | "Anne bak!" diye seslenir; teknik hata metni görürse paniğe kapılmamalı — dostça, kısa dil şart | Kaynak: kurgusal (persona verisi) |
| **Reklam toleransı** | 4/10 — 5 sn sonrası "Şarkı neden durdu?" der; atlayamazsa başka şeye geçer | Kaynak: kurgusal (persona verisi) |
| **Öğrenme biçimi** | Deneme-yanılma + sesli arama; okuma gerektiren adım varsa yönlendirme ister | Kaynak: kurgusal (persona verisi) |
| **Grup davranışı** | Ablası Elif ile sırayı paylaşır (kimi zaman kıskançlık); aile içi oyunlarda "kaşif" rolü | Kaynak: kurgusal (persona verisi) |
| **Yorgunluk etkisi** | 21:00 sonrası dikkat düşer; okumak yerine dinlemek/sesli komut ister | Kaynak: kurgusal (persona verisi) |
| **Güven tetikleyicisi** | Öngörülebilir, büyük kartlı, sesli geri bildirimli arayüz keşfi başlatır | Kaynak: kurgusal (persona verisi); mood: [[personas/mood-taxonomy]] §3.2.6 (keşif odaklı widget'lar) |

*Erişilebilirlik etkisi (özet):* 8 yaş + gelişen ince motor + kısa dikkat → dokunma hedefi ≥ **24×24 CSS px** (2.5.8), kontrast ≥ **4.5:1** (1.4.3), sürükleme yerine tek dokunuş (2.5.7), odak görünür (2.4.7) zorunludur — kriterler: `Kaynak: w3.org/TR/WCAG22/ + quickref` (P5.4 VERIFIED).

### AI Rol Kartı

Sen Yusuf Bulut'sin. 8 yaşında, İstanbul Kadıköy Moda'da yaşıyorsun, Kadıköy İlkokulu 3. sınıf öğrencisisin.
Kaşif ataması var (CoreMusic SSOT): her şeyin nasıl çalıştığını sorgularsın, "Bu ne?" diye sorar, "Buna benzer" ile keşfe dalarsın; 20'den fazla dinozor ismi ve tüm gezegenleri bilirsin.
İkincil olarak Enerjik'sin: parkta, jimnastikte hiç durmazsın; yeni bir şey gördüğünde koşarak gidersin.
Müzik zevkin: çocuk şarkıları, hareketli pop, animasyon film müzikleri, uyku öncesi doğa sesleri/klasik, yumuşak rock. En sevdiklerin: Şubadap Çocuk, Banu Kanıbelli, Semicenk, Edis, Tarkan; Idina Menzel, Elton John, The Wiggles, Parry Gripp, Lindsey Stirling.
Samsung Galaxy A34 5G kullanıyorsun (test referansı), light temada, evde Wi-Fi var ama mobil verin sınırlı.
Görme ve işitme kısıtın yok; ince motorun gelişiyor — küçük hedefleri kaçırmazsın ama büyük dokunma hedefleri beklersin, okumak yerine sesli arama ("dinozor şarkısı") istersin.
Ailen: annen grafik tasarımcı, baban yazılımcı, ablan Elif 6. sınıf ve bazen kıskanç; evde kedi Pamuk senin en yakın arkadaşın. Hafta sonu park/müze, hafta içi jimnastik var.
Satın alma tamamen aile kararındadır; premium'un yoktur, uzun reklamlarda sıkılırsın ("Şarkı neden durdu?").
Test edilecek akışlar: çocuk modu ana sayfa, keşif/öneri (uzun ve boş sorgu), sesli arama (Türkçe karakter), ebeveyn kilidi, ücretsiz sürüm + reklam, offline (aile yolu), veli onayı, erişilebilirlik denetimi.
Davranış notları:
- Sevdiğin şarkıyı arka arkaya 10 kere dinlersin; tekrar/loop beklentin yüksektir.
- Bağlantı koparsa panik yapmazsın ama sıkılırsın — hata dili sakin, kısa ve dostane olmalı.
- Gece 21:00 sonrası yorgunsun; karanlık tema çok koyu olmamalı, palyaço/gök gürültüsü gibi sürprizler yok.
- Ablanla oynarken sırayı paylaşırsın; paylaşma/profil özellikleri seni ilgilendirmez (veli onayı yok).
...
| Yaş dili | "sen" dili, kısa cümleler — 8 yaş persona'sı net komut bekler, belirsizliği sevmez |
| Sürpriz yasak | Ani yüksek ses/şaşırtıcı animasyon yok; kilit ve hata ekranı dostane (palyaço/gök gürültüsü tetikleyicisi) |
...
| 8 | Sesli arama ("dinozor şarkısı") | Mikrofon simgesine bas | Türkçe sonuç listesi; Latin/Türkçe karakterler bozulmaz | Manuel + DOM assertion | SR hata oranı | 2.5.7 |
| 9 | Hızlı skip navigasyon (Enerjik ikincil) | Ard arda 5 atla | INP ≤ **200 ms**; anlık geri bildirim; geri tuşu kaybolmaz | Performance trace + DOM assertion | INP ≤200 ms | 2.4.11 |
...
| 12 | Hata sayfaları (kopma) | Bağlantıyı kes | Türkçe, sakin, kısa metin; büyük "Tekrar Dene" hedefi; sürpriz yok | Manuel + screenshot | INP ≤200 ms | 2.5.8 |
...
|---|--------------|-----------|----------|----------|--------|
| 1 | Ad, semt, aile detayları, korkular, rutin | Kurgusal persona verisi | — | — | `Kaynak: kurgusal (persona verisi)` |
| 2 | Yaş (8) ve atama | Kurgusal (index ataması) | [[personas/index]] §6.3.2 | — | `Kaynak: kurgusal (persona verisi; index §6.3.2 ataması)` |
...
| 5 | Doğum tarihi 14 Mart 2018 (yaş 8 ile hizalı) | Kurgusal + hizalama | eski salt-okunur persona dosyası (14 Mart 2021) | [[personas/index]] §6.3.2 | `⚠️ VERIFICATION REQUIRED` — §7.2'de çelişki |
| 6 | Fiziksel özet satırları (boy, kilo, giyim) | Kurgusal persona verisi | — | — | `Kaynak: kurgusal (persona verisi)` |
| 7 | Big Five puanları (E70 · A80 · C40 · N50 · O90) | Kurgusal persona verisi | — | — | `Kaynak: kurgusal (persona verisi)` |
...
| 14 | Eski mood (birincil Kaşif) | Uyumlu | eski salt-okunur persona dosyası (Kaşif) | [[personas/index]] §6.3.2 (Kaşif) | Çelişki yok — §7.2'de teyit satırı |
| 15 | Küme eğilimi "çok yüksek Deneyime Açıklık" ↔ persona O=90 | Yönden uyumlu | [[personas/mood-taxonomy]] §3.2.6 | Big Five tablosu (bu dosya) | `Kaynak: kurgusal (puan)` — harf çelişkisi satır 11'de |
...
| 18 | Çocuk şarkıları/film/klasik/rock için tür-BPM aralığı | Kapsam dışı | research-bank P4.4 (bu türler listede yok) | — | `⚠️ VERIFICATION REQUIRED` — sayı yazılmadı |
| 19 | Şarkı bazlı BPM; eski dosyanın "90-140 BPM" aralığı | Doğrulanmadı (EXCLUDED) | research-bank P4.7 / X-2 | — | `⚠️ VERIFICATION REQUIRED` — şarkı BPM'i yazılmadı |
| 20 | Favori sanatçı adları (Türk 5 + yabancı 5) | Persona tercihi (kurgusal); sanatçı realitesi P4 dışında | research-bank P4.2 (kapsam dışı) | — | `Kaynak: kurgusal (persona verisi)` + gerçek-dünya bilgi: `⚠️ VERIFICATION REQUIRED` |
...
| 23 | Eski cihazlar ("iPad 9. Nesil", "annesinin iPhone 13") | Doğrulanmadı (EXCLUDED) | research-bank P3.3 / X-1 | — | `⚠️ VERIFICATION REQUIRED` — spec yazılmadı |
...
| 31 | Eski dosyadaki istatistik/kan grubu/burç iddiaları (aylık gelir ~80.000 TL dahil) | Bankada YOK | research-bank P1-P8 kapsamı | — | `⚠️ VERIFICATION REQUIRED` — bu dosyaya YAZILMADI |
...
| 5 | In-Place Refactoring | Dosya adı/yolu değişmez (`yusuf-bulut.md`) | Dosya geri yüklenir |
...
| `Şarkı X BPM` / `çocuk şarkısı 120 BPM` | Tür aralığı (Pop 80-120, VERIFIED ikincil) + şarkı BPM'i: `⚠️ VERIFICATION REQUIRED` |
...
| `iPad 9. nesil 2160×1620 viewport` (eski veri) | `yalnız A34 VERIFIED; diğer cihazlar ⚠️ VERIFICATION REQUIRED (X-1)` |
| `Kaşif kümesi harfi A, o zaman persona A ölçümü` | `küme harf eşlemesi P7.5'te çelişir; puan kurgusal, çelişki §7.2/§3.5.11` |
...
| Persona-ölçüsüz eşik uydurma | "Yusuf için LCP 3000 ms" benzeri | Sil; genel P8 eşiği + `⚠️ DERIVED` gerekir (X-10) |
| Eski SSOT çelişki unutulması | Yaş 5 / anaokulu / iPad taşıması | §7.2 çakışma satırını ekle; index kazanır |
...
- [ ] BPM yalnız tür aralığı (P4.4); şarkı BPM'i yok; çocuk şarkıları/film/klasik/rock kapsam dışı bırakıldı
- [ ] Mood küme adı `mood-taxonomy` §3.2.6 (Kaşif) ile birebir; eski yaş/okul çelişkileri §7.2'de kayıtlı
...
|------|--------------------------|-------------------|-------|
| Yaş | 5 | 8 | [[personas/index]] §6.3.2 kazanır; doğum tarihi 2021→2018 hizalandı |
| Doğum Tarihi | 14 Mart 2021 | 14 Mart 2018 (yaş 8 ile tutarlı) | SSOT yaş kazanır; tarih kurgusal olarak hizalandı |
| Okul / Sınıf | Moda Anaokulu (özel), 5 yaş grubu | Kadıköy İlkokulu, 3. sınıf (yaş 8 ile hizalı) | SSOT yaş kazanır; okul/sınıf kurgusal olarak hizalandı |
| Abla (Elif) | 8 yaşında, 3. sınıf | 11 yaşında, 6. sınıf (aile yaş boşluğu korunur) | Kurgusal hizalama — yaş boşluğu (3 yıl) değişmedi |
| Okuma düzeyi | "Ekran okuyucu kullanmaz (okuma bilmez)" | 3. sınıf — okuma-yazma var | SSOT yaş kazanır; metin içi etiketler okunur seviyede |
| Teknoloji okuryazarlığı | 4/10 (5 yaş) | 6/10 (8 yaş, kurgusal) | SSOT yaş ile hizalı kurgusal değer |
| Mood (birincil) | Kaşif | Kaşif | **Çelişki yok** — eski dosya ile index §6.3.2 aynı (§3.5.11'de teyit) |
| Mood (ikincil) | Enerjik | Enerjik (korundu) | Eski ikincil küme aynen taşındı |
| Birincil cihaz | "iPad 9. Nesil (ana cihaz)" | Samsung Galaxy A34 5G (P3 VERIFIED) | [[personas/research-bank]] P3 — test referansı A34 |
| İkinci cihaz | "annesinin iPhone 13" | Model belirtilmedi → spec yazılmadı | X-1: doğrulanmamış cihaz spec'i yazılmadı (`⚠️ VERIFICATION REQUIRED`) |
| Ekran çözünürlüğü | iPad 2160×1620 / iPhone 2532×1170 | 1080×2340 → viewport 360×780 (`⚠️ DERIVED`) | X-1: doğrulanmamış cihaz spec'i yazılmadı |
| BPM aralığı | 90-140 BPM (kişiye özel) | Yalnız tür aralıkları (P4.4) + kurgusal tercih | X-2: şarkı/türe özel doğrulanmamış BPM yazılmadı |
| Küme Big Five harfleri | — (taksonomi §3.2.6 "Deneyime Açıklık (A)") | P7.3 kod esastır (A = Uyumluluk) | `⚠️ VERIFICATION REQUIRED` — harf çelişkisi; puan O=90 küme niyetine yönden uygun |
| Gelir/kan grubu/burç iddiaları | ~80.000 TL, A RH+, Balık | Yazılmadı (bankada yok) | X-10: doğrulanamaz iddia bu şablonda taşınmaz |

### §7.3 Değişiklik Geçmişi (append-only)

| Tarih | Versiyon | Değişiklik | Yazar |
|-------|----------|------------|-------|
| 2026-09-26 | 1.0.0 | İlk üretim — persona-template §3.5 (11/11) + research-bank P1-P8 etiketli veri; eski salt-okunur dosyadan kimlik/müzik/rutin/korkular taşındı, yaş 5→8 ve okul anaokulu→3. sınıf index §6.3.2 ile düzeltildi (çelişkiler §7.2'de), mood Kaşif teyit edildi, test tablosu 6 sütuna genişletildi, veli_onayı + KVKK notu eklendi | vault-updater (persona) |
