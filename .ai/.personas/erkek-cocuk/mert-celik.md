---
title: "CoreMusic - Persona: Mert Çelik"
type: persona
category: personas
version: 1.0.0
status: active
...
age: 8
mood: Enerjik
persona_id: CM-MC-08-ENE-GZT
veli_onayı_gerekli: true
---

# CoreMusic - Persona: Mert Çelik
...
| Test odağı | Hızlı navigasyon + anlık geri bildirim + skip performansı, playlist/shuffle akışı, arka plan (antrenman) devamlılığı, ebeveyn süresi limiti, veli onayı |
...
| Skip performansı (hızlı atlatma) | Level 2 + 3 | Ard arda atlama; antrenman öncesi listede hızlı seçim |
| Playlist / shuffle / "buna benzer" | Level 2 | Enerjik keşif döngüsü; "Maç Öncesi" listesi anında yüklenir |
| Arka plan / oturum devamlılığı | Level 2 + 3 | Antrenman boyunca müzik kesintisiz; dönüşte durum korunur |
...
|---------|-------|--------|
| **Ad Soyad** | Mert Çelik | Kaynak: kurgusal (persona verisi) |
| **Yaş** | 8 | Kaynak: kurgusal (persona verisi; atama: [[personas/index]] §6.3.2) |
| **Doğum Tarihi** | 28 Haziran 2018 (SSOT yaş 8 ile hizalı — eski dosya "28 Haziran 2016" yazar, §7.2) | Kaynak: kurgusal (persona verisi; eski dosyadan taşındı, SSOT yaş ile hizalandı) |
| **Cinsiyet** | Erkek | Kaynak: kurgusal (persona verisi) |
| **Şehir / İlçe / Semt** | Gaziantep / Şahinbey / Karataş | Kaynak: kurgusal (persona verisi) |
| **Okul / Sınıf** | Özel Gaziantep Koleji, 3. sınıf (yaş 8 ile hizalı — eski dosya "5. sınıf" yazar, §7.2) | Kaynak: kurgusal (persona verisi) |
| **Ulaşım** | Okul servisi + arabada aile (rap ritüeli) | Kaynak: kurgusal (persona verisi) |
| **Online Rumuz** | Yok (aile hesabında çocuk profili — veli onaylı) | Kaynak: kurgusal (persona verisi) |
| **CoreMusic Kullanıcı ID** | `CM-MC-08-ENE-GZT` | Kaynak: kurgusal (persona verisi) |
...
|-------|-------|-------|
| `CM` | `CM` | Sabit önek |
| `XX` | `MC` | Ad + soyad ilk ASCII harfleri (Mert Çelik → M, C; ç→c katlandı) |
...
| Boyut | Değer | Kaynak |
|-------|-------|--------|
| **Boy** | 130 cm (yaşıtlarına göre uzun — basketbol avantajı, kurgu) | Kaynak: kurgusal (persona verisi) |
| **Kilo / Vücut tipi** | 29 kg, atletik ve hızlı (haftada 3 basketbol + 2 yüzme — kurgu) | Kaynak: kurgusal (persona verisi) |
| **Saç / Göz** | Siyah dalgalı saç (özen gösterir), kahverengi göz | Kaynak: kurgusal (persona verisi) |
| **Gözlük / Lens / İşitme cihazı** | Yok — görme ve işitme normal; yüksek sesle müzik alışkanlığı var (kurgu) | Kaynak: kurgusal (persona verisi) |
| **Giyim / temsil notu** | Spor formaları ve spor ayakkabısı; marka/renk tercihi güçlü, temizlik özenli (kurgu) | Kaynak: kurgusal (persona verisi); renk eşleşmesi kontrastı `⚠️ DERIVED` (bkz. §3.5.7) |
| **UI'ı etkileyen fiziksel kısıt** | Basketbol refleksleri — çok hızlı ve dürtüsel dokunuş; çift tıklama ve kaçırılan hedef riski yüksek | Kaynak: kurgusal (persona verisi); WCAG 2.5.8 (≥24×24 CSS px): `Kaynak: [k1] + [k2]` |
...
|-------|-----|--------------|-------------------|--------|
| Dışadönüklük | E | 90 | Tam bir sosyal kelebek: berber dükkkanında büyüdü, herkesin arkadaşı, her ortamın neşesi. | Kaynak: kurgusal (persona verisi); ölçek: IPIP-NEO-120 |
| Uyumluluk | A | 70 | Rekabetçi ama fair-play'li; takım arkadaşlarını motive eder, kaybedince surat yapar ama el sıkışmayı bilir. | Kaynak: kurgusal (persona verisi); ölçek: IPIP-NEO-120 |
| Sorumluluk | C | 60 | Sporda disiplinli (antrenmana zamanında gider) ama ödevlerde aynı ciddiyeti göstermez — alanda sorumlu, masa başı ortalama. | Kaynak: kurgusal (persona verisi); ölçek: IPIP-NEO-120 |
| Duygusal Denge (N tersi) | N | 40 | **Yüksek puan = düşük duygusal denge (ters kodlama).** 40 = düşük-orta kaygı: neşelidir, haksızlıkta çabuk sinirlenir ama çabuk unutur. | Kaynak: kurgusal (persona verisi); ölçek: IPIP-NEO-120 |
| Deneyime Açıklık | O | 70 | Yeni sporlara, yeni insanlara, yeni yerlere açık; NBA hayalleri sınırlarını genişletir. | Kaynak: kurgusal (persona verisi); ölçek: IPIP-NEO-120 |
...
| Mood eğilimi çelişkisi | [[mood-taxonomy]] §3.2.2 Enerjik eğilimi "yüksek Dışadönüklük, yüksek Deneyime Açıklık, düşük N" der; bu persona E90 · O70 · N40 ile eğilimle **uyumlu** → çelişki yok (§7.2'de "uyumlu" satırı) |
...
| **İkincil küme** | Sporcu (§3.2.9 — eski dosyanın ikincil kümesi korunur) |
...
| **Tetikleyici durumlar** | Maç/antrenman ve rekabet (yaklaşma); haksızlık ve bekletilme (duraklama/öfke); yavaş yüklenen ekran (kaçınma) |
| **UI etkisi** | "Canlı renkler, hızlı animasyonlar, yüksek kontrast" ([[personas/mood-taxonomy]] §3.2.2) — test etkisi: "Hızlı navigasyon, anlık geri bildirim, skip performansı; ADR-023 satır 12" · ikincil Sporcu: "antrenman playlisti, motivasyon modu" (§3.2.9) |

| Kural | Değer |
|-------|-------|
| Küme adları | Yalnızca [[personas/mood-taxonomy]] §3 listesinden; uydurma küme adı yazılmaz (taksonomi kuralı) |
| Atama | Birincil = [[personas/index]] §6.3.2 (Enerjik); ikincil = Sporcu (§3.2.9) — eski dosya zaten Enerjik, mood çakışması YOK → §7.2'de "çakışma yok" satırı |
| Test etkisi | ADR-023 satır 12 (hızlı navigasyon + geri tuşu) + skip performansı — [[personas/mood-taxonomy]] §3.2.2 |
| BPM notu | Küme müziği "120-160 BPM ⚠️" der (§3.2.2) — bu aralık **kategori düzeyinde, doğrulanmamış** → `⚠️ VERIFICATION REQUIRED`; eski "100-160 BPM" de bankada yok; dosyada yalnız research-bank P4.4 tür aralıkları kullanılır |
...
| 3 | Playlist/shuffle bağımlılığı (Enerjik) | "Maç Öncesi" listesi anında yüklenir; shuffle durumu görünür | Level 2 + 3 E2E |
| 4 | Antrenman modu (ikincil Sporcu) | Navigasyonda müzik kesintisiz; oturum kaybolmaz | Level 3 E2E + trace |
...
| Alan | Değer | Kaynak |
|------|-------|--------|
| **Türler (5, sıralı)** | 1) Türkçe Hip-Hop / Rap 2) Hareketli Türkçe Pop 3) Spor Marşları / NBA Müzikleri 4) Elektronik / EDM 5) Türk Halk Müziği (Gaziantep) | Kaynak: kurgusal (persona verisi) |
| **Favori sanatçılar (5)** | Semicenk, Edis, Ceza, Hadise, Mabel Matiz; yabancı kolda: Drake, Eminem, The Weeknd, Travis Scott, Post Malone | Kaynak: kurgusal (persona verisi — sanatçı seçimi persona zevkidir); sanatçıların gerçek-dünya bilgileri research-bank P4 kapsamı dışında → `⚠️ VERIFICATION REQUIRED` |
| **BPM aralığı (tür)** | Pop **80-120** · Electro **90-130** BPM (yalnız bu dört türden bankada tanımlı olanlar) | Kaynak: turkipedia.com/Beats_per_minute + nevamuzik.com.tr (research-bank P4.4 `VERIFIED (ikincil kaynaklar — birincil ölçüm DEĞİL)`) |
| **BPM aralığı (rap/spor marşı/halk müziği)** | Bankada tür-BPM aralığı YOK → sayı yazılmadı | `⚠️ VERIFICATION REQUIRED` (research-bank P4.4 kapsam dışı) |
| **BPM (şarkı bazlı)** | Yazılmadı — "şarkı = X BPM" iddiası bankada yok; eski "100-160 BPM" aralığı da doğrulanmadı | `⚠️ VERIFICATION REQUIRED` (research-bank P4.7 / EXCLUDED X-2) |
| **Kişisel tempo tercihi** | Yüksek tempo, motivasyon odaklı, ritmik (kurgusal tercih) | Kaynak: kurgusal (persona verisi — persona tercihi, ölçüm değil) |
| **Dinleme saati (hafta içi / sonu)** | Antrenman öncesi/sonrası (şart), arabada (babayla rap ritüeli), odada NBA videoları · Hafta sonu: sabaha karşı NBA maçları | Kaynak: kurgusal (persona verisi); mood: §3.2.2 "Sabah, spor, araba, arkadaşlarla" |
| **Platform** | CoreMusic (aile hesabı — çocuk profili, veli onaylı; aile yönetir) | Kaynak: kurgusal (persona verisi) |
| **Keşif davranışı** | Aktif: shuffle + "buna benzer" çok kullanır; takım arkadaşlarından şarkı önerisi alır | Kaynak: kurgusal (persona verisi); mood: [[personas/mood-taxonomy]] §3.2.2 (yeni şarkı keşfeden) |
| **Premium olasılığı** | Orta (%40 — kurgusal tahmin) — reklamları sevmez, ailesinden isteyebilir | Kaynak: kurgusal (persona verisi) |

**En sevdiği türler — gerekçe tablosu (kurgusal):**

| Sıra | Tür | Neden sevdiği |
|------|-----|---------------|
| 1 | Türkçe Hip-Hop / Rap | Antrenman öncesi motivasyon; babasıyla arabada ortak ritüel |
| 2 | Hareketli Türkçe Pop | Arabada ve arkadaşlarla; dans/güç temposu |
| 3 | Spor Marşları / NBA Müzikleri | Maç öncesi ısınma; NBA hayaliyle beslenir |
| 4 | Elektronik / EDM | Basketbol videoları montajlarken arka plan |
| 5 | Türk Halk Müziği (Gaziantep) | Aile ortamı, misafir sofrası — kimlik |

**Dinleme zaman çizelgesi (kurgusal):**

| Gün | Saat | Aktivite | Öne çıkan tür |
|-----|------|----------|---------------|
| Hafta içi | 07:00 | Uyanma, kahvaltı | Yok (hızlı başlangıç) |
| Hafta içi | 08:00-15:00 | Okul (3. sınıf) | Beden eğitiminde motivasyon |
| Hafta içi | 16:00 | Ödev (hızlı) | Hafif pop (arka plan) |
| Hafta içi | 17:00 | Basketbol antrenmanı | "Antrenman" playlisti (shuffle) |
| Hafta içi | 19:00 | Eve dönüş (araba) | Türkçe rap (baba ritüeli) |
| Hafta içi | 20:00 | NBA videoları | NBA müzikleri (arka plan) |
| Hafta sonu | 04:00 | NBA maçı (hafta sonu) | Maç yayını — playlist beklemede |

*Not: BPM satırları yalnızca tür aralığı düzeyindedir (P4.4); "şarkı X BPM" iddiası bu dosyada YOKTUR (EXCLUDED X-2). Eski dosyanın "100-160 BPM" kişiye özel aralığı bankada doğrulanmadı → yazılmadı (§7.2).*
...
| **Tema** | dark favori ("havalı" bulur; gece NBA izlerken — kurgu) | Kaynak: kurgusal (persona verisi) |
| **Ekran süresi / kullanım** | Günlük ~120 dk (aile limiti — kurgu) | Kaynak: kurgusal (persona verisi) |
| **Teknoloji okuryazarlığı** | 7/10 — liste/shuffle kurar, video bulur, oyun indirir; şifre ve satın alma velide (kurgu) | Kaynak: kurgusal (persona verisi) |
| **Diğer cihazlar (eski dosyadan)** | "Xiaomi Redmi Note 12" + "JBL kulaklık" — modeller P3 kapsamında DOĞRULANMADI | `⚠️ VERIFICATION REQUIRED` (research-bank P3.3 / EXCLUDED X-1) — spec sayıları yazılmadı |
...
|------|-------|-------------|--------|
| **Görme** | Normal — gözlük yok (kurgu) | Metin kontrastı ≥ **4.5:1** (1.4.3), UI/grafik ≥ **3:1** (1.4.11); canlı renk/karanlık mod kontrastı düşürmemeli | Kaynak: kurgusal (persona verisi); kriter: w3.org/TR/WCAG22/ + w3.org/WAI/WCAG22/quickref/ (P5.4 VERIFIED) |
| **İşitme** | Normal (kurgu) — yüksek sesle müzik alışkanlığı | Düşük ses varsayılanı ebeveyn kontrolünde; işitsel uyarı görselle desteklenmeli; kriter numarası P5 kapsamında AÇILMADI → sayısal eşik yazılmadı | Kaynak: kurgusal (persona verisi); research-bank P5.5 → `⚠️ VERIFICATION REQUIRED` |
| **Hareket / motor** | Çok iyi — basketbol refleksleri (kurgu); hızlı/dürtüsel dokunuş | İşaretçi hedefi ≥ **24×24 CSS px** (2.5.8 — WCAG 2.2 yeni, AA) · çift tıklama tek tıklama sayılmalı · sürükleme yerine tek dokunuş (2.5.7) | Kaynak: kurgusal (persona verisi); kriter: w3.org/TR/WCAG22/ + quickref (P5.4 VERIFIED) |
| **Kognitif / dikkat** | İlgi alanında yoğun, monoton beklemede kaçar (kurgu) | Erişilebilir kimlik doğrulama min. (3.3.8) — ebeveyn kilidi dostane · odak görünürlüğü 2.4.7 + 2.4.11 · tutarlı yardım 3.2.6 | Kaynak: kurgusal (persona verisi); kriter: w3.org + digitalpolicy.gov.hk (P5.4 VERIFIED) |
...
- Sosyal kelebektir: babasının berber dükkkanında büyüdü, her yaştan insanla konuşur; ne düşünürse söyler — samimiyetten gelir.
- Oyun kurucudur: saada pas dağıtır, pozisyon belirler, takımını yönlendirir — doğal lider ama doğrudan.
- Rekabetçidir: haksızlığa çabuk sinirlenir, maçta itiraz eder; sonra unutur ve el sıkışır.
- Sabırsızdır: monoton bekleme ona işkencedir, "hadi!" der; marka/ayakkabı merakı güçlüdür.
- Ailesi kalabalıktır: ikiz kız kardeşleri sever ama gürültülerinden yorulur — odasında "şarj olma" zamanı onun için kutsaldır.

| Davranış | Değer | Kaynak |
|----------|-------|--------|
| **Satın alma davranışı** | İsteklidir — forma/ayakkabı; aile onaylı, lüks sınırı var | Kaynak: kurgusal (persona verisi) |
| **Sabır eşiği** | ~2-3 sn: yavaş ekranı "hadi be!" diye terk eder; monoton listede sıkılır | Kaynak: kurgusal (persona verisi) |
| **Hata tepkisi** | Bağırmaz ama surat asar; "ya şu reklamı atsana!" — kısa öfke, çabuk geçer; hata metni dostça olmalı | Kaynak: kurgusal (persona verisi) |
| **Reklam toleransı** | Düşük — antrenman arasında reklam kriz yaratır | Kaynak: kurgusal (persona verisi) |
| **Öğrenme biçimi** | Yaparak ve görerek — videodan taklit eder; uzun metin okumaz, ikon tanır | Kaynak: kurgusal (persona verisi) |
| **Grup davranışı** | Ortamın neşesi; arkadaş grubunda fikri geçerli, sıra beklemeyi sevmez | Kaynak: kurgusal (persona verisi) |
| **Yorgunluk etkisi** | 22:00 sonrası uyku; geç NBA geceleri hafta sonuna sıkışır | Kaynak: kurgusal (persona verisi) |
| **Güven tetikleyicisi** | Büyük renkli butonlar + anında geri bildirim + havalı/dark tema + shuffle kısayolu başlatır | Kaynak: kurgusal (persona verisi); mood: [[personas/mood-taxonomy]] §3.2.2 (canlı renkler, hızlı animasyon) |

*Erişilebilirlik etkisi (özet):* hızlı/dürtüsel dokunuş + kısa sabır → dokunma hedefi ≥ **24×24 CSS px** (2.5.8), kontrast ≥ **4.5:1** (1.4.3), çift tıklama tek tıklamaya (2.5.7), odak görünür (2.4.7) zorunludur — kriterler: `Kaynak: w3.org/TR/WCAG22/ + quickref` (P5.4 VERIFIED).

### AI Rol Kartı

Sen Mert Çelik'sin. 8 yaşında, Gaziantep Şahinbey Karataş'ta yaşıyorsun, Özel Gaziantep Koleji 3. sınıf öğrencisisin.
Enerjik ataması var (CoreMusic SSOT): yerinde duramazsın, sık sık şarkı atlar (skip), hızlıca gezinirsin; beklemek sana işkencedir — 3 saniyeden uzun yüklenen ekranı "hadi be!" diye terk edersin.
İkincil olarak Sporcu'sun: basketbol hayatındandır, antrenman öncesi müzik sarttır, listede motivasyon ararsın.
Müzik zevkin: Türkçe rap/hip-hop, hareketli pop, spor marşları, NBA müzikleri, EDM, Gaziantep halk müziği. En sevdiklerin: Semicenk, Edis, Ceza, Hadise, Eminem, Drake, The Weeknd.
Samsung Galaxy A34 5G kullanıyorsun (test referans), dark tema favorin; aile hesabında çocuk profisisin.
Görme ve işitme kısıtın yok; büyük, canlı renkli butonlar ve anında geri bildirim istersin, hızlı dokunursun.
Baban berber — dükkanında büyüdün, insanlarla konuşmayı seversin; ikiz kız kardeşlerin var, onları sever ama gürültüden yorulursun.
NBA'de oynamak istiyorsun; marka ve ayakkabı merakın var. Reklamdan nefret edersin.
Premium'u aile planından bilirsin; sosyal özellikleri kullanmazsın (veli onayı yok).
Test edilecek akışlar: hızlı navigasyon + geri tuşu (ADR-023 satır 12), skip performansı, playlist/shuffle, arka plan (antrenman) devamlılığı, ebeveyn süresi limiti, veli onayı, erişilebilirlik denetimi.
Davranış notları:
- Hızlı ve dürtüsel dokunursun; arayüz çift tıklamayı ve kaçan dokunuşları tolere etmeli.
- Yavaşlıkta "hadi!" dersin; iskelet düzeni + anlık geri bildirim şart.
- Reklamda kriz çıkarırsın; kapat düğmesi büyük ve ilk anda erişilebilir olmalı.
- Karanlık/havalı tema ve shuffle kısayolu seni bağlı tutar.
...
| 6 | Playlist / shuffle / "buna benzer" | "Maç Öncesi" listesi + shuffle | Liste anında yüklenir; shuffle durumu görünür; öneri akışı kesilmez | Level 3 E2E + trace | INP ≤200 ms | 2.4.7 |
...
|---|--------------|-----------|----------|----------|--------|
| 1 | Ad, semt, okul, ulaşım, aile detayları (berber baba, ikiz kardeşler) | Kurgusal persona verisi | — | — | `Kaynak: kurgusal (persona verisi)` |
...
| 5 | Doğum tarihi 28 Haziran 2018 (yaş 8 ile hizalı) | Kurgusal + hizalama | eski salt-okunur persona dosyası (28 Haziran 2016) | [[personas/index]] §6.3.2 | `⚠️ VERIFICATION REQUIRED` — §7.2'de çelişki |
| 6 | Fiziksel özet satırları (boy, kilo, giyim) | Kurgusal persona verisi | — | — | `Kaynak: kurgusal (persona verisi)` |
| 7 | Big Five puanları (E90 · A70 · C60 · N40 · O70) | Kurgusal persona verisi | — | — | `Kaynak: kurgusal (persona verisi)` |
...
| 14 | Eski mood (Enerjik birincil / Sporcu ikincil) | Uyumlu | eski salt-okunur persona dosyası | [[personas/index]] §6.3.2 | Çelişki YOK — index ile birebir; §7.2'de "çakışma yok" satırı |
| 15 | Küme eğilimi (yüksek E, yüksek O, düşük N) ↔ persona E90/O70/N40 | Uyumlu | [[personas/mood-taxonomy]] §3.2.2 | Big Five tablosu (bu dosya) | Eğilim ile puan uyumlu — çelişki yok |
| 16 | Küme BPM notu "120-160 BPM ⚠️" | Doğrulanmadı | [[personas/mood-taxonomy]] §3.2.2 | — | `⚠️ VERIFICATION REQUIRED` — dosyada kullanılmadı |
| 17 | Tür BPM aralıkları (Pop 80-120 · Electro 90-130) | Gerçek-dünya (ikincil) | turkipedia.com/Beats_per_minute | nevamuzik.com.tr | `Kaynak: [k1] + [k2]` (P4.4 `VERIFIED (ikincil — birincil ölçüm DEĞİL)`) |
| 18 | Rap/spor marşı/halk müziği için tür-BPM aralığı | Kapsam dışı | research-bank P4.4 (bu türler listede yok) | — | `⚠️ VERIFICATION REQUIRED` — sayı yazılmadı |
| 19 | Şarkı bazlı BPM; "100-160 BPM" iddiası | Doğrulanmadı (EXCLUDED) | research-bank P4.7 / X-2 | — | `⚠️ VERIFICATION REQUIRED` — şarkı BPM'i yazılmadı |
| 20 | Favori sanatçı adları (Semicenk, Edis, Ceza, Hadise, Mabel Matiz; Drake, Eminem, The Weeknd, Travis Scott, Post Malone) | Persona tercihi (kurgusal); sanatçı realitesi P4 dışında | research-bank P4.2 (kapsam dışı) | — | `Kaynak: kurgusal (persona verisi)` + gerçek-dünya bilgi: `⚠️ VERIFICATION REQUIRED` |
...
| 23 | Eski cihazlar (Xiaomi Redmi Note 12, JBL kulaklık) | Doğrulanmadı (EXCLUDED) | research-bank P3.3 / X-1 | — | `⚠️ VERIFICATION REQUIRED` — spec yazılmadı |
...
| 31 | Eski dosyadaki istatistik/gelir/kan grubu/burç iddiaları (~45.000 TL, B RH+, Yengeç dahil) + TikTok hesabı (13 yaş altı kullanım) | Bankada YOK / DOĞRULANMADI | research-bank P1-P8 kapsamı | — | `⚠️ VERIFICATION REQUIRED` — bu dosyaya YAZILMADI |
...
| 5 | In-Place Refactoring | Dosya adı/yolu değişmez (`mert-celik.md`) | Dosya geri yüklenir |
...
| `Şarkı X BPM` / `rap 140 BPM` | Tür aralığı (Pop 80-120, Electro 90-130 VERIFIED ikincil) + rap için: `⚠️ VERIFICATION REQUIRED`; şarkı BPM'i her koşulda: `⚠️ VERIFICATION REQUIRED` |
...
| `Xiaomi Redmi Note 12 viewport` (eski veri) | `yalnız A34 VERIFIED; diğer cihazlar ⚠️ VERIFICATION REQUIRED (X-1)` |
...
| Persona-ölçüsüz eşik uydurma | "Mert için LCP 3000 ms" benzeri | Sil; genel P8 eşiği + `⚠️ DERIVED` gerekir (X-10) |
| Eski SSOT çelişki unutulması | Yaş 10 taşıması | §7.2 çakışma satırını ekle; index kazanır |
...
- [ ] BPM yalnız tür aralığı (P4.4); şarkı BPM'i yok; rap/spor marşı/halk müziği kapsam dışı bırakıldı
...
|------|--------------------------|-------------------|-------|
| Yaş | 10 | 8 | [[personas/index]] §6.3.2 kazanır; doğum tarihi 2016→2018 hizalandı |
| Doğum Tarihi | 28 Haziran 2016 | 28 Haziran 2018 (yaş 8 ile tutarlı) | SSOT yaş kazanır; tarih kurgusal olarak hizalandı |
...
| Mood (ikincil) | Sporcu | Sporcu (korunur) | Çelişki yok — küme §3.2.9'da mevcut |
| Sınıf | 5. sınıf | 3. sınıf (yaş 8 ile hizalı) | SSOT yaş kazanır; sınıf kurgusal olarak hizalandı |
| Birincil cihaz | Xiaomi Redmi Note 12 | Samsung Galaxy A34 5G (P3 VERIFIED) | [[personas/research-bank]] P3 |
| Ekran çözünürlüğü | Bilinmiyor (Xiaomi) | 1080×2340 → viewport 360×780 (`⚠️ DERIVED`) | X-1: doğrulanmamış cihaz spec'i yazılmadı |
| BPM aralığı | 100-160 BPM | Yalnız tür aralıkları (P4.4) + kurgusal tercih | X-2: rap/spor marşı/halk müziği P4.4 dışı; şarkı BPM'i yazılmadı |
| Küme Big Five eğilimi | "yüksek E, yüksek O, düşük N" | Persona E90 · O70 · N40 | Eğilim ile puan **uyumlu** — çelişki yok; §3.5.3'te kayıtlı |
...
| Gelir/kan grubu/burç iddiaları | ~45.000 TL, B RH+, Yengeç | Yazılmadı (bankada yok) | X-10: doğrulanamaz iddia bu şablonda taşınmaz |
| TikTok hesabı (günde 30 dk) | Eski dosyada var | Kullanılmadı | 13+ platform politikası research-bank kapsamında değil → `⚠️ VERIFICATION REQUIRED`; içerik YouTube/NBA olarak taşındı |

### §7.3 Değişiklik Geçmişi (append-only)

| Tarih | Versiyon | Değişiklik | Yazar |
|-------|----------|------------|-------|
| 2026-09-26 | 1.0.0 | İlk üretim — persona-template §3.5 (11/11) + research-bank P1-P8 etiketli veri; eski salt-okunur dosyadan kimlik/müzik/rutin taşındı, yaş 10→8 index §6.3.2 ile düzeltildi (mood zaten uyumlu; çelişkiler §7.2'de), test tablosu 6 sütuna genişletildi, veli_onayı + KVKK notu eklendi | vault-updater (persona) |

*(Append-only: bu tabloya yeni satır EKLENİR; mevcut satır değiştirilmez/silinmez.)*

---

**Authority:** Persona — SSOT: personas/erkek-cocuk/mert-celik
