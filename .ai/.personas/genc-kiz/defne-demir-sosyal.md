...
version: 1.0.0
status: active
authority: "Persona — SSOT: personas/genc-kiz/defne-demir-sosyal"
updated: 2026-09-26
group: genc-kiz
age: 17
mood: Sosyal
persona_id: CM-DD-17-SOS-ANT
...
| **Doğum Tarihi** | 27 Ağustos 2009 | Kaynak: kurgusal (persona verisi; yaş 17 ile tutarlılık için türetildi) |
| **Cinsiyet** | Kız | Kaynak: kurgusal (persona verisi) |
| **Şehir / İlçe / Semt** | Antalya / Muratpaşa / Lara | Kaynak: kurgusal (persona verisi) |
| **Okul / Meslek** | Muratpaşa Anadolu Lisesi, 12. sınıf | Kaynak: kurgusal (persona verisi; yaş 17 ile tutarlılık için türetildi — §7.2) |
| **Ulaşım** | Otobüs (ev-okul ~20 dk) | Kaynak: kurgusal (persona verisi) |
| **Online Rumuz** | defne_latinwave | Kaynak: kurgusal (persona verisi) |
| **CoreMusic Kullanıcı ID** | `CM-DD-17-SOS-ANT` | Kaynak: kurgusal (persona verisi) |
...
| **Saç / Göz** | Esmer düz saç, kahverengi göz | Kaynak: kurgusal (persona verisi) |
| **Gözlük / Lens / İşitme cihazı** | Yok (görme ve işitme iyi — kurgu) | Kaynak: kurgusal (persona verisi) |
| **Giyim / temsil notu** | Yazlık/sahil odaklı (renkli tişört, sandalet); açık tonlar; profil fotoğrafında gündüz ışığı (renk eşleşmesi `⚠️ DERIVED`, bkz. §3.7) | Kaynak: kurgusal (persona verisi) |
...
|-------|-----|--------------|-------------------|--------|
| Dışadönüklük | E | 100 | Akışta her şeyi paylaşır; yorum ve beğeni akışı onu canlı tutar. | Kaynak: kurgusal (persona verisi); ölçek: IPIP-NEO-120 |
| Uyumluluk | A | 90 | Arkadaş grubuyla uyumludur; çatışmadan kaçar, trend olanı onaylar. | Kaynak: kurgusal (persona verisi); ölçek: IPIP-NEO-120 |
| Sorumluluk | C | 50 | Paylaşım ve listeler yarı düzenli; okul işleri güne yayılır. | Kaynak: kurgusal (persona verisi); ölçek: IPIP-NEO-120 |
| Duygusal Denge (N tersi) | N | 20 | **Yüksek puan = düşük duygusal denge (ters kodlama).** 20 = beğeni dalgalanması ve eleştiri onu sarsmaz; sakin kalır. | Kaynak: kurgusal (persona verisi); ölçek: IPIP-NEO-120 |
| Deneyime Açıklık | O | 80 | Yeni türleri (latin, afrobeat) ve formatları hızla dener. | Kaynak: kurgusal (persona verisi); ölçek: IPIP-NEO-120 |
...
| Taksonomi BPM notu | §3.2.7 için küme BPM eşiği yoktur; test eşikleri olarak research-bank P4.4 (Pop 80-120 · House 110-128 · Dance 110-135) aralıkları kullanılır |
...
| 6 | Beğeni/ölçüm geri bildirimi | Anlık geri bildirim; kaygıyu büyüten bildirim yok (N=20) | DOM assertion + INP ≤ **200 ms** |

### Müzik DNA'sı

| Alan | Değer | Kaynak |
|------|-------|--------|
| **Türler (5, sıralı)** | 1) Latin pop 2) Deep house 3) Afrobeat 4) Yazlık pop 5) Reggaeton | Kaynak: kurgusal (persona verisi) |
| **Favori sanatçılar (5)** | Bad Bunny, Kygo, Rosalía, Dua Lipa, BLOK3 | Kaynak: kurgusal (persona verisi — sanatçı seçimi persona zevkidir); sanatçıların gerçek-dünya bilgileri research-bank P4 kapsamı dışında → `⚠️ VERIFICATION REQUIRED` (ek doğrulama turu) |
| **BPM aralığı (tür)** | Pop **80-120** · house **110-128** · dance **110-135** BPM | Kaynak: turkipedia.com/Beats_per_minute + nevamuzik.com.tr (research-bank P4.4 `VERIFIED`) |
| **BPM (şarkı bazlı)** | Yazılmadı — parçaya özgü BPM değeri doğrulanmadı | `⚠️ VERIFICATION REQUIRED` (research-bank P4.7 / EXCLUDED X-2) |
| **Kişisel tempo tercihi** | Orta-yüksek tempo (yazlık dans); latin ve house akışlarında yüksek dans BPM'i (kurgusal) | Kaynak: kurgusal (persona verisi — persona tercihi, ölçüm değil) |
| **Dinleme saati (hafta içi / sonu)** | Hafta içi 08:00-08:30 (yol), 16:00-18:00 (okul sonrası), 20:00-00:00 (akış/gece) · Hafta sonu 12:00-15:00 (sahil/arkadaş), 21:00-01:00 | Kaynak: kurgusal (persona verisi) |
| **Platform** | CoreMusic (sosyal profil + paylaşım) | Kaynak: kurgusal (persona verisi) |
| **Keşif davranışı** | Trend ve latin akışında hızlı keşif; gün içinde defalarca yeni liste | Kaynak: kurgusal (persona verisi) |
| **Premium olasılığı** | %70 (reklamsız dinleme + paylaşım grenleri — kurgusal) | Kaynak: kurgusal (persona verisi) |

**En sevdiği türler — gerekçe tablosu (kurgusal):**

| Sıra | Tür | Neden sevdiği |
|------|-----|---------------|
| 1 | Latin pop | Akışta görülen parça anında paylaşılır; yaz sohbetinin ortak dili |
| 2 | Deep house | Sahilde/Partide dans; ikincil Enerjik kümeyle örtüşür |
| 3 | Afrobeat | Yeni keşif; trend listelerinde öne çıkan ritim |
| 4 | Yazlık pop | Arkadaş grubuyla aynı parçalar; sosyal kanıt için gerekli |
| 5 | Reggaeton | Kısa, hızlı versiyon; hızlı skip ve paylaşım davranışına uyar |

**Dinleme zaman çizelgesi (kurgusal):**

| Gün | Saat | Aktivite | Öne çıkan tür |
|-----|------|----------|---------------|
| Hafta içi | 08:00-08:30 | Yol / otobüs | Latin pop / yazlık pop |
| Hafta içi | 16:00-18:00 | Okul sonrası | Afrobeat |
| Hafta içi | 20:00-21:30 | Ödev + akış | Deep house |
| Hafta içi | 21:30-00:00 | Sosyal akış / paylaşım | Reggaeton / latin pop |
| Hafta sonu | 12:00-15:00 | Sahil / arkadaş buluşması | Yazlık pop / latin |
| Hafta sonu | 21:00-01:00 | Dans / parti | Deep house / reggaeton |

*Not: BPM satırları yalnızca tür aralığı düzeyindedir (P4.4); "şarkı X BPM" iddiası bu dosyada YOKTUR (EXCLUDED X-2). Latin pop, afrobeat ve reggaeton için bankada aralık yok → o türler `⚠️ VERIFICATION REQUIRED` + kurgusal tercih olarak taşındı; house için P4.4 "house 110-128" aralığı kullanıldı.*
...
| **Diğer cihazlar (eski dosyadan)** | iPhone 14 Pro (eski ana cihaz), aile tableti — spec'leri bu turda DOĞRULANMADI | `⚠️ VERIFICATION REQUIRED` (research-bank P3.3 / EXCLUDED X-1) — spec sayıları yazılmadı |
...
- Gün içi ağırlıklıdır; paylaşım anları yolda, sahilde ve molada yoğunlaşır.
- Eleştiriye kapalı değil; kaygıyı büyüten bildirimlerde uygulamayı sessize alır.
...
| **Yorgunluk etkisi** | 00:00 sonrası paylaşım sıklığı düşer; karmaşık akış reddedilir | Kaynak: kurgusal (persona verisi) |
| **Güven tetikleyicisi** | Akışta tanıdık latin/dans parça duyulursa keşif + paylaşım tetiklenir | Kaynak: kurgusal (persona verisi); mood: [[mood-taxonomy]] §3.2.7 (paylaşan akış) |
...
Sen Defne Demir'sin. 17 yaşında, Antalya/Muratpaşa-Lara'da yaşıyorsun, Muratpaşa Anadolu Lisesi 12. sınıf öğrencisisin.
...
Müzik zevkin: latin pop, deep house, afrobeat, yazlık pop, reggaeton. En sevdiklerin: Bad Bunny, Kygo, Rosalía, Dua Lipa, BLOK3.
...
| 14 | Pop/House/Dance BPM aralıkları (80-120 · 110-128 · 110-135) | Gerçek-dünya (ikincil) | turkipedia.com/Beats_per_minute | nevamuzik.com.tr | `Kaynak: [k1] + [k2]` (P4.4 `VERIFIED (ikincil — birincil ölçüm DEĞİL)`) |
| 15 | Latin/afrobeat/reggaeton BPM aralığı | Doğrulanmadı | research-bank P4 (bu türler paket kapsamında değil) | — | `⚠️ VERIFICATION REQUIRED` + kurgusal tercih |
| 16 | Şarkı bazlı BPM | Doğrulanmadı (EXCLUDED) | research-bank P4.7 / X-2 | — | `⚠️ VERIFICATION REQUIRED` — şarkı BPM'i yazılmadı |
| 17 | Dans/sahil tempolu kişisel BPM tercihi | Kurgusal persona verisi (ölçüm değil) | — | — | `Kaynak: kurgusal (persona verisi)` |
| 18 | Favori sanatçı adları (Bad Bunny, Kygo, Rosalía, Dua Lipa, BLOK3) | Persona tercihi (kurgusal); sanatçı realitesi P4 dışında | research-bank P4.2 (kapsam dışı) | — | `Kaynak: kurgusal (persona verisi)` + gerçek-dünya bilgi: `⚠️ VERIFICATION REQUIRED` |
...
| 21 | Diğer cihazlar (eski iPhone 14 Pro, aile tableti) spec'leri | Doğrulanmadı (EXCLUDED) | research-bank P3.3 / X-1 | — | `⚠️ VERIFICATION REQUIRED` — spec yazılmadı |
...
| 30 | Davranış eşikleri (sabır 3 sn, spinner ~6 sn, reklam 5/10, premium %70) | Kurgusal persona verisi | — | — | `Kaynak: kurgusal (persona verisi)` |
...
| 5 | In-Place Refactoring | Dosya adı/yolu değişmez (`defne-demir-sosyal.md`) | Dosya geri yüklenir |
...
| `Şarkı X BPM` / `viral 120 BPM` | Tür aralığı (pop 80-120, house 110-128, dance 110-135, VERIFIED ikincil) + şarkı BPM'i: `⚠️ VERIFICATION REQUIRED` |
...
| Persona-özel eşik uydurma | "Defne için INP 500 ms" benzeri | Sil; genel P8 eşiği + `⚠️ DERIVED` gerekir (X-10) |
...
|------|--------------------------|-------------------|-------|
| Yaş | 15 | 17 | Index §6.3 kazanır; okul/sınıf yaşa göre yeniden kurgulandı (§3.1) |
| Mood | Sosyal (birincil) / Enerjik (ikincil) | Sosyal / Enerjik | Tutarlı — index §6.3 + mood-taxonomy §3.2.7/§3.2.2 |
| Kullanıcı ID | `CM-DD-15-ANT` (eski format) | `CM-DD-17-SOS-ANT` (şablon §3.5.1 `CM-XX-YY-ZZZ-XXX`) | Şablon formatı kazanır |
| Birincil cihaz | iPhone 14 Pro (EXCLUDED) | Samsung Galaxy A34 5G (P3 VERIFIED) | [[research-bank]] P3 |
| BPM tercihi | Latin/afrobeat için doğrulanmamış BPM | Yalnız tür aralıkları (P4.4) + `⚠️ VERIFICATION REQUIRED` latin/afrobeat/reggaeton | X-2: şarkı/türe özel doğrulanmamış BPM yazılmaz |
