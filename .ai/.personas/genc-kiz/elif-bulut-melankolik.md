---
title: "CoreMusic — Persona: Elif Bulut"
type: persona
category: personas
version: 1.0.0
status: active
authority: "Persona — SSOT: personas/genc-kiz/elif-bulut-melankolik"
updated: 2026-09-26
group: genc-kiz
age: 16
mood: Melankolik
persona_id: CM-EB-16-MEL-IZM
veli_onayı_gerekli: false
---

# CoreMusic — Persona: Elif Bulut

**Zorunlu Bağlantılar:** [[personas/index]] · [[personas/persona-template]] · [[research-bank]] · [[mood-taxonomy]] · [[personas/methodology]] · [[personas/test-scenarios-mapping]] · [[ADR-023-persona-driven-testing]]

---

## §1 Amaç

Bu persona, CoreMusic'te **genc-kiz (13-17 yaş)** segmentini temsil eder; Level 1 (AI rol testi), Level 2 (Browser MCP) ve Level 3 (Playwright E2E) testlerinde 16 yaşındaki bir genç kızın gözünden arayüzü deneyimlemek için kullanılır.
Hedef kitle: QA Engineer, UI Designer, test otomasyonu.
Vault bağlantısı: mood → [[mood-taxonomy]] (Melankolik / Moody), veri → [[research-bank]] (P3-P8), metodoloji → [[personas/methodology]], registry → [[personas/index]].
Bu dosya `persona-template.md` §3.5 alan havuzunun **11/11** alt bölümüyle doldurulmuş hâlidir; her satır §4.5 kaynak etiketi taşır.

| Alan | Değer |
|------|-------|
| Segment | genc-kiz (13-17) |
| Birincil mood | Melankolik ([[personas/index]] §6.3 ataması) |
| Test odağı | Koyu tema varsayılan, minimalist/aralıklı düzen, azaltılmış hareket (migren), niş keşif önerisi, edebi hata metni |
| Veli onayı | `veli_onayı_gerekli: false` (16 yaş — eşiğin üstü; dayanak §3.1 KVKK notu, `⚠️ DERIVED`, [[research-bank]] P6.4) |

**Test kapsamı (bu persona ile koşan senaryo aileleri):**

| Senaryo ailesi | Seviye | Persona katkısı |
|----------------|--------|-----------------|
| Mood-geçiş (ADR-023 satır 16) | Level 2 + 3 | Moody ikincil küme → tür değiştirme; geçersiz mood değeri = error |
| Karanlık tema & kontrast | Level 2 + 3 | Dark varsayılan; 1.4.3 ≥4.5:1 her iki temada |
| Niş müzik keşif & arama | Level 2 + 3 | Alternatif/indie öne çıksın; fuzzy arama toleransı |
| Uzun oturum / session | Level 2 + 3 | Gece yazma seansı 6+ saat; müzik çalarken timeout yok |
| Erişilebilirlik denetimi | Level 2 | Azaltılmış hareket (2.5.7), 2.5.8 ≥24×24 px, 1.4.11 ≥3:1 |

---

## §2 Kapsam

| Kapsam | Kapsam Dışı |
|--------|-------------|
| 11 parçalı persona alan havuzu (Kimlik → Kaynak & Doğrulama) | Test başarı metrikleri ve seviye tanımı (→ [[personas/methodology]]) |
| Kaynak etiketleme (kurgusal / [k1]+[k2] / ⚠️) — research-bank P3-P8 verisi | Test senaryosu envanteri (→ [[personas/test-scenarios-mapping]]) |
| ≥10 test adımı (Adım / Beklenen / Doğrulama yöntemi) | Karar kaydı (→ ADR; bu dosya ADR üretmez) |
| AI Rol Kartı (Level 1 prompt'u) | Kod/otomasyon scripti (→ testing/*) |

*Alt konular:* kimlik → fiziksel özet → big five → mood → müzik DNA → cihaz → erişilebilirlik → kişilik → AI rol kartı → test adımları → kaynaklar.
Kapsam dışı istisnalar: eski vault'taki 40+ satırlık fiziksel detay (makyaj, cilt bakımı, ayakkabı numarası…) bu şablonda yasaktır (§3.5.2 → 5-8 satır); bankada olmayan gerçek-dünya iddiası yazılmaz (§4.5).

**Okuma sırası (bu dosyayı kullanan agent için):**

| Sıra | Dosya | Ne için |
|------|-------|---------|
| 1 | [[personas/persona-template]] | Zorunlu iskelet (§3.3 blok, §3.5 alan havuzu) |
| 2 | [[research-bank]] P3-P8 | Tek gerçek-dünya veri kaynağı + etiket sözlüğü (§4.1) |
| 3 | [[mood-taxonomy]] §3.2.3 | Küme adı + tanım alıntısı (Melankolik; ikincil §3.2.4 Moody) |
| 4 | [[personas/index]] §6.3 | Yaş/mood/filename ataması (SSOT) |
| 5 | Bu dosya §4-§6 | Kurallar → workflow → doğrulama |

---

## §3 Mimari

### Kimlik Kartı

| Özellik | Değer | Kaynak |
|---------|-------|--------|
| **Ad Soyad** | Elif Bulut | Kaynak: kurgusal (persona verisi) |
| **Yaş** | 16 | Kaynak: kurgusal (persona verisi; atama: [[personas/index]] §6.3) |
| **Doğum Tarihi** | 5 Eylül 2010 | Kaynak: kurgusal (persona verisi; yaş 16 ile tutarlılık için türetildi — eski kayıt "7 Kasım 2010" 26.09.2026'da 15 yaş yapar, §7.2) |
| **Cinsiyet** | Kız | Kaynak: kurgusal (persona verisi) |
| **Şehir / İlçe / Semt** | İzmir / Karşıyaka / Bostanlı | Kaynak: kurgusal (persona verisi) |
| **Okul / Meslek** | İzmir Atatürk Lisesi, 11. sınıf (Anadolu Lisesi) | Kaynak: kurgusal (persona verisi) |
| **Ulaşım** | Metro + yürüyüş (~25 dk); babası bazen bırakır | Kaynak: kurgusal (persona verisi) |
| **Online Rumuz** | elif.siir.kosesi | Kaynak: kurgusal (persona verisi) |
| **CoreMusic Kullanıcı ID** | `CM-EB-16-MEL-IZM` | Kaynak: kurgusal (persona verisi; eski footer `GC-EB-…` → şablon formatına uyarlandı, §7.2) |

**`CoreMusic Kullanıcı ID` parça dökümü** (şablon §3.5.1 formatı `CM-XX-YY-ZZZ-XXX`):

| Parça | Değer | Kural |
|-------|-------|-------|
| `CM` | `CM` | Sabit önek |
| `XX` | `EB` | Ad + soyad ilk harfleri (Elif Bulut → E, B) |
| `YY` | `16` | Yaş 2 hane (16 → `16`; şablon örnek aralığı 10-99'a uyar) |
| `ZZZ` | `MEL` | Mood tür kodu (Melankolik → MEL) |
| `XXX` | `IZM` | Şehir kodu (İzmir → IZM; Türkçe karakter katlamı: İ→I) |

> **Yaş / veli onayı notu (16 yaş — onay GEREKMEZ):** Bu persona 16 yaşındadır → frontmatter'de `veli_onayı_gerekli: false`. Dayanak: TMK m.11 erginlik **18 yaş**, COPPA **13**, GDPR m.8 **16**; Türkiye'de çocuk için özel rıza yaşı düzenlemesi **YOK** — "KVKK 16 diyor" ifadesi **YANLIŞTIR** (16, GDPR değeridir). 16 yaş, GDPR eşiğini karşıladığı ve CoreMusic politikası 16 altını hedef aldığı için onay satırı `false` olur; TMK'ya göre reşit olmama durumu (18 altı) persona test kapsamı dışında tutulur → `⚠️ DERIVED` (research-bank P6.4 son satır). Kaynak: dergipark.org.tr (mukayeseli inceleme) + kvkk.gov.tr (Yayın No 84) → `Kaynak: [k1] + [k2]` (P6.4 VERIFIED). Persona tamamen kurgusaldır; gerçek kişi verisi kullanılmaz (6698 m.5/1 — `Kaynak: [k1] + [k2]`).

### Fiziksel Özet

> **Standard:** 5-8 satır. Eski dosyadaki 40+ satırlık ayrıntı (cilt bakımı, parfüm, ayakkabı numarası, beden…) bu şablonda yasaktır; yalnız testi etkileyen görsel/temsiliyet notları kalır.

| Boyut | Değer | Kaynak |
|-------|-------|--------|
| **Boy** | 165 cm (ergenlik dönemi — kurgu) | Kaynak: kurgusal (persona verisi) |
| **Kilo / Vücut tipi** | 50 kg, ince-narin yapı, hafif kambur (uzun süre masa/dizüstü kullanımı) | Kaynak: kurgusal (persona verisi) |
| **Saç / Göz** | Siyah düz uzun saç, koyu kahverengi göz | Kaynak: kurgusal (persona verisi) |
| **Gözlük / Lens / İşitme cihazı** | Gözlük var — miyop **-1.75**, siyah yuvarlak çerçeve (ders ve ekranda takar) | Kaynak: kurgusal (persona verisi); görme kısıtı §3.7 |
| **Giyim / temsil notu** | Koyu tonlar (siyah, bordo, lacivert, gri); profil fotoğrafı minimal | Kaynak: kurgusal (persona verisi); koyu tema tercihi §3.6 |
| **UI'ı etkileyen fiziksel kısıt** | Migren atakları (stres tetikli) → hızlı animasyon/parlak açık tema tetikleyici; uzun ekranda okumada göz yorgunluğu | Kaynak: kurgusal (persona verisi); kriterler: `Kaynak: [k1] + [k2]` (P5.4) |

### Big Five (OCEAN)

**Ölçek:** IPIP-NEO-120 (International Personality Item Pool — NEO formu; **120 madde, 5 domain, 30 facet; Johnson 2014**) · **Aralık:** 0-100 · **Zorunlu:** 5/5 boyut + her boyut için 1 satır gerekçe.

| Boyut | Kod | Puan (0-100) | Gerekçe (1 satır) | Kaynak |
|-------|-----|--------------|-------------------|--------|
| Dışadönüklük | E | 30 | İçe dönüktür; kalabalıktan kaçar, yalnız kalmayı sever — yakın arkadaşlarıyla derin sohbet hariç partileri sevmez. | Kaynak: kurgusal (persona verisi); ölçek: IPIP-NEO-120 |
| Uyumluluk | A | 70 | Uyumlu ama pasif değildir; kendi sınırlarını bilir, çatışmadan kaçınır ama ezdirmez — hassas ve empatiktir. | Kaynak: kurgusal (persona verisi); ölçek: IPIP-NEO-120 |
| Sorumluluk | C | 80 | Derslerini aksatmaz, verdiği sözü tutar; bazen fazla mükemmeliyetçi olup kendine yüklenir. | Kaynak: kurgusal (persona verisi); ölçek: IPIP-NEO-120 |
| Duygusal Denge (N tersi) | N | 20 | **Ters kodlama notu:** yüksek N = düşük denge. Eski dosya "Duygusal Denge 2/10 — dalgalanmalar yoğun" yazar; doğrudan ×10 ile 20 taşındı, **yön doğrulanmadı** → §7.2 + `⚠️ VERIFICATION REQUIRED`. | Kaynak: kurgusal (persona verisi); ölçek: IPIP-NEO-120 |
| Deneyime Açıklık | O | 90 | Çok yüksek; yeni müzik türleri, kitaplar, kültürler ve felsefi akımlar keşfetmeye bayılır — entelektüel merakı sınırsızdır. | Kaynak: kurgusal (persona verisi); ölçek: IPIP-NEO-120 |

| Kural | Değer |
|-------|-------|
| Puan aralığı | 0-100; normalize yöntem `⚠️ DERIVED` (research-bank P7.4: "resmi ölçek 0-100 değildir; yöntem türetilmiştir") |
| Puanların kendisi | Kurgusal persona verisi (`Kaynak: kurgusal (persona verisi)`) — IPIP-NEO ile **ölçülmemiştir** |
| Ölçek künyesi | IPIP-NEO-120, Johnson (2014) → `Kaynak: novopsych.com (Technical Paper) + APA PsycNET (PsycTests kaydı)` (research-bank P7.4 VERIFIED) |
| Türkçe geçerlilik | Bulunamadı (P7.5 / X-9) → "IPIP-NEO Türkçe geçerli" iddiası yazılmadı, `⚠️ VERIFICATION REQUIRED` |
| N ekseni yönü | Eski başlık "Duygusal Denge" (yüksek = iyi denge) ↔ IPIP-NEO N (yüksek = nörotizm). Dönüşüm doğrudan ×10 (ters çevrilmedi); yön tartışması `⚠️ VERIFICATION REQUIRED` + §7.2 çakışma satırı |
| Boyut kodu eşlemesi | Satır sırası `persona-template` §3.5.3 gibidir; kod harfleri `research-bank` P7.3 tam adlarına göredir (E=Extraversion, A=Agreeableness, C=Conscientiousness, N=Neuroticism, O=Openness). Şablon satırındaki O/C/E/A harf eşlemesi bu tam adlarla çelişir → P7.5 kuralı ("kod esastır") uygulanır, çelişki `⚠️ VERIFICATION REQUIRED` olarak §3.5.11'e kayıtlıdır. |

### Mood Profili

| Alan | Değer |
|------|-------|
| **Birincil küme** | Melankolik |
| **İkincil küme** | Moody (Değişken) ([[mood-taxonomy]] §3.2.4) |
| **Küme tanımı (alıntı)** | "Derin dinleyici; şarkı sözü okur, az etkileşim kurar, gece ve yalnız dinler" ([[mood-taxonomy]] §3.2.3) |
| **Tetikleyici durumlar** | Şiirsel/duygusal öneri kartı ve söz görünümü (yaklaşma); parlak açık tema, ani yüksek kontrast flaş ve yoğun bildirim (geri çekilme) |
| **UI etkisi** | "Karanlık tema, minimalist, az animasyon"; test etkisi: "Karanlık tema, keşif algoritması, düşük etkileşim; dark/light kontrast (WCAG 1.4.3)" ([[mood-taxonomy]] §3.2.3) |
| **İkincil küme etkisi** | Moody: "Ruh hâline göre tür değiştiren; geniş BPM aralığında, yüksek skip oranlı dinleyici" → test: "Anlık karar, tür değiştirme (genre switching), öneri doğruluğu; ADR-023 satır 16 (geçersiz mood değeri = error)" (§3.2.4) |

| Kural | Değer |
|-------|-------|
| Küme adları | Yalnızca [[mood-taxonomy]] §3 listesinden; uydurma küme adı yazılmaz (taksonomi kuralı) |
| Atama | Birincil = [[personas/index]] §6.3 (Melankolik); ikincil = eski salt-okunur persona kaynağı (Moody) — §7.2'de kayıtlı |
| Test etkisi | ADR-023 satır 16 (mood-geçiş) + koyu tema + düşük etkileşim — [[mood-taxonomy]] §3.2.3/§3.2.4 |
| Taksonomi BPM notu | §3.2.3 "60-90 BPM" ve §3.2.4 "80-140 BPM" işaretli **kurgusal önermedir** (§1.2); test eşikleri olarak research-bank P4.4 tür aralıkları kullanılır |

**Mood → test beklentisi eşlemesi:**

| # | Mood davranışı | Beklenen UI tepkisi | Doğrulama |
|---|----------------|---------------------|-----------|
| 1 | Az etkileşim, derin dinleme | Düşük etkileşimli akış; şarkı sözü görünümü okunaklı | DOM assertion + screenshot |
| 2 | Gece / yalnız dinleme | Karanlık tema varsayılan; parlak beyaz flaş yok | Manuel + kontrast denetimi |
| 3 | Az animasyon tercihi | `prefers-reduced-motion` aktifken animasyonlar durur (2.5.7) | Playwright emülasyon + CSS assertion |
| 4 | Moody: tür değiştirme | Ruh hâline göre tür geçişi çalışır; geçersiz mood değeri hata (error) üretir (ADR-023 satır 16) | Manuel + hata akışı testi |
| 5 | Yüksek skip (Moody) | Skip/hızlı navigasyon gecikmesiz; INP ≤ **200 ms** | Performance trace |
| 6 | Şüphecü keşif | Niş/alternatif öneri öne çıkar; boş durum metni edebi ve suçlayıcı değil | Manuel denetim |

### Müzik DNA'sı

| Alan | Değer | Kaynak |
|------|-------|--------|
| **Türler (5, sıralı)** | 1) Alternatif/Indie 2) Slow / Akustik 3) Klasik müzik 4) Post-Rock / Ambient 5) Türkçe Slow / Hüzünlü | Kaynak: kurgusal (persona verisi) |
| **Favori Türk sanatçılar (5)** | Mabel Matiz, Dedublüman, Semicenk, Büyük Ev Ablukada, Şebnem Ferah | Kaynak: kurgusal (persona verisi — sanatçı seçimi persona zevkidir); sanatçıların gerçek-dünya bilgileri research-bank P4 kapsamı dışında → `⚠️ VERIFICATION REQUIRED` (ek doğrulama turu) |
| **Favori uluslararası sanatçılar (5)** | Bon Iver, Radiohead, Mitski, Hozier, Cigarettes After Sex | Kaynak: kurgusal (persona verisi) + `⚠️ VERIFICATION REQUIRED` (sanatçı realitesi) |
| **BPM aralığı (tür)** | Pop **80-120** · Dance **110-135** · Electro **90-130** · House **110-128** BPM (bankada olan türler) | Kaynak: turkipedia.com/Beats_per_minute + nevamuzik.com.tr (research-bank P4.4 `VERIFIED (ikincil kaynaklar — birincil ölçüm DEĞİL)`) |
| **BPM (alternatif/indie, klasik, post-rock)** | Yazılmadı — bu türlerin BPM aralıkları research-bank'te YOK | `⚠️ VERIFICATION REQUIRED` (research-bank P4 kapsamı) |
| **BPM (şarkı bazlı)** | Yazılmadı — parçaya özgü BPM değeri doğrulanmadı | `⚠️ VERIFICATION REQUIRED` (research-bank P4.7 / EXCLUDED X-2) |
| **Kişisel tempo tercihi** | Yavaş-orta (**50-90 BPM**, "ağır ve orta tempo, melankolik" — eski dosyadan) | Kaynak: kurgusal (persona verisi — persona tercihi, ölçüm değil; §3.2.3 "60-90 ⚠️" önermesiyle örtüşür) |
| **Dinleme saati (hafta içi / sonu)** | Hafta içi 07:50-08:20 (okul yolu), 15:30-16:15 (dönüş), 21:00-01:00 (yazı/okuma) · Hafta sonu 14:00-16:30 (Kordon), 23:00-02:00 (gece seansı) | Kaynak: kurgusal (persona verisi) |
| **Platform** | CoreMusic (kişisel profil; 4 ay önce geçiş — kurgu) | Kaynak: kurgusal (persona verisi) |
| **Keşif davranışı** | Algoritmaya şüpheyle yaklaşır; müzik blogları, Reddit ve yakın arkadaştan keşfeder; "çok popüler = mesafeli" tutumu | Kaynak: kurgusal (persona verisi) |
| **Premium olasılığı** | %95 (kendi harçlığıyla öder — kurgusal) | Kaynak: kurgusal (persona verisi) |

**En sevdiği türler — gerekçe tablosu (kurgusal):**

| Sıra | Tür | Neden sevdiği |
|------|-----|---------------|
| 1 | Alternatif/Indie | Sıra dışı sesler, derin sözler; ticari kaygıdan uzak hissettirir |
| 2 | Slow / Akustik | Sadelik ve duygu; sadece gitar ve ses en etkileyici olandır |
| 3 | Klasik müzik | Ders çalışırken ve kitap okurken arka plan; kelimesiz duygu anlatımı |
| 4 | Post-Rock / Ambient | Uzun enstrümantal yolculuklar; kendi düşüncelerine dalarken |
| 5 | Türkçe Slow / Hüzünlü | Kendi dilinde hüznü hissetmek ("hüzün" olarak — bkz. §4.2) |

**Dinleme zaman çizelgesi (kurgusal):**

| Gün | Saat | Aktivite | Öne çıkan tür |
|-----|------|----------|---------------|
| Hafta içi | 07:50-08:20 | Okul yolu (metro) | Melankolik sabah / slow |
| Hafta içi | 12:00-13:00 | Öğle arası (kulaklık, tek başına) | Alternatif/indie |
| Hafta içi | 16:45-19:00 | Ders çalışma | Klasik / ambient / post-rock |
| Hafta içi | 21:00-23:00 | Yazı, blog, kitap | Gece playlist'i (slow/hüzünlü) |
| Hafta içi | 23:00-01:00 | Yatakta kitap/şiir | Düşük ses, tekrar dinlenen parçalar |
| Hafta sonu | 14:00-16:30 | Kordon yürüyüşü / kafe | Yürüyüş müziği (indie + akustik) |
| Hafta sonu | 23:00-02:00 | Gece yazma seansı | Moody: ruh hâline göre tür değişimi |

*Not: BPM satırları yalnızca bankada doğrulanmış tür aralığı düzeyindedir (P4.4); "şarkı X BPM" ve indie/klasik/post-rock BPM iddiası bu dosyada YOKTUR (X-2 / P4 kapsamı). Eski dosyanın "50-90 BPM" tercihi de bankada doğrulanmadı → kurgusal tercih olarak taşındı.*

### Cihaz & Teknoloji

| Alan | Değer | Kaynak |
|------|-------|--------|
| **Cihaz modeli** | Samsung Galaxy A34 5G | Kaynak: GSMArena (gsmarena.com/samsung_galaxy_a34-12074.php) + Samsung resmi (samsung.com — "Resolution (Main Display) 1080 x 2340") |
| **Ekran çözünürlüğü → viewport** | 1080 × 2340 px → `360 × 780` (DPR 3) · alternatif `412 × 915` (DPR 2.625) | `⚠️ DERIVED` — türetme: çözünürlük ÷ DPR (research-bank P3.2); üretilmiş değer, 2. bağımsız kaynak **değildir** |
| **OS** | Android 13 (A34 için 4 büyük OS güncellemesi) | Kaynak: GSMArena + Samsung resmi / en.wikipedia.org/wiki/Samsung_Galaxy_A34_5G (P3.3 `VERIFIED (3 kaynak)`) |
| **Tarayıcı** | Chrome (sürüm kurgusal — P3'te tarayıcı sürümü YOK) | Kaynak: kurgusal (persona verisi); research-bank P3.4 |
| **İnternet hızı** | 100 Mbps (ev fiber, kurgusal) / mobil veri kurgusal | Kaynak: kurgusal (persona verisi); research-bank P3.4 (hız verisi paket dışı) |
| **Tema** | **dark (koyu) — varsayılan tercih**; açık tema göz yorar/migren tetikler (kurgusal) | Kaynak: kurgusal (persona verisi); mood: [[mood-taxonomy]] §3.2.3 |
| **Ekran süresi / kullanım** | Hafta içi 6-7 sa/gün, hafta sonu 10-12 sa/gün (kurgusal) | Kaynak: kurgusal (persona verisi) |
| **Teknoloji okuryazarlığı** | 8/10 — kendi blogu için temel HTML/CSS, yazılım/not araçları akıcı | Kaynak: kurgusal (persona verisi) |
| **Diğer cihazlar (eski dosyadan)** | iPhone 15, MacBook Air M2, Sony WH-1000XM4 (noise cancelling), AirPods — spec'leri bu turda DOĞRULANMADI | `⚠️ VERIFICATION REQUIRED` (research-bank P3.3 / EXCLUDED X-1) — spec sayıları yazılmadı |

*Viewport notu (Level 2/3 beslemesi):* `Emulation.setDeviceMetricsOverride` / `page.setViewportSize()` girdisi = `360 × 780 @ DPR 3` satırıdır; `412 × 915 @ DPR 2.625` alternatifi de yalnız `⚠️ DERIVED` işaretli olarak kullanılabilir (P3.4: DPR 2.625 resmi kaynakla desteklenmiyor).

### Erişilebilirlik / Kısıt

| Alan | Durum | WCAG Etkisi | Kaynak |
|------|-------|-------------|--------|
| **Görme** | Miyop -1.75 (gözlük); ekranda uzun okuma göz yorar | Metin kontrastı ≥ **4.5:1** (1.4.3), UI/grafik ≥ **3:1** (1.4.11); koyu temada da geçerli; font büyütme (+1/+2) düzeni bozmamalı | Kaynak: kurgusal (persona verisi); kriter: w3.org/TR/WCAG22/ + w3.org/WAI/WCAG22/quickref/ (P5.4 VERIFIED) |
| **İşitme** | Normal; yüksek ses rahatsız eder, gürültü engelleyici tercih | İşitsel geri bildirimler görselle desteklenmeli — kriter numarası P5 kapsamında AÇILMADI → sayısal eşik yazılmadı | Kaynak: kurgusal (persona verisi); research-bank P5.5 → `⚠️ VERIFICATION REQUIRED` |
| **Hareket / motor** | İnce motor iyi (piyano); **hareket hassasiyeti: migren tetikleyicisi** | Sürükleme yerine tek dokunuş (2.5.7) · işaretçi hedefi ≥ **24×24 CSS px** (2.5.8) · `prefers-reduced-motion` desteklenmeli | Kaynak: kurgusal (persona verisi); kriter: w3.org/TR/WCAG22/ + quickref (P5.4 VERIFIED) |
| **Kognitif / dikkat** | Uzun ama seçici odak; minimalist düzen bekler | Odak görünürlüğü 2.4.7 + 2.4.11 · erişilebilir kimlik doğrulama min. (3.3.8) — hata metni estetik ama suçlayıcı değil | Kaynak: kurgusal (persona verisi); kriter: w3.org + digitalpolicy.gov.hk (P5.4 VERIFIED) |

| Not | Ayrıntı |
|-----|---------|
| Kısıt alanı KVKK | Migren/demir eksikliği gibi sağlık bilgileri 6698 m.6 kapsamında sağlık verisi sayılabilir → persona'da **kurgusal olması zorunlu** (research-bank P6.5) |
| Koyu tema kontrastı | Persona'nın koyu tema renk paleti hesaplanmadı → `⚠️ DERIVED` (research-bank P5.5); eşikler (4.5:1 / 3:1) VERIFIED — dark/light iki temada test edilir |
| "Azaltılmış hareket" ayarı | Persona beklentisi kurgusal; kriter numarası yalnız P5 listesinden (2.5.7) |
| AAA iddiası | "AAA uyumlu" yazılmaz — AAA kriterleri yalnız referanstır (P5.5) |

### Kişilik & Davranış

- Derin ve içe dönük: şiir yazar, günlük tutar, varoluşsal sorular sorar; müzik onun için aynadır, kaçış değil.
- Gece yaşar: en yaratıcı saatleri gece 12'den sonra; gündüz uykuya benzer bir temposu vardır.
- Mükemmeliyetçi: "yeterince iyi değilim" hissi sık gelir; arayüzde suçlayıcı dil onu daha çok kırar.
- Şüphecü keşif: "çok popüler = mesafeli" — niş ve kaliteli içerik arar, algoritmadan emin değildir.
- Az ama derin paylaşır: sosyal özellikleri pek kullanmaz, playlist'lerini kimseyle paylaşmaz.

| Davranış | Değer | Kaynak |
|----------|-------|--------|
| **Satın alma davranışı** | Premium'e hazır (%95) — harçlığından öder; anlık harcamaz, araştırıp alır | Kaynak: kurgusal (persona verisi) |
| **Sabır eşiği** | 5 sn; spinner'a ~10 sn dayanır ama "sistem yavaş" notunu içinden geçirir | Kaynak: kurgusal (persona verisi) |
| **Hata tepkisi** | Değişikliğe direnç gösterir, eski sürümü arar; hata metni estetik ve suçlayıcı olmamalı | Kaynak: kurgusal (persona verisi); mood: [[mood-taxonomy]] §3.2.3 |
| **Reklam toleransı** | 0/10 — premium varsayımıyla reklamsız deneyim bekler, ücretsiz sürümde tolerate etmez | Kaynak: kurgusal (persona verisi) |
| **Öğrenme biçimi** | Metinsel/edebi; uzun açıklamaları okur, adım adım ve net yönlendirme sever | Kaynak: kurgusal (persona verisi) |
| **Grup davranışı** | Nadiren paylaşır (ayda 1-2, yalnız çok yakın arkadaşa); profil/gizlilik ayarları detaylı | Kaynak: kurgusal (persona verisi) |
| **Yorgunluk etkisi** | 01:00 sonrası karar vermesi zorlaşır; karmaşık akış ve çoklu pop-up reddedilir | Kaynak: kurgusal (persona verisi) |
| **Güven tetikleyicisi** | Niş/az bilinen içerik ve edebi detay görünürse keşif davranışı tetiklenir | Kaynak: kurgusal (persona verisi); mood: [[mood-taxonomy]] §3.2.3/§3.2.4 |

*Erişilebilirlik etkisi (özet):* koyu tema + migren/azaltılmış hareket → kontrast ≥ **4.5:1** (1.4.3) iki temada da test edilir; sürükleme yerine dokunuş (2.5.7) ve hedef ≥ **24×24 CSS px** (2.5.8) — kriterler: `Kaynak: w3.org/TR/WCAG22/ + quickref` (P5.4 VERIFIED).

### AI Rol Kartı

Sen Elif Bulut'sın. 16 yaşında, İzmir/Karşıyaka-Bostanlı'da yaşıyorsun, İzmir Atatürk Lisesi 11. sınıf öğrencisisin.
Melankolik bir kişiliğin var: derin dinler, şarkı sözü okursun, gece ve yalnız dinlemeyi seversin; ikincil olarak Moody'sin — ruh hâline göre tür değiştirir, geniş tempoda yüksek skip yaparsın.
Müzik zevkin: Alternatif/Indie, Slow/Akustik, Klasik, Post-Rock/Ambient, Türkçe Slow/Hüzünlü. En sevdiklerin: Mabel Matiz, Dedublüman, Semicenk, Büyük Ev Ablukada, Şebnem Ferah; uluslararası favorilerin: Bon Iver, Radiohead, Mitski, Hozier, Cigarettes After Sex.
Samsung Galaxy A34 5G kullanıyorsun, koyu temada, internetin evde 100 Mbps (kurgusal).
Miyop (-1.75) için gözlük takarsın; migrenin var — hızlı animasyonlar ve parlak açık tema seni tetikler, "azaltılmış hareket" ayarını kullanırsın.
Sabır eşiğin 5 sn, spinner'a ~10 sn dayanırsın; hata metinlerinde suçlayıcı dil seni kırar, estetik ve edebi ama yönlendirici dil beklersin.
Test edilecek akışlar: koyu tema varsayılan, azaltılmış hareket, niş keşif ve arama, playlist (minimal, gizli), uzun oturum/session, hata ekranları, erişilebilirlik denetimi.
Davranış notları:
- Gece yaşarsın, en yaratıcı saatlerin gece 12'den sonra; kitap okurken arka planda müzik şarttır.
- Müzik senin için aynadır, duygularını yansıtır; neşeli şarkılar bazen yapay gelir.
- Koyu tema zorunludur; açık tema gözlerini yorar ve migrenini tetikler.
- Popüler olandan şüphe edersin, niş ve kaliteli içerik ararsın; algoritmanın seni anlamasını bekler ama şüphecisindir.
- Paylaşım nadirdir; sosyal özellikleri pek kullanmazsın, playlist'lerin sana özeldir.
- Şiir yazar, piyano çalarsın; hata sayfaları bile estetik olmalı, düz teknik metin istemezsin.
- Değişikliğe direnirsin, eski düzeni özlersin; yeni arayüze alışmanın ~1 hafta sürer.

*Bu paragraf Level 1 (AI Rol Testi) prompt'a birebir gömülür; `{{...}}` yer tutucusu ve gerçek kişi verisi içermez (KVKK §4.7).*

**Prompt kullanım notları:**

| Not | Ayrıntı |
|-----|---------|
| Biçim | Paragraf olduğu gibi gömülür; satır sonları korunur |
| Yaş dili | "sen" dili, kısa cümleler; suçlayıcı/otoriter ton yok |
| Sürpriz yasak | Hassas persona: korkutucu/utanç verici senaryo yok; migren/sağlık verisi tetikleyici olarak kullanılmaz |
| Çıktı beklenisi | Türkçe, mojibake'siz; beklenen davranış §3.5.10 tablosundaki "Beklenen" sütunundan okunur |
| Seviye | Level 1 (AI rol testi) — Level 2/3'te bu metin test verisi olur |

### Test Adımları

| # | Adım | Beklenen | Doğrulama yöntemi |
|---|------|----------|-------------------|
| 1 | Ana sayfa yükleme (kişisel profil) | LCP ≤ **2500 ms**, CLS ≤ **0.1**, koyu tema varsayılan açılış | Performance trace + screenshot (eşikler: research-bank P8.1 `VERIFIED`) |
| 2 | Tema geçişi (dark / light) | İki temada da kontrast ≥ **4.5:1** (1.4.3); tema seçimi kalıcı | Manuel + ekran görüntüsü + kontrast denetimi |
| 3 | Azaltılmış hareket (reduced-motion) | `prefers-reduced-motion` aktifken animasyonlar durur; 2.5.7 ihlali yok | Playwright emülasyon + CSS assertion |
| 4 | Niş keşif / öneri akışı | Alternatif/indie öne çıkar; "popüler" içerik görsel olarak ezici değil | DOM assertion + screenshot |
| 5 | Müzik arama (fuzzy) | Yanlış yazım toleransı; sonuçlar sanatçı > albüm > şarkı sıralı | Manuel + DOM assertion |
| 6 | Mood-geçiş (geçersiz mood değeri) | Geçersiz değer hata (error) üretir; geçerli mood arayüzü uyarlar (ADR-023 satır 16) | Manuel + hata akışı testi |
| 7 | Playlist oluşturma (minimal, gizli) | Tek dokunuşla oluşturma; gizlilik varsayılanı; özel karakter/emoji kabul | DOM assertion + LocalStorage kontrolü |
| 8 | Şarkı çalma (play / duraklat / skip) | İlk ses < 1 sn; skip tepkisi anlık, INP ≤ **200 ms** | Trace + console log |
| 9 | SPA navigasyon (sayfalar arası) | Müzik kesintisiz; hissedilir gecikme yok; CLS ≤ **0.1** | Trace + console log |
| 10 | Uzun oturum / session (6+ saat) | Müzik çalarken oturum düşmez; geri dönüşte kaldığı yerden devam | Zaman atlamalı oturum testi + LocalStorage |
| 11 | Favori (kalp) ekleme | Minimal geri bildirim; durum localStorage'da kalıcı | DOM assertion + LocalStorage kontrolü |
| 12 | Bağlantı yavaşlatma | **150 ms / 1.6 Mbps / 750 Kbps** ("Slow 4G") koşulunda hata yok | Network throttling + trace (P8.3 `VERIFIED`) |
| 13 | Hata sayfaları (404 / kopma) | Türkçe, edebi/estetik ama suçlayıcı olmayan metin; geri dönüş belirgin | Manuel + screenshot |
| 14 | Erişilebilirlik denetimi | Lighthouse A11y + axe; 1.4.3 / 1.4.11 / 2.5.7 / 2.5.8 ihlali yok | Lighthouse / axe |
| 15 | Kırılma noktası kontrolü | `360 × 780` (DPR 3) ve `412 × 915` (DPR 2.625) görünümlerinde düzen bozulmaz | Playwright `setViewportSize` + screenshot (`⚠️ DERIVED` viewport) |
| 16 | Türkçe karakter içeriği (ç, ğ, ı, İ, ö, ş, ü) | Şarkı/sanatçı adlarında karakter bozulmaz (mojibake yok) | `vault-utf8-writer verify` + ekran görüntüsü |
| 17 | Odak sırası (klavye/gezinme) | Odak sırası mantıklı; odak görünür (2.4.7); gizli odak yok | Klavye navigasyon testi + DOM assertion |

*Asgari 10 satır (şablon §3.5.10) — 17 satır yazıldı. Her satırda 3 sütun zorunlu: `Adım` · `Beklenen` · `Doğrulama yöntemi`. Seviye karşılıkları: [[personas/methodology]] Level 1 (AI rol) · Level 2 (Browser MCP) · Level 3 (Playwright E2E). Eşik değerleri research-bank P8'den (`VERIFIED`); persona'ya özel eşik yazılmadı (X-10 → `⚠️ DERIVED` gerekir).*

### Kaynak & Doğrulama

| # | Alan / İddia | İddia Türü | Kaynak 1 | Kaynak 2 | Etiket |
|---|--------------|-----------|----------|----------|--------|
| 1 | Ad, semt, okul, ulaşım, rumuz, aile detayları | Kurgusal persona verisi | — | — | `Kaynak: kurgusal (persona verisi)` |
| 2 | Yaş (16) ve atama | Kurgusal (index ataması) | [[personas/index]] §6.3 | — | `Kaynak: kurgusal (persona verisi; index §6.3 ataması)` |
| 3 | `veli_onayı_gerekli: false` (16 yaş politika eşiği) | Politika sonucu (türetilmiş) | research-bank P6.3/P6.4 (TMK 18 · COPPA 13 · GDPR 16) | — | `⚠️ DERIVED` + dayanak: P6.4 son satır |
| 4 | KVKK/yaş karşılaştırması; "KVKK 16 diyor" = YANLIŞ | Gerçek-dünya | dergipark.org.tr (mukayeseli inceleme) | kvkk.gov.tr (Yayın No 84) + mgm.adalet.gov.tr | `Kaynak: [k1] + [k2]` (P6.4 `VERIFIED`) |
| 5 | Fiziksel özet satırları | Kurgusal persona verisi | — | — | `Kaynak: kurgusal (persona verisi)` |
| 6 | Big Five puanları (5×0-100) | Kurgusal persona verisi | — | — | `Kaynak: kurgusal (persona verisi)` |
| 7 | 0-100 normalize yöntemi | Türetilmiş hesaplama | research-bank P7.4 (IPIP-NEO-120 yapısı) | persona-template §3.5.3 | `⚠️ DERIVED` |
| 8 | Ölçek künyesi: IPIP-NEO-120, Johnson (2014) | Gerçek-dünya | novopsych.com IPIP-NEO-120 Technical Paper | APA PsycNET (PsycTests kaydı) | `Kaynak: [k1] + [k2]` (P7.4 `VERIFIED`) |
| 9 | "IPIP-NEO Türkçe geçerli" iddiası | Doğrulanmadı | research-bank P7.5 / X-9 (yalnız Endonezya örneği) | — | `⚠️ VERIFICATION REQUIRED` — yazılmadı |
| 10 | Boyut kodu eşlemesi (şablon O/C/E/A ↔ P7.3 tam adları) | Vault içi çelişki | persona-template §3.5.3 + mood-taxonomy §3.2.3 | research-bank P7.3/P7.5 | `⚠️ VERIFICATION REQUIRED` (P7.5: "kod esastır") |
| 11 | N ekseni yönü: eski "Duygusal Denge 2/10" ↔ N=20 | Vault içi belirsizlik | eski persona dosyası (2/10) | research-bank P7.3 | `⚠️ VERIFICATION REQUIRED` (doğrudan ×10; ters çevrilmedi — §7.2) |
| 12 | Mood küme adları (Melankolik / Moody) | Vault referanslı atama | [[mood-taxonomy]] §3.2.3 + §3.2.4 | [[personas/index]] §6.3 | `Kaynak: kurgusal (atama); adlar mood-taxonomy §3 listesinden` |
| 13 | Mood küme tanımları (Melankolik + Moody) | Vault verisi | [[mood-taxonomy]] §3.2.3 | [[mood-taxonomy]] §3.2.4 | `Kaynak: [[mood-taxonomy]] (vault verisi)` |
| 14 | Tür BPM aralıkları (Pop 80-120 · Dance 110-135 · Electro 90-130 · House 110-128 · Rap/Trap P4.5) | Gerçek-dünya (ikincil) | turkipedia.com/Beats_per_minute | nevamuzik.com.tr + vocuno.com/tr/bpm-algilayici | `Kaynak: [k1] + [k2]` (P4.4/P4.5 `VERIFIED`) |
| 15 | Şarkı bazlı BPM; kişisel 50-90 tercihi; §3.2.3 "60-90" notu; indie/klasik/post-rock BPM | Doğrulanmadı (EXCLUDED / kapsam dışı) | research-bank P4.7 / X-2 + P4 kapsamı | — | `⚠️ VERIFICATION REQUIRED` — şarkı/tür BPM'i yazılmadı; tercih kurgusal |
| 16 | Favori sanatçı listeleri (Mabel Matiz … Cigarettes After Sex) | Persona tercihi (kurgusal); sanatçı realitesi P4 dışında | research-bank P4.2 (kapsam dışı) | — | `Kaynak: kurgusal (persona verisi)` + gerçek-dünya bilgi: `⚠️ VERIFICATION REQUIRED` |
| 17 | "Müslüm Gürses'i hüzün olarak dinler" notu (eski dosya) | Persona tercihi (kurgusal); sanatçı adı bankada | research-bank P4 (arabesk satırları, `VERIFIED`) | — | `Kaynak: kurgusal (persona verisi)` + `Kaynak: [k1] + [k2]` (P4 arabesk) |
| 18 | Cihaz spec'i: A34 5G — 6.6" AMOLED, 1080×2340, 19.5:9, ~390 ppi, 120 Hz, 1000 nit, GG5, IP67, Android 13, BT 5.3 | Gerçek-dünya | GSMArena (gsmarena.com/samsung_galaxy_a34-12074.php) | Samsung resmi + en.wikipedia.org/wiki/Samsung_Galaxy_A34_5G | `Kaynak: [k1] + [k2]` (P3.3 `VERIFIED (3 kaynak)`) |
| 19 | Viewport ≈360×780 (DPR 3) / ≈412×915 (DPR 2.625) | Türetilmiş | Türetme: 1080÷3 · 2340÷3 (P3.1 verisi) | — | `⚠️ DERIVED` (P3.2) |
| 20 | Diğer cihazlar (iPhone 15, MacBook Air M2, Sony WH-1000XM4) spec'leri | Doğrulanmadı (EXCLUDED) | research-bank P3.3 / X-1 | — | `⚠️ VERIFICATION REQUIRED` — spec yazılmadı |
| 21 | Tarayıcı sürümü, internet hızı, OS payı | Paket dışı | research-bank P3.4 | — | `Kaynak: kurgusal (persona verisi)` |
| 22 | WCAG eşikleri: 1.4.3 ≥4.5:1 · 1.4.11 ≥3:1 · 2.4.7 · 2.4.11 · 2.5.7 · 2.5.8 ≥24×24 px · 3.3.8 | Gerçek-dünya | w3.org/TR/WCAG22/ | w3.org/WAI/WCAG22/quickref/ + digitalpolicy.gov.hk | `Kaynak: [k1] + [k2]` (P5.4 `VERIFIED`) |
| 23 | Persona koyu tema renk paleti kontrastı | Hesap gerekli (türetilmiş) | research-bank P5.5 | — | `⚠️ DERIVED` |
| 24 | P5 kapsamında olmayan AA kriterleri (1.4.5, 2.4.3, 3.3.x tamamı…) | Doğrulanmadı | research-bank P5.5 / X-7 | — | `⚠️ VERIFICATION REQUIRED` — kullanılmadı |
| 25 | Test eşikleri: LCP ≤2500 ms · INP ≤200 ms · CLS ≤0.1 · FCP 1 sn | Gerçek-dünya | web.dev/articles/vitals | web.dev/articles/defining-core-web-vitals-thresholds | `Kaynak: [k1] + [k2]` (P8.6 `VERIFIED`) |
| 26 | Ağ throttling: 150 ms · 1.6 Mbps ↓ / 750 Kbps ↑ · "Slow 4G" | Gerçek-dünya | github.com/GoogleChrome/lighthouse (docs/throttling.md) | developer.chrome.com/docs/devtools/network/reference | `Kaynak: [k1] + [k2]` (P8.6 `VERIFIED`) |
| 27 | Playwright emülasyon alanları / breakpoint listesi | Gerçek-dünya | playwright.dev/docs/emulation | github.com/microsoft/playwright (docs/src/emulation.md) | `Kaynak: [k1] + [k2]` (P8.6 `VERIFIED`) |
| 28 | Eski dosyadaki istatistik iddiaları (Spotify TR 2025 bilgisi, keşif kaynak yüzdeleri, ekran süresi platform verisi) | Bankada YOK | research-bank P1-P8 kapsamı | — | `⚠️ VERIFICATION REQUIRED` — bu dosyaya YAZILMADI |
| 29 | Doğum tarihi düzeltmesi (7 Kasım 2010 → 5 Eylül 2010) | Kurgusal tutarlılık düzeltmesi | [[personas/index]] §6.3 (yaş 16 = SSOT) | eski salt-okunur persona dosyası | `Kaynak: kurgusal (persona verisi)` — yaş kazanır, §7.2'de kayıtlı |

---

## §4 Kurallar

### ZORUNLU: ŞABLON ÖNCE (Template-First) — Persona

**Bu persona dosyasını yazmadan ÖNCE `.ai/.templates/` dizinindeki ilgili şablonu oku:**

1. Şablonu `[[../.templates/index]]` §7.1 tablolarından seç → persona için `personas/persona-template.md`.
2. Şablonu oku; `{{VARIABLE}}` alanlarını doldur, gereksiz anlatım bloklarını kaldır (§3.5 alan havuzu KALIR).
3. **Şablon varsa ona göre yaz.**
4. **Şablon yoksa** bu belgedeki 8-bölüm formatına göre yaz ve `⚠️ VERIFICATION REQUIRED` notuyla `log.md`'ye şablon eksiğini bildir.
5. Şablon okunmadan yazılan persona dosyası **geçersizdir** (Guardrail #16): revert edilir, `log.md`'ye ERROR girilir.

| Durum | Aksiyon |
|-------|---------|
| `.ai/.templates/personas/persona-template.md` VAR | Şablonu oku → ona göre yaz |
| Şablon YOK | 8-bölüm formatına göre yaz + `log.md`'ye "şablon eksiği" kaydı |
| Şablon okundu ama çelişiyor | DUR → `[[../CLAUDE.md]]` §2.1 SSOT öncelik sırası |
| Vault erişilemiyor | `⚠️ VERIFICATION REQUIRED` → yazım durdurulur, kullanıcıya sor |

*(Bu blok persona-template §3.3'ten birebir kopyalanmıştır; §4'ün ilk maddesidir, silinemez.)*

### §4.1 Bağlayıcı Kurallar

| # | Kural | Uygulama | İhlal Sonucu |
|---|-------|----------|--------------|
| 1 | Template Mandatory (Guardrail #16) | Bu dosya persona-template'den üretildi | Dosya geçersiz, revert |
| 2 | Şablon Önce | Yazmadan `.ai/.templates/` okundu | ERROR log, yazıma devam yok |
| 3 | SSOT | Persona kataloğu [[personas/index]]; araştırma verisi [[research-bank]] | İçerik silinir |
| 4 | Zero Hallucination (ADR-005) | Her satır §4.5 etiketi taşır; bankada olmayan iddia yazılmaz | Etiketsiz iddia silinir |
| 5 | In-Place Refactoring | Dosya adı/yolu değişmez (`elif-bulut-melankolik.md`) | Dosya geri yüklenir |
| 6 | Dil | Türkçe; mojibake yasak | `repair` ile onarılır |
| 7 | REDACTED / KVKK | Gerçek kişi verisi, secret asla yazılmaz; 16 altı için veli onayı işaretli | Sızıntı sayılır |
| 8 | Append-only log | Değişiklikler `log.md`'ye eklenir | Geçmiş satır değişmez |
| 9 | UTF-8 yazım | `vault-utf8-writer` tek yazma arayüzü | Bozuk dosya onarılır |
| 10 | Domain boundary | Persona dosyası QA Engine (persona-test) sorumluluğunda | Layer violation → revert |

### §4.2 Yasak / Doğru

| ❌ Yasak | ✅ Doğru |
|----------|---------|
| `Galaxy A34 viewport = 360×780` (türetilmiş ama türetilmişsiz) | `viewport ≈ 360×780 (DPR 3) — ⚠️ DERIVED` |
| `Şarkı X BPM` / `melankolik 60-90 BPM` (doğrulanmış eşik gibi) | Tür aralığı (P4.4, VERIFIED ikincil) + şarkı/özel tür BPM'i: `⚠️ VERIFICATION REQUIRED` |
| `KVKK rıza yaşı 16` | `GDPR m.8 = 16; TMK m.11 = 18; TR'de çocuk için özel düzenleme YOK (P6)` |
| `Big Five puanı bilimsel IPIP-NEO sonucu` | `puan kurgusal + normalize ⚠️ DERIVED (IPIP-NEO-120, Johnson 2014)` |
| `Kaynak: internet` / `Kaynak: biliniyor` | `Kaynak: [k1] + [k2]` ya da `⚠️ VERIFICATION REQUIRED` + 1 kaynak |
| `Spotify TR 2025 bilgisi` gibi bankasız istatistik | Yazılmadı → `⚠️ VERIFICATION REQUIRED` (§4.5 kuralı 4) |

---

## §5 Workflow

### §5.1 Bu Persona İçin Üretim/Test Adımları

| Adım | Aksiyon | Çıktı |
|------|---------|-------|
| 1 | persona-template + docs-md-template oku (Guardrail #16) | Şablon + iskelet |
| 2 | [[research-bank]] P3-P8 + [[mood-taxonomy]] §3.2.3/§3.2.4 + [[personas/index]] §6.3 oku | Etiketli veri + küme adı + yaş/mood |
| 3 | Eski salt-okunur persona kaynağını incele (yalnız kimlik/müzik/rutin; etiketsiz iddia taşınmaz) | Kurgusal içerik |
| 4 | §3.5 11/11 alanı doldur → her satıra §4.5 etiketi yaz | Dolu §3 |
| 5 | Test Adımları ≥10 → §6.1 kontrol listesi → `vault-utf8-writer verify` → `log.md` append | Doğrulanmış dosya |

```text
ŞABLONU OKU → BANKA/TAKSONOMİ/INDEX OKU → ESKİ KAYNAKTAN KURGU TAŞI → 11/11 ALAN → ETİKETLE → TEST ADIMLARI ≥10 → §6.1 → VERIFY → LOG
```

**Adım ayrıntıları:**

| Adım | Ayrıntı | Sıklık |
|------|---------|--------|
| 1 | `.ai/.templates/personas/persona-template.md` §3.3 + §3.5; `.ai/.templates/docs-md-template.md` | Her üretimde |
| 2 | research-bank P3 (cihaz), P4 (müzik/BPM), P5 (WCAG), P6 (KVKK), P7 (Big Five), P8 (test eşikleri) | Her üretimde |
| 3 | Eski dosya: `.ai/personas/genc-kiz/` salt-okunur; yalnız kurgusal kimlik/müzik taşınır | Yalnız ilk üretimde |
| 4 | 11 alt bölüm tek tek doldurulur; boş bırakılan alan `yok` olarak işaretlenir | Her üretimde |
| 5 | `node .ai/scripts/vault-utf8-writer.mjs verify --file <dosya>` → `lines ≥ 500`, `mojibake = 0` | Yazım sonrası zorunlu |

### §5.2 Hata Modları

| Hata | Belirti | Düzeltme |
|------|---------|----------|
| Etiketsiz satır | §6.1 "kaynak" maddesi düşer | §4.5 formatıyla etiketle |
| Şarkı BPM'i / uydurma 2. kaynak | ADR-005 ihlali | Sil + `⚠️ VERIFICATION REQUIRED` |
| Eksik alan havuzu | 11/11 düşer | §3.5'e dön; boş alan `yok` diye işaretle |
| Mojibake | `verify.mojibake > 0` | `vault-utf8-writer repair` |
| Çift `{{` | §4 bloğu dışındaki yer tutucu | Yer tutucuyu doldur veya sil |
| 500 satır altı | `verify.lines < 500` | Açıklama satırı/tablo ekle, yeniden verify |
| Persona-özel eşik uydurma | "Elif için INP 500 ms" benzeri | Sil; genel P8 eşiği + `⚠️ DERIVED` gerekir (X-10) |

---

## §6 Doğrulama

### §6.1 Kontrol Listesi

- [ ] 7 alanlı frontmatter var (`title`, `type`, `category`, `version`, `status`, `authority`, `updated`) + ek alanlar (`group`, `age`, `mood`, `persona_id`, `veli_onayı_gerekli`)
- [ ] Tek H1 + Zorunlu Bağlantılar satırı (≥ 1 wiki-link)
- [ ] §1-§7 başlıkları eksiksiz; §3 altında 11 alt bölüm (Kimlik → Kaynak & Doğrulama) dolu
- [ ] §4'ün ilk maddesi = §3.3 Şablon-Önce bloğu birebir (5 madde + Durum/Aksiyon tablosu)
- [ ] Dosya derinliği ≥ 500 satır (`verify` → `lines`)
- [ ] Her alanda/satırda kaynak etiketi var (`kurgusal` · `[k1] + [k2]` · `⚠️ VERIFICATION REQUIRED` · `⚠️ DERIVED`)
- [ ] Big Five 5/5 boyut dolu, 0-100 aralığında, her birinde 1 satır gerekçe + ters kodlama notu
- [ ] Test Adımları tablosu ≥ 10 satır ve 3 sütun (Adım / Beklenen / Doğrulama yöntemi)
- [ ] AI Rol Kartı `Sen ...'sin` paragrafıyla başlıyor, `{{...}}` kalmamış (yalnız §4.3 bloğundaki `{{VARIABLE}}` kural metni)
- [ ] Wiki-link'ler `[[...]]` formatında; hedefi olmayanlar §7.1'de `📋 planlanan`
- [ ] Yaş ≥ 16: `veli_onayı_gerekli: false` + yaş notu (TMK 18 / COPPA 13 / GDPR 16; "KVKK 16" YANLIŞ)
- [ ] Cihaz: yalnız A34 spec'i VERIFIED; viewport `⚠️ DERIVED`; diğer cihazlar `⚠️ VERIFICATION REQUIRED`
- [ ] WCAG kodları yalnız P5 listesinden; persona renk/tema kontrastı `⚠️ DERIVED`
- [ ] BPM yalnız tür aralığı (P4.4/P4.5); şarkı BPM'i yok
- [ ] Mojibake yok (`verify` → mojibake 0, BOM false)
- [ ] §7.2 Çakışma Kayıtları + §7.3 Değişiklik Geçmişi (append-only) + Authority footer mevcut

### §6.2 Quality Report

| Metrik | Değer |
|--------|-------|
| Versiyon | 1.0.0 |
| Durum | Red Team · Human Mode · Truth Mode verified |
| Bölüm | 7 (H1 + §1-§7) + frontmatter |
| Alan havuzu | 11/11 alt bölüm |
| Test adımı | 17 (asgari 10) |
| Kaynak & Doğrulama satırı | 29 |
| Frontmatter alan | 7 zorunlu + 5 ek |
| Dil | Türkçe (mojibake yasak) |
| Derinlik | ≥ 500 satır (`verify` → `lines`) |
| Kayıt | `log.md` append-only |

---

## §7 Referanslar

### §7.1 Wiki-link Referansları

| Wiki-link | İlişki | Durum / Gerçek hedef |
|-----------|--------|----------------------|
| `[[personas/index]]` | Persona kataloğu — yaş/mood atamasının sahibi | ✅ `.ai/.personas/index.md` |
| `[[personas/persona-template]]` | Persona şablonu — bu dosyanın iskeleti | ✅ `.ai/.templates/personas/persona-template.md` |
| `[[research-bank]]` | Araştırma paketleri P1-P8 — tek gerçek-dünya veri kaynağı | ✅ `.ai/.personas/research-bank.md` |
| `[[mood-taxonomy]]` | Mood küme adları ve tanımları (§3.2.3 / §3.2.4) | ✅ `.ai/.personas/mood-taxonomy.md` |
| `[[personas/test-scenarios-mapping]]` | Test senaryosu eşlemesi | ✅ `.ai/.personas/test-scenarios-mapping.md` |
| `[[personas/methodology]]` | Test seviyeleri (Level 1/2/3) | 📋 planlanan — `.ai/.personas/methodology.md` (bu turda başka agent sorumluluğunda; diskte henüz yok) |
| `[[ADR-023-persona-driven-testing]]` | Persona-driven testing kararı (frozen) | ✅ `.ai/.decisions/accepted/ADR-023-persona-driven-testing.md` |

*`📋 planlanan` satır hedef dosya diskte oluşana kadar kırık kabul edilir; oluştuğunda `✅`ye çevrilir ve `log.md`'ye append edilir.*

### §7.2 Çakışma Kayıtları (eski salt-okunur kaynak ↔ index SSOT)

| Alan | Eski dosya (salt-okunur) | Yeni değer (SSOT) | Karar |
|------|--------------------------|-------------------|-------|
| Yaş | 16 (tutarlı) | 16 | Çelişki yok — [[personas/index]] §6.3 |
| Doğum tarihi | 7 Kasım 2010 (26.09.2026'da 15 yaş yapar) | 5 Eylül 2010 (yaş 16 ile tutarlı) | Yaş SSOT kazanır; kurgusal tarih türetildi |
| Mood | Melankolik (birincil) / Moody (ikincil) | Melankolik / Moody | Tutarlı — index §6.3 + mood-taxonomy §3.2.3/§3.2.4 |
| Kullanıcı ID | `GC-EB-16-MEL-IZM` (eski footer) | `CM-EB-16-MEL-IZM` (şablon §3.5.1 `CM-XX-YY-ZZZ-XXX`) | Şablon formatı kazanır |
| Birincil cihaz | iPhone 15 + MacBook Air M2 (EXCLUDED) | Samsung Galaxy A34 5G (P3 VERIFIED) | [[research-bank]] P3 |
| N ekseni | "Duygusal Denge 2/10" (düşük denge = yoğun dalgalanma) | N = 20 (doğrudan ×10) | Çelişki saklı; eksen yönü yeniden doğrulamak `⚠️ VERIFICATION REQUIRED` (P7.5) |
| BPM tercihi | 50-90 (kişiye özel) | Yalnız tür aralıkları (P4.4) + kurgusal tercih | X-2: şarkı/türe özel doğrulanmamış BPM yazılmaz |
| Fiziksel detay | 40+ satır (cilt, parfüm, beden, ayakkabı 37…) | 6 satır (şablon §3.5.2 yasak listesi) | Şablon kazanır; detaylar bilgi korunumuyla eski dosyada |
| Etki alanındaki istatistikler | Spotify TR 2025, keşif kaynak yüzdeleri, konser sayıları | yazılmadı | §4.5 kuralı 4 (bankada yok → yazma) |

### §7.3 Değişiklik Geçmişi (append-only)

| Tarih | Versiyon | Değişiklik | Yazar |
|-------|----------|------------|-------|
| 2026-09-26 | 1.0.0 | İlk üretim — persona-template §3.5 (11/11) + research-bank P3-P8 etiketli veri; eski salt-okunur dosyadan kimlik/müzik taşındı; ID formatı şablona uyarlandı; doğum tarihi yaş 16 ile tutarlı kılındı; veli_onayı `false` + yaş notu eklendi | vault-updater (persona) |

*(Append-only: bu tabloya yeni satır EKLENİR; mevcut satır değiştirilmez/silinmez.)*

---

**Authority:** Persona — SSOT: personas/genc-kiz/elif-bulut-melankolik
**Last Updated:** 2026-09-26
**Mode:** Red Team · Human Mode · Truth Mode