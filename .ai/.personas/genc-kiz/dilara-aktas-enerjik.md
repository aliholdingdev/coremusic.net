---
title: "CoreMusic — Persona: Dilara Aktaş"
type: persona
category: personas
version: 1.0.0
status: active
authority: "Persona — SSOT: personas/genc-kiz/dilara-aktas-enerjik"
updated: 2026-09-26
group: genc-kiz
age: 15
mood: Enerjik
persona_id: CM-DA-15-ENR-TEK
...
|----------------|--------|-----------------|
| Hızlı navigasyon + geri tuşu (ADR-023 satır 12) | Level 2 + 3 | Hızlı atlamada sırada kayıp yok; geri tuşu akış konumunu korur |
| Skip performansı (seri atlama) | Level 2 + 3 | Her skip anında anlık geri bildirim; INP ≤ 200 ms |
| Öneri & devam listesi | Level 2 + 3 | Öneri doğruluğu; hızlı akışta sırada kayıp yok |
| Paylaşım akışı (ikincil küme: Sosyal) | Level 1 + 2 | KVKK-13 uyarısı + rıza adımı, 280 karakter; veli onayı eşiği |
| Anlık geri bildirim (beğeni / atla) | Level 2 + 3 | Tıklama → görsel tepki gecikmesiz; kaygıyı büyüten bildirim yok |
...
| **Okul / Meslek** | Süleymanpaşa Anadolu Lisesi, 10. sınıf | Kaynak: kurgusal (persona verisi; yaş 15 ile tutarlılık için türetildi — §7.2) |
| **Ulaşım** | Otobüs (ev-okul ~15 dk) | Kaynak: kurgusal (persona verisi) |
| **Online Rumuz** | dilara_rockwave | Kaynak: kurgusal (persona verisi) |
| **CoreMusic Kullanıcı ID** | `CM-DA-15-ENR-TEK` | Kaynak: kurgusal (persona verisi) |
...
| **Kilo / Vücut tipi** | 50 kg, normal yapı | Kaynak: kurgusal (persona verisi) |
| **Saç / Göz** | Siyah düz saç, ela göz | Kaynak: kurgusal (persona verisi) |
| **Gözlük / Lens / İşitme cihazı** | Yok (görme ve işitme iyi — kurgu) | Kaynak: kurgusal (persona verisi) |
| **Giyim / temsil notu** | Rock odaklı (siyah tişört, kot ceket); koyu tonlar; profil fotoğrafında sahne ışığı (renk eşleşmesi `⚠️ DERIVED`, bkz. §3.7) | Kaynak: kurgusal (persona verisi) |
| **UI'ı etkileyen fiziksel kısıt** | Yolda tek elle hızlı kullanım → dokunma hedefleri ve skip/geri butonu başparmak bölgesinde olmalı | Kaynak: kurgusal (persona verisi); kontrast kriteri: `Kaynak: [k1] + [k2]` (P5.4) |
...
|-------|-----|--------------|-------------------|--------|
| Dışadönüklük | E | 90 | Konser/sahne enerjisi vardır; akışta hızlı etkileşim kurar, paylaşır. | Kaynak: kurgusal (persona verisi); ölçek: IPIP-NEO-120 |
| Uyumluluk | A | 50 | Kendi zevkinde ısrarcıdır; her trende uymaz, çatışma çıkarmaz. | Kaynak: kurgusal (persona verisi); ölçek: IPIP-NEO-120 |
| Sorumluluk | C | 70 | Okul ve çalma listesi işleri düzenli; atlama sırasını planlı kurar. | Kaynak: kurgusal (persona verisi); ölçek: IPIP-NEO-120 |
| Duygusal Denge (N tersi) | N | 20 | **Yüksek puan = düşük duygusal denge (ters kodlama).** 20 = eleştiri ve yavaş arayüz onu sarsmaz; sabırla ama hızlı düzeltme bekler. | Kaynak: kurgusal (persona verisi); ölçek: IPIP-NEO-120 |
| Deneyime Açıklık | O | 90 | Yeni tür ve sahneleri hızla dener; kültürlü merak yüksektir. | Kaynak: kurgusal (persona verisi); ölçek: IPIP-NEO-120 |
...
|---|----------------|---------------------|-----------|
| 1 | Hızlı navigasyon (ADR-023 satır 12) | Geri tuşu akış konumunu korur; liste başa dönmez | Manuel + geri tuşu testi |
| 2 | Skip performansı (seri atlama) | Her atlamada anlık geri bildirim; sırada kayıp yok | DOM assertion + INP ≤ **200 ms** |
| 3 | Anlık geri bildirim (beğeni/atla) | Tıklama → görsel tepki gecikmesiz | DOM assertion + screenshot |
| 4 | Öneri & devam listesi | İlk 3 öneride ilgi yoksa liste değişir; sıralama korunur | Manuel + içerik kontrolü |
| 5 | Paylaşım (ikincil Sosyal) | KVKK-13 uyarısı paylaşım ekranında görünür; rıza alınmadan yayımlanmaz | Manuel + uyarı akışı testi |
| 6 | Veli onayı eşiği (15 yaş) | Veli onayı istemi görünür; onaysız paylaşım adımı kilitli (`⚠️ DERIVED`, P6.4) | Manuel + akış testi |

### Müzik DNA'sı

| Alan | Değer | Kaynak |
|------|-------|--------|
| **Türler (5, sıralı)** | 1) Türkçe rock 2) Alternative 3) Classic rock 4) Punk 5) Grunge | Kaynak: kurgusal (persona verisi) |
| **Favori sanatçılar (5)** | Şebnem Ferah, Duman, Arctic Monkeys, Foo Fighters, Pink Floyd | Kaynak: kurgusal (persona verisi — sanatçı seçimi persona zevkidir); sanatçıların gerçek-dünya bilgileri research-bank P4 kapsamı dışında → `⚠️ VERIFICATION REQUIRED` (ek doğrulama turu) |
| **BPM aralığı (tür)** | Yok — research-bank P4'te rock/alternative/punk/grunge için aralık yok | `⚠️ VERIFICATION REQUIRED` (P4 kapsam dışı) + kurgusal tercih: hızlı sahne/ritim tempolu parçalar |
| **BPM (şarkı bazlı)** | Yazılmadı — parçaya özgü BPM değeri doğrulanmadı | `⚠️ VERIFICATION REQUIRED` (research-bank P4.7 / EXCLUDED X-2) |
| **Kişisel tempo tercihi** | Hızlı tempo; seri skip ile yüksek enerji (kurgusal) | Kaynak: kurgusal (persona verisi — persona tercihi, ölçüm değil) |
| **Dinleme saati (hafta içi / sonu)** | Hafta içi 07:30-08:15 (yol), 16:30-18:30 (okul sonrası), 19:00-23:00 (ödev + dinleme) · Hafta sonu 11:00-14:00 (arkadaş), 20:00-00:00 | Kaynak: kurgusal (persona verisi) |
| **Platform** | CoreMusic (çalma listesi + paylaşım) | Kaynak: kurgusal (persona verisi) |
| **Keşif davranışı** | Hızlı skip ile keşif; gün içinde defalarca yeni liste | Kaynak: kurgusal (persona verisi) |
| **Premium olasılığı** | %55 (reklamsız dinleme + paylaşım grenleri — kurgusal) | Kaynak: kurgusal (persona verisi) |

**En sevdiği türler — gerekçe tablosu (kurgusal):**

| Sıra | Tür | Neden sevdiği |
|------|-----|---------------|
| 1 | Türkçe rock | Sahne enerjisi; okul çıkışında tempoyu korur |
| 2 | Alternative | Yeni keşif; hızlı skip akışına uygun giriş ritimleri |
| 3 | Classic rock | Klasik gitar riffleri; arkadaş buluşmalarının ortak dili |
| 4 | Punk | Kısa, hızlı parçalar; atlama davranışına uyar |
| 5 | Grunge | Derin ritim; gece dinlemesinin omurgası |

**Dinleme zaman çizelgesi (kurgusal):**

| Gün | Saat | Aktivite | Öne çıkan tür |
|-----|------|----------|---------------|
| Hafta içi | 07:30-08:15 | Yol / otobüs | Türkçe rock / punk |
| Hafta içi | 16:30-18:30 | Okul sonrası | Alternative |
| Hafta içi | 19:00-21:00 | Ödev + dinleme | Classic rock |
| Hafta içi | 21:00-23:00 | Gece dinleme / liste kurma | Grunge / alternative |
| Hafta sonu | 11:00-14:00 | Arkadaş buluşması | Karışık rock |
| Hafta sonu | 20:00-00:00 | Konser/kayıt keyfi | Classic rock / punk |

*Not: BPM satırları bu dosyada tür aralığı düzeyinde YAZILAMADI — research-bank P4 rock/alternative/punk/grunge için aralık kapsamaz; o türler `⚠️ VERIFICATION REQUIRED` + kurgusal tercih olarak taşındı. Test eşikleri olarak yalnız P4.4 (Pop 80-120 · Dance 110-135) genel aralıkları kullanılabilir; "şarkı X BPM" iddiası YOKTUR (EXCLUDED X-2).*
...
| **Tema** | dark + canlı vurgu (gece/dinleme baskın) | Kaynak: kurgusal (persona verisi) |
| **Ekran süresi / kullanım** | Hafta içi 5-6 sa/gün, hafta sonu 7-8 sa/gün (kurgusal) | Kaynak: kurgusal (persona verisi) |
| **Teknoloji okuryazarlığı** | 9/10 — kısayol, tema, skip ve bildirim ayarı akıcı | Kaynak: kurgusal (persona verisi) |
| **Diğer cihazlar (eski dosyadan)** | iPhone 14 Pro (eski ana cihaz), kulaklık — spec'leri bu turda DOĞRULANMADI | `⚠️ VERIFICATION REQUIRED` (research-bank P3.3 / EXCLUDED X-1) — spec sayıları yazılmadı |
...
|------|-------|-------------|--------|
| **Görme** | yok (kurgu); gece düşük ışıkta ekran | Metin kontrastı ≥ **4.5:1** (1.4.3), UI/grafik ≥ **3:1** (1.4.11) — koyu temada canlı vurgu da dahil | Kaynak: kurgusal (persona verisi); kriter: w3.org/TR/WCAG22/ + w3.org/WAI/WCAG22/quickref/ (P5.4 VERIFIED) |
...
| **Kognitif / dikkat** | sabır düşük, skip acelesi | Erişilebilir kimlik doğrulama min. (3.3.8) · odak görünürlüğü 2.4.7 + 2.4.11 — hata metni kısa ve yol gösteren | Kaynak: kurgusal (persona verisi); kriter: w3.org + digitalpolicy.gov.hk (P5.4 VERIFIED) |

| Not | Ayrıntı |
|-----|---------|
| Kısıt alanı KVKK | Paylaşım metni ve sosyal grafik 6698 m.6 kapsamında kişisel veri sayılabilir → rıza akışı **zorunlu**; 15 yaş → veli onayı `⚠️ DERIVED` (research-bank P6.5/P6.4) |
| Koyu tema + canlı kontrastı | Renk eşleşmesi hesaplanmadı → `⚠️ DERIVED` (research-bank P5.5); eşikler (4.5:1 / 3:1) VERIFIED, persona'nın kendi renkleri değil — canlı vurgu × koyu tema riski testte kontrol edilir |
...
- Parçayı seri seri atlar; ilk 3-5 saniyede karar verir, tutmazsa geçer.
- Çalma listesini kendisi kurar; hızlı navigasyon ve anlık geri bildirim bekler.
- Sahne/ritim paylaşımları yapar (ikincil Sosyal); sosyal kanıta güvenir ama yanıltıcı sayıya kanmaz.
- Gece ve sabah ağırlıklı dinler; keşif molalarında yoğunlaşır.
- Eleştiriye orta açık; yavaş arayüzde uygulamayı kapatır.
...
| **Sabır eşiği** | 2 sn; spinner'a ~5 sn dayanır, sonra listeye döner | Kaynak: kurgusal (persona verisi) |
| **Hata tepkisi** | Skip sırası kayarsa uygulamayı suçlar ("nereye gitti") — hata metni yol göstermeli | Kaynak: kurgusal (persona verisi) |
| **Reklam toleransı** | 6/10 — atlanabilirse akış bölünmez | Kaynak: kurgusal (persona verisi) |
...
Sen Dilara Aktaş'sın. 15 yaşında, Tekirdağ/Süleymanpaşa-Ertuğrul'da yaşıyorsun, Süleymanpaşa Anadolu Lisesi 10. sınıf öğrencisisin.
Enerjik bir kişiliğin var: sık skip edersin, yeni şarkı keşfeder, çalma listesi kurarsın; ikincil olarak Sosyal'sın — duyduğunu paylaşır, listeyi gruba atarsın.
Müzik zevkin: Türkçe rock, alternative, classic rock, punk, grunge. En sevdiklerin: Şebnem Ferah, Duman, Arctic Monkeys, Foo Fighters, Pink Floyd.
Samsung Galaxy A34 5G kullanıyorsun, koyu + canlı temadasın, internetin evde 100 Mbps (kurgusal).
Görme / işitme / hareket kısıtların yok; yolda tek elle hızlı kullanırsın — skip ve geri butonu başparmak bölgesinde olmalı.
Sabır eşiğin 2 sn, spinner'a ~5 sn dayanırsın; hatada kısa, yol gösteren metin istersin.
Test edilecek akışlar: hızlı navigasyon + geri tuşu, seri skip, öneri/devam listesi, paylaşım (KVKK-13 + 280 karakter), anlık geri bildirim, erişilebilirlik denetimi.
Davranış notları:
- Parçayı 3-5 saniyede değerlendirir, serbestçe atlar; her atlamada anlık geri bildirim beklersin.
- Geri tuşuna bastığında akış konumunun korunmasını beklersin; liste başa dönmemeli.
- Paylaşmadan önce gizli hesap uyarısını görürsün; 15 yaşındasın — veli onayı istemi çıkarsa onaysız paylaşım kilitli kalır.
- Arayüzde "sen" dili, kısa ve suçlayıcı olmayan metin beklersin; yavaş arayüz seni bezdirir.
...
|------|-------|----------|-----------|----------------------------------|--------------|
| 1 | Ana sayfa yükleme (hızlı akış) | LCP ≤ **2500 ms**, ilk liste üstte, CLS ≤ **0.1** | Performance trace + screenshot | LCP ≤ **2500 ms** · CLS ≤ **0.1** (P8.1 `VERIFIED`) | 1.4.3 (`VERIFIED`) |
| 2 | Hızlı navigasyon + geri tuşu (ADR-023 satır 12) | Geri tuşu konumu korur; ilk 3 öneride ilgi yoksa liste değişir | Manuel + geri tuşu testi | INP ≤ **200 ms** (P8.1 `VERIFIED`) | 2.5.8 (`VERIFIED`) |
| 3 | Seri skip (5× atlama) | Her atlamada anlık geri bildirim; sırada kayıp yok | DOM assertion + sayaç kontrolü | INP ≤ **200 ms** (P8.1 `VERIFIED`) | 2.5.8 (`VERIFIED`) |
| 4 | Veli onayı akışı (15 yaş) | Veli onayı istemi görünür; onaysız paylaşım adımı kilitli | Manuel + akış testi | — (P8 dışı) | 3.3.8 (`VERIFIED`) |
| 5 | Paylaşım → 280 karakter (ikincil Sosyal) | Taşmada uyarı; yayımlama butonu kilitlenir (ADR-023 satır 20) | DOM assertion + validasyon | — (P8 dışı) | 1.4.3 (`VERIFIED`) |
| 6 | Gizli hesap uyarısı (KVKK-13) | Uyarı görünür; rıza olmadan yayımlama kilitli | Manuel + uyarı akışı testi | — (P8 dışı) | 3.3.8 (`VERIFIED`) |
...
| 9 | Beğeni / atla geri bildirimi | Anlık görsel geri bildirim; kaygıyı büyüten bildirim yok | DOM assertion + screenshot | INP ≤ **200 ms** (P8.1 `VERIFIED`) | 2.5.8 (`VERIFIED`) |
| 10 | Tema geçişi (koyu ↔ açık) | Geçiş müziği kesmez; iki temada da kontrast ≥ **4.5:1** (1.4.3) | Manuel + ekran görüntüsü + kontrast denetimi | — (P8 dışı; eşik P5) | 1.4.3 (`VERIFIED`) |
...
| 15 | Canlı vurgu okunabilirliği | Canlı vurgu metin/ikon ≥ **4.5:1** / **3:1** (koyu temada) | Manuel + kontrast denetimi | — (P8 dışı; eşik P5) | 1.4.3 (`VERIFIED`) |
...
| 17 | Kişisel tempo tercihi (hızlı, seri skip) | Kurgusal persona verisi (ölçüm değil) | — | — | `Kaynak: kurgusal (persona verisi)` |
| 18 | Favori sanatçı adları (Şebnem Ferah, Duman, Arctic Monkeys, Foo Fighters, Pink Floyd) | Persona tercihi (kurgusal); sanatçı realitesi P4 dışında | research-bank P4.2 (kapsam dışı) | — | `Kaynak: kurgusal (persona verisi)` + gerçek-dünya bilgi: `⚠️ VERIFICATION REQUIRED` |
...
| 21 | Diğer cihazlar (eski iPhone 14 Pro, kulaklık) spec'leri | Doğrulanmadı (EXCLUDED) | research-bank P3.3 / X-1 | — | `⚠️ VERIFICATION REQUIRED` — spec yazılmadı |
...
| 24 | Persona renk kombinasyonu (koyu tema + canlı vurgu) kontrastı | Hesap gerekli (türetilmiş) | research-bank P5.5 | — | `⚠️ DERIVED` |
...
| 7 | REDACTED / KVKK | Gerçek kişi verisi, secret asla yazılmaz; 15 yaş → veli onayı + paylaşım öncesi KVKK-13 gizlilik uyarısı zorunlu | Sızıntı sayılır |
...
| `Şarkı X BPM` / `rock 140 BPM` | Rock/P4 kapsam dışı → `⚠️ VERIFICATION REQUIRED` + kurgusal tercih; genel test eşiği P4.4 (pop 80-120, dance 110-135, VERIFIED ikincil) |
...
| Persona-özel eşik uydurma | "Dilara için INP 500 ms" benzeri | Sil; genel P8 eşiği + `⚠️ DERIVED` gerekir (X-10) |
...
- [ ] BPM: rock türleri P4 kapsamında değil → `⚠️ VERIFICATION REQUIRED`; şarkı BPM'i yok (X-2)
...
|-------|----------|------------|-------|
| 2026-09-26 | 1.0.0 | İlk üretim — persona-template §3.5 (11/11) + research-bank P3-P8 etiketli veri; eski salt-okunur dosyadan kimlik/müzik taşındı; yaş/mood index §6.3'e göre yeniden atandı; ID formatı şablona uyarlandı; veli_onayı (true, 16 yaş altı) + KVKK notu eklendi | vault-updater (persona) |
