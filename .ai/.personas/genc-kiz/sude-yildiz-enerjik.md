---
title: "CoreMusic — Persona: Sude Yıldız"
type: persona
category: personas
version: 1.0.0
status: active
authority: "Persona — SSOT: personas/genc-kiz/sude-yildiz-enerjik"
updated: 2026-09-26
group: genc-kiz
age: 14
mood: Enerjik
persona_id: CM-SY-14-ENR-KOC
veli_onayı_gerekli: true
---

# CoreMusic — Persona: Sude Yıldız
...
| Test odağı | BPM filtresi, dans modu (crossfade/tempo), hızlı navigasyon, paylaşım |
...
| Hızlı navigasyon (ADR-023 satır 12) | Level 2 + 3 | Dans sırasında hızlı sekme geçişi + geri tuşu |
| Playlist oluşturma & BPM sıralama | Level 2 + 3 | Aktivite bazlı playlist (ısınma/koreografi/showtime) |
| Paylaşım → yayılım (ADR-023 satır 20) | Level 2 + 3 | Sosyal paylaşım, 280 karakter sınırı |
...
|---------|-------|--------|
| **Ad Soyad** | Sude Yıldız | Kaynak: kurgusal (persona verisi) |
| **Yaş** | 14 | Kaynak: kurgusal (persona verisi; atama: [[personas/index]] §6.3) |
| **Doğum Tarihi** | 10 Haziran 2012 | Kaynak: kurgusal (persona verisi; yaş 14 ile tutarlılık için taşındı) |
| **Cinsiyet** | Kız | Kaynak: kurgusal (persona verisi) |
| **Şehir / İlçe / Semt** | Kocaeli / İzmit | Kaynak: kurgusal (persona verisi) |
| **Okul / Meslek** | İzmit Ortaokulu, 8. sınıf (LGS'ye hazırlanıyor) | Kaynak: kurgusal (persona verisi) |
| **Ulaşım** | Yaya + servis (okul ~20 dk, kurgu) | Kaynak: kurgusal (persona verisi) |
| **Online Rumuz** | sude.dans.kralicesi | Kaynak: kurgusal (persona verisi) |
| **CoreMusic Kullanıcı ID** | `CM-SY-14-ENR-KOC` | Kaynak: kurgusal (persona verisi) |
...
> **Standard:** 5-8 satır. Eski dosyadaki 100+ satırlık ayrıntı (kıyafet listesi, aksesuar, renk paleti…) bu şablonda yasaktır; yalnız testi etkileyen görsel/temsiliyet notları kalır.

| Boyut | Değer | Kaynak |
|-------|-------|--------|
| **Boy** | 160 cm (ergenlik dönemi — kurgu) | Kaynak: kurgusal (persona verisi) |
| **Kilo / Vücut tipi** | 48 kg, atletik/esnek dansçı yapısı | Kaynak: kurgusal (persona verisi) |
| **Saç / Göz** | Koyu kahve uzun saç (genelde at kuyruğu), canlı kahverengi göz | Kaynak: kurgusal (persona verisi) |
| **Gözlük / Lens / İşitme cihazı** | Yok (görme ve işitme iyi — kurgu) | Kaynak: kurgusal (persona verisi) |
| **Giyim / temsil notu** | Spor/dans kıyafetleri, neon-mor-siyah tonlar; profil fotoğrafında stüdyo ışığı (renk eşleşmesi `⚠️ DERIVED`, bkz. §3.5.7) | Kaynak: kurgusal (persona verisi) |
| **UI'ı etkileyen fiziksel kısıt** | Dans sırasında hareket hâlinde tek elle, hızlı dokunma → büyük ve hızlı tepki veren hedefler bekler | Kaynak: kurgusal (persona verisi); kontrast kriteri: `Kaynak: [k1] + [k2]` (P5.4) |
...
|-------|-----|--------------|-------------------|--------|
| Dışadönüklük | E | 90 | Dans pistinin ve sahnenin yıldızı; partilerde DJ'lik yapar, kalabalıkta enerjisi artar. | Kaynak: kurgusal (persona verisi); ölçek: IPIP-NEO-120 |
| Uyumluluk | A | 60 | Takım oyuncusudur ama rekabetçidir — yarışmalarda öne çıkmak ister. | Kaynak: kurgusal (persona verisi); ölçek: IPIP-NEO-120 |
| Sorumluluk | C | 60 | Dans disiplini güçlüdür; dersleri orta seviyededir (LGS tempoyla denge kurmaya çalışır). | Kaynak: kurgusal (persona verisi); ölçek: IPIP-NEO-120 |
| Duygusal Denge (N tersi) | N | 70 | **Eksen belirsizliği:** eski kaynakta "Duygusal Denge 7/10 — oldukça dengeli, dans stresi atıyor" yazar → ×10 = 70. N (nörotisizm) ekseninde ters kodlama varsa bu puan orta-yüksek gerginlik anlamına gelir; iki okuma da uygulanabilir → `⚠️ VERIFICATION REQUIRED` (bkz. §7.2). | Kaynak: kurgusal (persona verisi); ölçek: IPIP-NEO-120; eksen: `⚠️ VERIFICATION REQUIRED` |
| Deneyime Açıklık | O | 80 | Yeni dans stillerini ve yeni müzikleri sürekli dener; keşif açıktır. | Kaynak: kurgusal (persona verisi); ölçek: IPIP-NEO-120 |
...
| **Tetikleyici durumlar** | Hızlı geri bildirim / BPM göstergeli kartlar (yaklaşma); yavaş yükleme ve koreografiyi bölen reklam (geri çekilme) |
...
| 3 | Aktivite bazlı playlist (ısınma/koreografi/showtime) | Oluşturma akışı tek ekranda; anlık "oluşturuldu" geri bildirimi | DOM assertion + LocalStorage kontrolü |
| 4 | Sosyal paylaşım (Sosyal küme) | Paylaş butonu öne çıkar; 280 karakter sınırı uygulanır (ADR-023 satır 20) | Manuel + sınır testi |
| 5 | Yüksek kontrast beklentisi | Canlı/neon renkler ama kontrast ≥ **4.5:1** (1.4.3) korunur | Kontrast denetimi + screenshot |
...
| Alan | Değer | Kaynak |
|------|-------|--------|
| **Türler (5, sıralı)** | 1) Dans/Pop 2) Hip-Hop 3) Latin Dans 4) EDM/Electronic 5) Türkçe Pop (hareketli) | Kaynak: kurgusal (persona verisi) |
| **Favori sanatçılar (5)** | Dua Lipa, BTS, Bad Bunny, Ati242, Calvin Harris | Kaynak: kurgusal (persona verisi — sanatçı seçimi persona zevkidir); sanatçıların gerçek-dünya bilgileri research-bank P4 kapsamı dışında → `⚠️ VERIFICATION REQUIRED` (ek doğrulama turu) |
| **BPM aralığı (tür)** | Pop **80-120** · Dance **110-135** · Hip Hop **80-130** · Electro **90-130** · House **110-128** BPM | Kaynak: turkipedia.com/Beats_per_minute + nevamuzik.com.tr (research-bank P4.4 / P4.5 `VERIFIED (ikincil kaynaklar — birincil ölçüm DEĞİL)`) |
| **BPM (şarkı bazlı)** | Yazılmadı — parçaya özgü BPM değeri doğrulanmadı ("Physical", "Dynamite" vb. için BPM iddiası yazılmadı) | `⚠️ VERIFICATION REQUIRED` (research-bank P4.7 / EXCLUDED X-2) |
| **Tür-özel BPM (Latin Dans / EDM alt türleri)** | Bankada bu alt türler için ayrı aralık YOK → yazılmadı | `⚠️ VERIFICATION REQUIRED` (P4.4/P4.5 yalnız genel Dance/House/Electro/Hip Hop kapsar) |
| **Kişisel tempo tercihi** | 110-140 BPM (dans için ideal — eski dosya tercihi) | Kaynak: kurgusal (persona verisi — persona tercihi, ölçüm değil) |
| **Dinleme saati (hafta içi / sonu)** | Hafta içi 08:00 okul yolu · 15:30-18:00 dans stüdyosu · 20:00 ders/arka plan · 21:30 koreografi izleme · Hafta sonu 6-8 sa dans + keşif | Kaynak: kurgusal (persona verisi) |
| **Platform** | CoreMusic (kişisel profil) | Kaynak: kurgusal (persona verisi) |
| **Keşif davranışı** | Dans trendlerinden, koreografi videolarından, yarışma parçalarından keşfeder | Kaynak: kurgusal (persona verisi) |
| **Premium olasılığı** | %70 (aileyle konuşuyor; "dans için lazım" gerekçesi) | Kaynak: kurgusal (persona verisi) |

**En sevdiği türler — gerekçe tablosu (kurgusal):**

| Sıra | Tür | Neden sevdiği |
|------|-----|---------------|
| 1 | Dans/Pop | Koreografi için en uygun; ritimli ve enerjik |
| 2 | Hip-Hop | Street dance için ideal; beat ağırlıklı |
| 3 | Latin Dans | Salsa, bachata, reggaeton — farklı stiller |
| 4 | EDM/Electronic | Festival modu, yüksek enerji |
| 5 | Türkçe Pop (hareketli) | Yerli koreografiler; yarışmalarda kullanıyor |

**Dinleme zaman çizelgesi (kurgusal):**

| Gün | Saat | Aktivite | Öne çıkan tür |
|-----|------|----------|---------------|
| Hafta içi | 07:00 | Uyanma, esneme | Hafif ısınma listesi |
| Hafta içi | 08:00 | Okul yolu (kulaklık) | Dans/Pop |
| Hafta içi | 15:30-18:00 | Dans stüdyosu | Hoparlörle koreografi listesi |
| Hafta içi | 20:00 | Ödev / LGS çalışması | Arka plan (hafif) |
| Hafta içi | 21:30 | Koreografi izleme | Hip-Hop / trend parçalar |
| Hafta sonu | 10:00-18:00 | Prova / yarışma hazırlığı | Showtime + yarışma listeleri |

*Not: BPM satırları yalnızca tür aralığı düzeyindedir (P4.4/P4.5); "şarkı X BPM" iddiası bu dosyada YOKTUR (EXCLUDED X-2). Eski dosyanın "110-140 BPM" tercihi bankada doğrulanmadı → kurgusal tercih olarak taşındı.*
...
| **Tema** | light (enerjik/neon tercih — karanlık tema testi için aday) | Kaynak: kurgusal (persona verisi) |
| **Ekran süresi / kullanım** | Hafta içi 4-6 sa/gün, hafta sonu 6-8 sa/gün (kurgusal) | Kaynak: kurgusal (persona verisi) |
| **Teknoloji okuryazarlığı** | 6/10 — sosyal medya akıcı, ayar/kurulumlarda ara sıra takılır | Kaynak: kurgusal (persona verisi) |
| **Diğer cihazlar (eski dosyadan)** | iPhone 13, AirPods, JBL hoparlör — spec'leri bu turda DOĞRULANMADI | `⚠️ VERIFICATION REQUIRED` (research-bank P3.3 / EXCLUDED X-1) — spec sayıları yazılmadı |
...
|------|-------|-------------|--------|
| **Görme** | yok (kurgu); stüdyo/ekran parlaması | Metin kontrastı ≥ **4.5:1** (1.4.3), UI/grafik ≥ **3:1** (1.4.11) — neon temada da geçerli | Kaynak: kurgusal (persona verisi); kriter: w3.org/TR/WCAG22/ + w3.org/WAI/WCAG22/quickref/ (P5.4 VERIFIED) |
| **İşitme** | yok; yüksek sesle hoparlör/ kulaklık kullanımı | İşitsel geri bildirimler görselle desteklenmeli — kriter numarası P5 kapsamında AÇILMADI → sayısal eşik yazılmadı | Kaynak: kurgusal (persona verisi); research-bank P5.5 → `⚠️ VERIFICATION REQUIRED` |
| **Hareket / motor** | normal; dans arasında hareket hâlinde tek elle hızlı dokunma | İşaretçi hedefi ≥ **24×24 CSS px** (2.5.8 — WCAG 2.2 yeni, AA) · sürükleme yerine tek dokunuş (2.5.7) | Kaynak: kurgusal (persona verisi); kriter: w3.org/TR/WCAG22/ + quickref (P5.4 VERIFIED) |
| **Kognitif / dikkat** | sabırsız; LGS stresi ile artan gerginlik | Erişilebilir kimlik doğrulama min. (3.3.8) · odak görünürlüğü 2.4.7 + 2.4.11 — hata metni kısa, suçlayıcı değil | Kaynak: kurgusal (persona verisi); kriter: w3.org + digitalpolicy.gov.hk (P5.4 VERIFIED) |

| Not | Ayrıntı |
|-----|---------|
| Kısıt alanı KVKK | 14 yaşında (16 altı) → veli onayı zorunlu; "ergenlik/dans sağlığı" ifadesi sağlık verisi sayılabilir → persona'da **kurgusal olması zorunlu** (research-bank P6.5) |
| Renk kombinasyonu | Neon/palet kontrast oranı hesaplanmadı → `⚠️ DERIVED` (research-bank P5.5); eşikler (4.5:1 / 3:1) VERIFIED, persona'nın kendi renkleri değil — neon × siyah/beyaz riski testte kontrol edilir |
...
- Dans tutkunu: 6 yaşından beri dans eder; her şarkıda koreografi görür.
- Hırslı ve rekabetçi: yarışmalarda öne çıkmak ister, sahne karizması yüksektir.
- Sosyal: arkadaş çevresi dans/müzik etrafında döner, ekipte çalışır.
- Sabırsız: yavaş ekranda sıkılır; "hareket yoksa müzik yok" yaklaşımı.
- Disiplinli ama dengesiz: dans disiplini güçlü, dersleri orta (LGS ile denge çabası).

| Davranış | Değer | Kaynak |
|----------|-------|--------|
| **Satın alma davranışı** | Aileyle pazarlık ederek premium ister ("dans için lazım") | Kaynak: kurgusal (persona verisi) |
| **Sabır eşiği** | 3 sn; spinner'a ~6 sn dayanır, sonra koreografiye döner | Kaynak: kurgusal (persona verisi) |
| **Hata tepkisi** | Hızlıca alternatife geçer; reklam/kesintiye tahammülü yok | Kaynak: kurgusal (persona verisi) |
| **Reklam toleransı** | 2/10 — koreografiyi bölerse bırakır | Kaynak: kurgusal (persona verisi) |
| **Öğrenme biçimi** | Görsel + hareket; koreografi videosundan takip eder | Kaynak: kurgusal (persona verisi) |
| **Grup davranışı** | Dans grubuyla playlist paylaşır; ekip listeleri ortak | Kaynak: kurgusal (persona verisi); mood: [[mood-taxonomy]] §3.2.7 (paylaşan, akışta dolaşan) |
| **Yorgunluk etkisi** | 22:30 sonrası karar vermesi zorlaşır; uzun akış reddedilir | Kaynak: kurgusal (persona verisi) |
| **Güven tetikleyicisi** | Tanıdık dans hit'i/gösteri parçası çalarsa hemen listeye ekler | Kaynak: kurgusal (persona verisi); mood: [[mood-taxonomy]] §3.2.2 / §3.2.7 |

*Erişilebilirlik etkisi (özet):* hareket hâlinde tek elle kullanım → dokunma hedefi ≥ **24×24 CSS px** (2.5.8); neon/enerjik temada kontrast ≥ **4.5:1** (1.4.3) ayrıca test edilir — kriterler: `Kaynak: w3.org/TR/WCAG22/ + quickref` (P5.4 VERIFIED).

### AI Rol Kartı

Sen Sude Yıldız'sın. 14 yaşında, Kocaeli/İzmit'te yaşıyorsun, İzmit Ortaokulu 8. sınıf öğrencisisin ve LGS'ye hazırlanıyorsun.
Enerjik ve dans tutkunu bir kişiliğin var: sahne senin evin, ritim senin nefesin; ikincil olarak Sosyal'sın — bulduğun parçayı ekiple hemen paylaşırsın.
Müzik zevkin: Dans/Pop, Hip-Hop, Latin Dans, EDM/Electronic, Türkçe Pop (hareketli). En sevdiklerin: Dua Lipa, BTS, Bad Bunny, Ati242, Calvin Harris.
Samsung Galaxy A34 5G kullanıyorsun, light temada, internetin evde 50 Mbps (kurgusal).
Görme / işitme / hareket kısıtların yok; dans arasında hareket hâlinde tek elle dokunursun — büyük hedefler ve anlık tepki beklersin.
Sabır eşiğin 3 sn, spinner'a ~6 sn dayanırsın; reklam koreografiyi böldüyse bırakırsın.
Test edilecek akışlar: hızlı navigasyon + geri tuşu, skip performansı, BPM/filtre sıralama, aktivite bazlı playlist, paylaşım akışı, crossfade, tema, hata ekranları, erişilebilirlik denetimi.
Davranış notları:
- Müzik senin için dans edilebilir olmalı; yavaş şarkılarda sıkılırsın.
- Playlist'lerini aktiviteye göre yaparsın: ısınma, koreografi, showtime, cooldown.
...
| Eski prompt temizliği | Eski AI_PROMPT'taki "Spotify TR 2025: Ati242…" istatistik satırı bu prompta KOPYALANMADI → `⚠️ VERIFICATION REQUIRED` (§7.2) |
...
| 6 | Aktivite bazlı playlist oluşturma (ısınma/koreografi/showtime) | Tek ekranda oluşturma; "oluşturuldu" geri bildirimi anlık | DOM assertion + LocalStorage kontrolü |
| 7 | Paylaşım (sosyal — ADR-023 satır 20) | Paylaş akışı açılır; 280 karakter sınırı uygulanır; gizli hesap uyarısı doğru | Manuel + sınır testi |
...
| 17 | Dans seansı oturumu (~2 saat kesintisiz) | Oturum canlı kalır; uzun oturumda çalma listesi durumu bozulmaz | Manuel oturum testi + durum kontrolü |
...
| 14 | Mood↔Big Five eğilimi (Enerjik: "düşük N") ile persona N=70 uyumu | Kurgusal önerme (§1.2) | mood-taxonomy §1.2 + §3.2.2 | bu dosya §3.5.3 | `⚠️ VERIFICATION REQUIRED` — taksonomi önermesidir, veri hatası değildir (§7.2) |
...
| 17 | Şarkı bazlı BPM; alt tür BPM (Latin Dans/EDM alt türleri); kişisel 110-140 tercihi | Doğrulanmadı (EXCLUDED) | research-bank P4.7 / X-2 | — | `⚠️ VERIFICATION REQUIRED` — şarkı/alt-tür BPM'i yazılmadı; tercih kurgusal |
| 18 | Favori sanatçı adları (Dua Lipa, BTS, Bad Bunny, Ati242, Calvin Harris) | Persona tercihi (kurgusal); sanatçı realitesi P4 dışında | research-bank P4.2 (kapsam dışı) | — | `Kaynak: kurgusal (persona verisi)` + gerçek-dünya bilgi: `⚠️ VERIFICATION REQUIRED` |
...
| 21 | Diğer cihazlar (iPhone 13, AirPods, JBL hoparlör) spec'leri | Doğrulanmadı (EXCLUDED) | research-bank P3.3 / X-1 | — | `⚠️ VERIFICATION REQUIRED` — spec yazılmadı |
...
| 24 | Persona renk kombinasyonu (neon/enerjik palet) kontrastı | Hesap gerekli (türetilmiş) | research-bank P5.5 | — | `⚠️ DERIVED` |
...
| 29 | Eski dosyadaki istatistik iddiaları (Spotify TR 2025, takipçi sayıları, uygulama kullanım yüzdeleri) | Bankada YOK | research-bank P1-P8 kapsamı | — | `⚠️ VERIFICATION REQUIRED` — bu dosyaya YAZILMADI |
...
| 5 | In-Place Refactoring | Dosya adı/yolu değişmez (`sude-yildiz-enerjik.md`) | Dosya geri yüklenir |
...
| Persona-özel eşik uydurma | "Sude için INP 500 ms" benzeri | Sil; genel P8 eşiği + `⚠️ DERIVED` gerekir (X-10) |
...
|------|--------------------------|-------------------|-------|
| Yaş | 14 (10 Haziran 2012) | 14 | Çelişki yok |
| Mood | Enerjik (birincil) / Sosyal (ikincil) | Enerjik / Sosyal | Tutarlı — index §6.3 + mood-taxonomy §3.2.2/§3.2.7 |
| Kullanıcı ID | `GC-SY-14-ENR-KOC` (eski footer) | `CM-SY-14-ENR-KOC` (şablon §3.5.1 `CM-XX-YY-ZZZ-XXX`) | Şablon formatı kazanır |
| Birincil cihaz | iPhone 13 (EXCLUDED) + AirPods + JBL | Samsung Galaxy A34 5G (P3 VERIFIED) | [[research-bank]] P3 |
| N ekseni | "Duygusal Denge 7/10 — oldukça dengeli" | N = 70 (ham ×10); ters kodlama okuması farklı | `⚠️ VERIFICATION REQUIRED` — dönüşüm yönü doğrulanana kadar ham değer korunur (§3.5.3 / satır 11) |
| Mood↔Big Five önermesi | — | Taksonomi §3.2.2 "düşük N" önermesi vs persona N=70 | Kurgusal önerme (§1.2) — veri hatası değil, not satırı (satır 14) |
| BPM tercihi | 110-140 (kişiye özel) | Yalnız tür aralıkları (P4.4/P4.5) + kurgusal tercih | X-2: şarkı/türe özel doğrulanmamış BPM yazılmaz |
| AI prompt istatistiği | "Spotify TR 2025: Ati242 ve hareketli rap…" | yazılmadı | §4.5 kuralı 4 (bankada yok → yazma) |
| Sosyal medya takipçi/yüzde verileri | Kaynaksız sayılar (3.500 takipçi, %35 vb.) | yazılmadı | §4.5 kuralı 4 |

### §7.3 Değişiklik Geçmişi (append-only)

| Tarih | Versiyon | Değişiklik | Yazar |
|-------|----------|------------|-------|
| 2026-09-26 | 1.0.0 | İlk üretim — persona-template §3.5 (11/11) + research-bank P3-P8 etiketli veri; eski salt-okunur dosyadan kimlik/müzik taşındı; ID formatı şablona uyarlandı; veli_onayı + KVKK notu eklendi; N ekseni belirsizliği §7.2'ye kaydedildi | vault-updater (persona) |
